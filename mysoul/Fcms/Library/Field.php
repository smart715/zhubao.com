    ...
    class Field {
        ...
        public function toform($id, $field, $data = [], $show = 0) {
            ...
            foreach ($field as $t) {
                ...
                if (!IS_ADMIN) {
                    if (!$t['ismember']) {
                        ...
                        && array_intersect(\Soulcms\Service::C()->member['groupid'], $t['setting']['show_member'])) {
                    ...
                    switch ($t['fieldtype']) {
                        ...
                        case 'Pays':
                            $value = [
                                ...
                                'sku' => mys_string2array($data[$t['fieldname'].'_sku']),
            ...
            if ($merge) {
                ...
                <div class="portlet light bordered" id="mys_{name}">
        ...
        public function sys_field($field) {
                ...
                'author' => array(
                    ...
                    'setting' => array(
                        'option' => array(
                            ...
                            'name' => '资料',
                            'icon' => 'fa fa-user',
                            'func' => 'mys_show_member',
                            'extend_field' => 'uid',
                            'extend_function' => 'member:uid',
                            'value'	=> \Soulcms\Service::C()->member['username']
                        ),
                        'validate' => array(
                            'tips' => (IS_ADMIN ? '</span><span class="help-block"><input name="no_author" type="checkbox" value="1" /> '.mys_lang('不验证账号').'</label>' : ''),
                            'check' => '_check_member',
                            'required' => 1,
                        )
                    )
                ...
                'inputip' => array(
                    ...
                    'setting' => array(
                        'option' => array(
                            ...
                            'func' => 'mys_show_ip',
                            'value' => \Soulcms\Service::L('input')->ip_address()
                        )
                    )
                ),
                'displayorder' => array(
            ...
            if (is_file(MYPATH.'Config/Sys_field.php')) {
                ...
                $system = mys_array22array($system, $sys2);
            }
        ...
        public function get($name, $id = 0, $post = []) {
            ...
            if (!isset($this->objects[$name])) {
                if ($this->app && is_file(mys_get_app_dir($this->app).'Fields/'.$name.'.php')) {
                    $class = '\\My\\Field\\'.$this->app.'\\'.$name;
                    require mys_get_app_dir($this->app).'Fields/'.$name.'.php';
                } elseif (is_file(MYPATH.'Field/'.$name.'.php')) {
                    $class = '\\My\\Field\\'.$name;
                } else {
                    $class = '\\Soulcms\\Field\\'.$name;
                }
        ...
        public function type($name) {
            ...
            if (is_file(MYPATH.'Field/Field.php')) {
                ...
                $my && $type = mys_array2array($type, $my);
            }
            // 应用目录自定义字段类别
            if ($this->app && is_file(mys_get_app_dir($this->app).'Fields/Field.php')) {
                $my = require mys_get_app_dir($this->app).'Fields/Field.php';
                $my && $type = mys_array2array($type, $my);
            }
            // 组合组装
            foreach ($type as $i => $t) {
            ...
            foreach ($data as $n => $value) {
                if (isset($fields[$n]) && $fields[$n]) {
                    if ($n == 'content' && $fields[$n]['fieldtype'] == 'Ueditor' && strpos($value, '_ueditor_page_break_tag_') !== FALSE) {
                        ...
                        $content = $title = [];
                        $data['_'.$n] = $value;
                        $content[$page]['title'] = mys_lang('第%s页', $page);
                        ...
                        foreach ($match[0] as $i => $t) {
                            ...
                            $content[$page]['title'] = trim($match[1][$i]) ? trim($match[1][$i]) : mys_lang('第%s页', $page);
                        ...
                        $data[$n.'_sku'] = mys_string2array($data[$n.'_sku']);
...
<div class="form-group" id="mys_row_{name}">
    ...
    <div class="col-md-9">{value}</div>
        ...
        public function insert_value($field) {
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = \Soulcms\Service::L('Field')->post[$field['fieldname']];
        }
        ...
        public function member_field_select() {
            ...
            $str.= '<option value="username"> '.mys_lang('会员名称').' </option>';
            $str.= '<option value="email"> '.mys_lang('会员邮箱').' </option>';
            $str.= '<option value="groupid"> '.mys_lang('会员组ID').' </option>';
            $str.= '<option value="levelid"> '.mys_lang('会员等级ID').' </option>';
            $str.= '<option value="name"> '.mys_lang('姓名').' </option>';
            $str.= '<option value="phone"> '.mys_lang('电话').' </option>';
            // 这里要改的
            if (\Soulcms\Service::C()->member_cache['field']) {
                foreach (\Soulcms\Service::C()->member_cache['field'] as $field => $t) {
        ...
        public function get_default_value($value) {
            if (preg_match('/\{(\w+)\}/', $value, $match)) {
                return isset(\Soulcms\Service::C()->member[$match[1]]) ? \Soulcms\Service::C()->member[$match[1]] : '';
            }
            return $value;
        }
        ...
        public function field_type($name = NULL, $length = NULL) {
            ...
            } elseif (mys_count($this->fieldtype) > 1) {
                $select	= '<option value="">-</option>';
		...
		<div class="form-group">
			...
			<div class="col-md-9">
				...
				<span class="help-block">'.mys_lang('根据你的实际情况选择字段类型，如果你不懂MySQL数据库知识就不要填写此项').'</span>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('存储长度/值').' </label>
			<div class="col-md-9">
				<label><input type="text" class="form-control" size="10" value="'.$length.'" name="data[setting][option][fieldlength]"></label>
				<span class="help-block">'.mys_lang('如果你不懂MySQL数据库知识就不要填写此项').'</span>
			</div>
		</div>';
        ...
        public function attachment($id) {
            ...
            $html.= '<option value="0"> '.mys_lang('本地存储').' </option>';

            $remote = \Soulcms\Service::C()->get_cache('attachment');
            if ($remote) {
                foreach ($remote as $i => $t) {
                    $html.= '<option value="'.$i.'" '.($i == $id ? 'selected' : '').'> '.mys_lang($t['name']).' </option>';
                }
            }
			...
			<label class="col-md-2 control-label">'.mys_lang('附件存储策略').' </label>
			<div class="col-md-9">
				'.$html.'
			</div>
		</div>';
