...
class Router
{
    ...
    public function go_member_login($url) {
        ...
        $url = \Soulcms\Service::L('Router')->member_url('login/index', ['back' => urlencode($url)]);
        mys_redirect($url);
        exit;
    }
    ...
    {
        $name = md5($_SERVER['HTTP_USER_AGENT'] . SELF . $uri . \Soulcms\Service::C()->uid . SITE_ID . \Soulcms\Service::L('input')->ip_address());
        $value = \Soulcms\Service::L('cache')->init()->get($name);
        if ($value) {
            $uri = $value[0];
            $param = mys_array22array($param, $value[1]);
        }
    ...
    public function set_back($uri, $param = [], $nuri = '')
    {
        ...
        \Soulcms\Service::L('cache')->init()->save(
            $name,
            [$nuri ? $nuri : $uri, $param],
            3600
        );
    ...
    function is_redirect($type, $url)
    {
        ...
        if (defined('IS_NOT_301')) {
            ...
            return; // 排除移动端
        } elseif (\Soulcms\Service::_is_mobile()) {
            return; // 排除移动端
        ...
        // 跳转
        $url != mys_now_url() && mys_redirect($url, 'location', '301');
    }
        ...
        switch (mys_count($url)) {
            ...
            case 3:
                ...
                if (is_dir(APPSPATH . ucfirst($uri['s'])) && trim($self, '/') == 'index.php' && !IS_MEMBER) {
                    $mod = \Soulcms\Service::C()->get_cache('module-' . SITE_ID . '-' . $uri['s']);
                    if ($mod['domain']) {
                        unset($uri['s']);
                        $self = (\Soulcms\Service::IS_PC() ? $mod['url'] : mys_mobile_url($mod['url'])) . 'index.php';
                    } else {
                        $self = (\Soulcms\Service::IS_PC() ? SITE_URL : SITE_MURL) . 'index.php';
                    }
                }
                $uri['c'] = $url[1];
                $uri['m'] = $url[2];
    ...
    function member_url($url = '', $query = [], $self = 'index.php')
    {
        ...
        switch (mys_count($url)) {
            ...
            case 3:
                ...
                $url[0] != 'member' && is_dir(mys_get_app_dir($url[0])) && $uri['app'] = $url[0];
                $uri['c'] = $url[1];
                $uri['m'] = $url[2];
                break;
        }
        ...
        // 未绑定域名的情况下
        return (\Soulcms\Service::IS_PC() ? SITE_URL : SITE_MURL) . $self . '?' . @http_build_query($uri);
    }
    ...
    function category_url($mod, $data, $page = 0, $fid = 0)
    {
        ...
        // 获取自定义URL
        $rule = isset($data['setting']['urlrule']) ? \Soulcms\Service::L('cache')->get('urlrule', (int)$data['setting']['urlrule'], 'value') : 0;
        if ($rule && $rule['list']) {
            // URL模式为自定义，且已经设置规则
            $data['fid'] = $fid;
    ...
    function show_url($mod, $data, $page = 0)
    {
        ...
        $rule = \Soulcms\Service::L('cache')->get('urlrule', (int)$cat['setting']['urlrule'], 'value');
        if ($rule && $rule['show']) {
            // URL模式为自定义，且已经设置规则
            $data['modname'] = $mod['dirname'];
            ...
            $data['fid'] = defined('IS_PLUS_FENZHAN') ? mys_get_show_fid($data) : 0;
    ...
    function page_url($data, $page = 0)
    {
        ...
        $rule = \Soulcms\Service::L('cache')->get('urlrule', (int)$data['setting']['urlrule'], 'value');
        if ($rule && $rule['page']) {
            // URL模式为自定义，且已经设置规则
    ...
    function tag_url($name)
    {
        ...
        } elseif (!mys_is_app('tag')) {
            return '关键词库应用没有安装';
        }

        // PC端
        $cfg = \Soulcms\Service::M('app')->get_config('tag');
        $rule = \Soulcms\Service::L('cache')->get('urlrule', (int)$cfg['urlrule'], 'value');;
        if ($rule && $rule['tag']) {
            $data['tag'] = $name;
    ...
    function module_url($mod, $sid)
    {
        ...
        if ($mod['site'][$sid]['domain']) {
            return mys_http_prefix($mod['site'][$sid]['domain']) . '/';
        }

        // 自定义规则的情况下
        $rule = \Soulcms\Service::L('cache')->get('urlrule', (int)$mod['urlrule'], 'value', 'module');
        if ($rule) {
    ...
    function search_url($params = [], $name = '', $value = '', $mid = '', $fid = SITE_FID)
    {
        ...
        $mod = \Soulcms\Service::L('cache')->get('module-' . SITE_ID . '-' . $dir);
        if (!$mod) {
            return '模块[' . $dir . ']缓存不存在';
        }
        ...
        $rule = \Soulcms\Service::L('cache')->get('urlrule', (int)$mod['urlrule'], 'value');
        if ($rule && $rule['search']) {
            $fid && $data['fid'] = $fid;
            $data['modname'] = $mod['dirname'];
            $data['param'] = mys_search_rewrite_encode($params, $mod['setting']['search']);
    ...
    public function furl($fid)
    {
        ...
        // 自定义规则
        $rule = \Soulcms\Service::L('cache')->get('urlrule', SITE_REWRITE, 'value');
        if ($rule && $rule['findex']) {
    ...
    public function remove_domain($url, $doamin = '')
    {
        ...
        if (!$this->_temp['domain']) {
            foreach (\Soulcms\Service::C()->site_domain as $u => $i) {
    ...
    public function get_rewrite_code() {
        ...
        $data = \Soulcms\Service::M()->table('urlrule')->getAll();
        if (!$data) {
            return mys_return_data(0, mys_lang('你没有设置URL规则'));
        }
        ...
        $write = []; // 防止重复
        foreach ($data as $r) {
            $value = mys_string2array($r['value']);
            if ($r['type'] == 1) {
                // 独立模块
        ...
        return mys_return_data(1, mys_lang('生成成功'), [
            'code' => $code,
            'error' => $error,
        ]);
    }
