...
class Form
{
    ...
    public function auto_form_data($name, $data) {
        // 默认数据
        $dt = \Soulcms\Service::L('cache')->init('file')->get($name);
        if (!$dt) {
            return $data;
        }
        $dt['is_form_cache'] = 1;
    ...
    public function auto_form_data_delete($name) {
        \Soulcms\Service::L('cache')->init('file')->delete($name);
    }
    ...
    public function validation($data, $config, $fields = [], $old = []) {
        ...
        if ($config) {
            foreach ($config as $name => $t) {
                ...
                if ($t['length'] && strlen($data[$name]) > $t['length']) {
                    return [[], ['name' => $name, 'error' => mys_lang('长度不规范')]];
                }
                // 规则验证
                if ($t['rule']) {
        ...
        if ($fields) {
            ...
            foreach ($fields as $fid => $field) {
                // 验证字段对象的有效性
                $obj = \Soulcms\Service::L('Field')->get($field['fieldtype']);
                if (!$obj) {
                    continue; // 对象不存在
                ...
                if ($field['fieldtype'] != 'Group' && $validate['required']) {
                    if ($value == '') {
                        // 验证值为空
                        return [[], ['name' => $name, 'error' => $validate['errortips'] ? $validate['errortips'] : mys_lang('%s不能为空', $field['name'])]];
                    ...
                    if (!is_array($value) && $validate['pattern'] && !preg_match($validate['pattern'], $value)) {
                        return [[], ['name' => $name, 'error' => $field['name'].'：'.($validate['errortips'] ? $validate['errortips'] : mys_lang('格式不正确'))]];
                    }
                }
                // 编辑器长度判断
                if ($field['fieldtype'] == 'Ueditor' && strlen($data[$name]) > 1000000) {
                    return [[], ['name' => $name, 'error' => mys_lang('%s长度超限', $field['name'])]];
                }
                // 函数/方法校验
                if ($validate['check']) {
                ...
                if ($field['ismain'] && $field['setting']['option']['unique']) {
                    if (\Soulcms\Service::C()->init['table']) {
                        $table = \Soulcms\Service::M()->dbprefix(\Soulcms\Service::C()->init['table']);
                        if (\Soulcms\Service::M()->db->fieldExists($name, $table)) {
                            $rt = \Soulcms\Service::M()->db->table(\Soulcms\Service::C()->init['table'])->where('id<>', $this->id)->where($name, $post[$name])->countAllResults();
                            if ($rt) {
                                return [[], ['name' => $name, 'error' => mys_lang('%s已经存在', $field['name'])]];
                            }
                        } else {
            ...
            foreach ($fields as $field) {
                // 验证字段对象的有效性
                $obj = \Soulcms\Service::L('Field')->get($field['fieldtype'], $this->id, $post);
                if (!$obj) {
                    continue; // 对象不存在
                }
                ...
                if (SYS_ATTACHMENT_DB) {
                    ...
                    list($add_id, $del_id) = $obj->attach(
                        \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']],
                        $old[$field['fieldname']]
                    );
            ...
            // 格式化后的数据
            $data = \Soulcms\Service::L('Field')->data;
            // 获取uid
        }
    ...
    public function check_captcha($id) {
        ...
        $data = \Soulcms\Service::L('input')->post($id);
        if (!$data) {
            return false;
        }

        $code = \Soulcms\Service::C()->session()->get('captcha');
        if (strtolower($data) == strtolower($code)) {
            \Soulcms\Service::C()->session()->remove('captcha');
            return true;
        }
    ...
    public function check_username($value) {
        ...
        if (!$value) {
            return false;
        } elseif (\Soulcms\Service::C()->member_cache['register']['preg']
            && !preg_match(\Soulcms\Service::C()->member_cache['register']['preg'], $value)) {
            return false;
        } elseif (\Soulcms\Service::C()->member_cache['register']['notallow']
            && in_array($value, \Soulcms\Service::C()->member_cache['register']['notallow'])) {
            return false;
        }
    ...
    public function check_member($value) {
        ...
        if (!$value) {
            return mys_return_data(0, mys_lang('账号不能为空'));
        } elseif (!\Soulcms\Service::M('member')->uid($value)) {
            if (IS_ADMIN && isset($_POST['no_author']) && $_POST['no_author']) {
                return mys_return_data(1);
            }
            return mys_return_data(0, mys_lang('账号【%s】不存在', $value));
        }
