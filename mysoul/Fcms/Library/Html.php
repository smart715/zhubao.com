...
class Html
{
    ...
    public function get_category_data($app, $cat) {
        ...
        // 获取生成栏目
        !$cat && \Soulcms\Service::C()->_json(0, '没有可用生成的栏目数据');
        ...
        foreach ($cat as $t) {
            if ($t['tid'] == 0) {
                ...
                if ($t['child'] && $t['setting']['template']['list'] != $t['setting']['template']['category']) {
                    ...
                    // 内容列表页面
                    $db = \Soulcms\Service::M()->db->table(SITE_ID.'_'.$t['mid'].'_index');
                    ...
                    if ($total) {
                        // 分页
                        if (\Soulcms\Service::V()->_is_mobile) {
        ...
        $data = mys_save_bfb_data($data);
        !mys_count($data) && \Soulcms\Service::C()->_json(0, '没有可用生成的栏目数据');

        $name = 'category-'.($app ? $app : '').'-html-file';
        \Soulcms\Service::L('cache')->init()->save($name, $data, 3600);
        \Soulcms\Service::C()->_json(1, $name);
    }
    ...
    public function get_show_data($app, $param) {
        ...
        if ($param['catids']) {
            ...
            if ($catids) {
                $cats = \Soulcms\Service::C()->get_cache('module-'.SITE_ID.'-'.($app ? $app : 'share'), 'category');
                foreach ($catids as $id) {
                    if ($cats[$id]) {
                        $cids = mys_array2array($cids, explode(',', $cats[$id]['childids']));
        ...
        if (!$app) {
            if ($cids) {
                if (!$mids) {
                    \Soulcms\Service::C()->_json(0, '没有可用生成的内容模块');
                }
                $data = [];
                foreach ($mids as $mid) {
                    $db = \Soulcms\Service::M()->db->table(SITE_ID.'_'.$mid)->select('id,catid,title,url');
                ...
                \Soulcms\Service::C()->_json(0, '模块参数app不存在');
            }
        } else {
            $db = \Soulcms\Service::M()->db->table(SITE_ID.'_'.$app)->select('id,catid,title,url');
            if (isset($param['date_form']) && $param['date_form']) {
                $db->where('`updatetime` BETWEEN ' . strtotime($param['date_form'].' 00:00:00') . ' AND ' . ($param['date_to'] ? strtotime($param['date_to'].' 23:59:59') : SYS_TIME));
            } elseif (isset($param['date_to']) && $param['date_to']) {
                $db->where('`updatetime` BETWEEN 0 AND ' . strtotime($param['date_to'].' 23:59:59'));
        ...
        $data = mys_save_bfb_data($data);
        !mys_count($data) && \Soulcms\Service::C()->_json(0, '没有可用生成的内容数据');

        \Soulcms\Service::L('cache')->init()->save('show-'.$app.'-html-file', $data, 3600);
        \Soulcms\Service::C()->_json(1, 'ok');
    }
