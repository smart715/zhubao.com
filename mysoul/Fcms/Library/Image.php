...
class Image
{
    ...
    {
        return (mys_count($this->error_msg) > 0) ? $open.implode($close.$open, $this->error_msg).$close : '';
    }

    public function get_file($id) {
        return mys_get_file($id);
    }
    ...
    public function thumb($img, $width = 0, $height = 0, $water = 0) {
        ...
        // 创建缓存目录
        mys_mkdirs($cache_path.dirname($cache_file));

        if (is_numeric($img)) {
            $attach = \Soulcms\Service::C()->get_attachment($img);
        ...
        if (!is_file($file)) {
            if ($attach['remote']) {
                // 远程图片
                $remote = \Soulcms\Service::C()->get_cache('attachment', $attach['remote']);
            ...
            $data = mys_catcher_data($attach['url']);
            if (!$data) {
                return ROOT_THEME_PATH.'assets/images/nopic.gif';
            }
        ...
        // 水印处理
        if ($water) {
            $data = \Soulcms\Service::C()->get_cache('site', SITE_ID, 'watermark');
            if ($data) {
