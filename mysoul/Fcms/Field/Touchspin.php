 ...
 *
 * */
class Touchspin extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('显示模式').'</label>
			<div class="col-md-9">
				<input type="checkbox" name="data[setting][option][show]" '.($option['show'] ? 'checked' : '').' value="1"  data-on-text="'.mys_lang('按钮').'" data-off-text="'.mys_lang('箭头').'" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
				<span class="help-block">'.mys_lang('按钮模式是左右两个加减按钮；箭头模式是左边上下箭头符号').'</span>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('加按钮颜色').'</label>
			<div class="col-md-9">
				<label>'.$this->_color_select('up', $option['up']).'</label>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('减按钮颜色').'</label>
			<div class="col-md-9">
	...
	public function input($field, $value = null) {
		...
		$str.= '<div '.$style.'><input class="form-control '.$field['setting']['option']['css'].'" type="text" name="data['.$field['fieldname'].']" id="mys_'.$field['fieldname'].'" value="'.$value.'" '.$required.' '.$attr.' /></div>';
		$str.= '
<script type="text/javascript">
    $(function(){
        $("#mys_'.$field['fieldname'].'").TouchSpin({
