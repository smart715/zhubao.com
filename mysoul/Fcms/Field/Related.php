...
class Related extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		$_option = '';
		$_module = \Soulcms\Service::C()->get_cache('module-'.SITE_ID.'-content');
		if ($_module) {
		...
		return ['<div class="form-group">
                    <label class="col-md-2 control-label">'.mys_lang('内容模块').'</label>
                    <div class="col-md-9">
                    <label><select class="form-control" name="data[setting][option][module]">
					'.$_option.'
					</select></label>
					<span class="help-block">'.mys_lang('必须选择一个模块作为关联数据源').'</span>
                    </div>
                </div>
				<div class="form-group">
                    <label class="col-md-2 control-label">'.mys_lang('最大显示数量').'</label>
                    <div class="col-md-9">
                    <label><input type="text" class="form-control" size="10" name="data[setting][option][limit]" value="'.$option['limit'].'"></label>
					<span class="help-block">'.mys_lang('关联列表搜索结果最大显示数量，默认50条').'</span>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">'.mys_lang('友情提示').'</label>
                    <div class="col-md-9">
                        <div class="form-control-static">'.mys_lang('此字段不能参与搜索条件筛选').'</div>
                    </div>
                </div>'];
	}
    ...
    public function input($field, $value = '') {
		...
		// 字段提示信息
		$tips = isset($field['setting']['validate']['tips']) && $field['setting']['validate']['tips'] ? '<span class="help-block" id="mys_'.$name.'_tips">'.$field['setting']['validate']['tips'].'</span>' : '';
		// 禁止修改
		...
		//
		$tpl = '<tr id="mys_items_'.$name.'_{id}"><td>{id}</td><td>{value}<input type="hidden" name="data['.$name.'][]" value="{id}"></td><td width="45"><a class="btn btn-xs red" href="javascript:;" onclick="$(\\\'#mys_items_'.$name.'_{id}\\\').remove()"><i class="fa fa-trash"></i></a></td></tr>';
		//
        ...
        if ($value && is_string($value)) {
			$db = \Soulcms\Service::M()->db->query('select id,title,url from '.\Soulcms\Service::M()->dbprefix(SITE_ID.'_'.$module).' where id IN ('.$value.') order by instr("'.$value.'", id)');
		...
		$str.= '</table>';
		$str.= '<p>';
		$str.= '<button type="button" class="btn blue btn-sm" onClick="mys_add_related_'.$name.'()"> <i class="fa fa-plus"></i> '.mys_lang('关联内容').'</button>';
        $str.= '</p>';
        $str.= $tips;
        $str.= '
		<script type="text/javascript">
		function mys_add_related_'.$name.'() {
		
            layer.open({
                type: 2,
                title: \'<i class="fa fa-cog"></i> '.mys_lang('关联内容').'\',
                fix:true,
                shadeClose: true,
                shade: 0,
                area: '.$area.',
                        ...
                        success: function(json) {
                            ...
                            if (json.code == 1) {
                                ...
                                for(var i in json.data.result){
                                    ...
                                    if($("#mys_items_'.$name.'_"+v.id).length>0)
                                    {
                                      mys_tips(0, "'.mys_lang('已经存在').'");
                                      return;
                                    }
