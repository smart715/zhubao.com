...
class Members extends \Soulcms\Library\A_Field {
    ...
    public function input($field, $value = '') {
        ...
        if ($value && is_string($value)) {
			$db = \Soulcms\Service::M()->db->query('select * from '.\Soulcms\Service::M()->dbprefix('member').' where id IN ('.$value.') order by instr("'.$value.'", id)');
            ...
            if ($query) {
                foreach ($query as $t) {
                    ...
                    $str.= str_replace(array('{avatar}', '{id}', '{value}', '\\'), array(mys_avatar($t['id']), $id, $value, ''), $tpl);
                }
            }
		}	
		$str.= '</tbody>';
		$str.= '</table>';
		$str.= '<p>';
		$str.= '<button type="button" class="btn blue btn-sm" onClick="mys_add_rmember_'.$name.'()"> <i class="fa fa-plus"></i> '.mys_lang('关联用户').'</button>';
        $str.= '</p>';
        $str.= $tips;
        $str.= '
		<script type="text/javascript">
		function mys_add_rmember_'.$name.'() {
		
            layer.open({
                type: 2,
                title: \'<i class="fa fa-user"></i> '.mys_lang('关联用户').'\',
                fix:true,
                shadeClose: true,
                shade: 0,
                area: ["70%", "65%"],
