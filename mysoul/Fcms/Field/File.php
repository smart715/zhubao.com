...
class File extends \Soulcms\Library\A_Field {
	...
	public function insert_value($field) {
		...
		// 存在缩略图值时
		if (!$value && $field['fieldname'] == 'thumb' && isset(\Soulcms\Service::L('Field')->data[1]['thumb']) && \Soulcms\Service::L('Field')->data[1]['thumb']) {
			return;
		}

		\Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = $value;
	}
	...
	public function input($field, $value = '') {
		...
		$ts = mys_lang('上传格式要求：%s（%s）', str_replace(',', '、', $field['setting']['option']['ext']), intval($field['setting']['option']['size']).'MB');
		...
		// 显示模板
		$tpl = '<div style="margin-bottom: 10px;" id="mys_'.$name.'_files_row" class="template-download files_row">';
		$tpl.= '<div >';
		$tpl.= '<span class="files_row_preview preview">{preview}</span>';
		$tpl.= '</div>';
		$tpl.= '<input type="hidden" '.$attr.' id="mys_'.$name.'" class="files_row_id" name="data['.$name.']" value="{id}" />';
		$tpl.= '</div>';
		...
		$show_delete = 0;
		if ($value) {
			$file = \Soulcms\Service::C()->get_attachment($value);
			if ($file) {
				...
				$filepath = $value;
				$preview = mys_file_preview_html($value);
				$upload = '';
				$title = '';
			}
			$val.= str_replace(
				['{title}', '{id}', '{filepath}', '{preview}', '{upload}'],
				[$title, $value, mys_strcut($filepath, 30), $preview, $upload],
				$tpl
			);
            $show_delete = 1;
		} else {
			$val.= '<input type="hidden" '.$attr.' id="mys_'.$name.'" name="data['.$name.']" value="" />';
		}
        ...
        $use.= '<button onclick="mys_file_remove_'.$name.'()" style="margin-left: 5px;'.(!$show_delete ? 'display:none' : '').'" type="button" class="btn red btn-sm fileinput-delete">
						<i class="fa fa-trash"></i>
						<span> '.mys_lang('删除').' </span>
					</button>';
...
function mys_file_remove_'.$name.'() {
