...
class Color extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		return ['
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('附加到指定字段').'</label>
				<div class="col-md-9">
					<label><input type="text" class="form-control" size="10" name="data[setting][option][field]" value="'.$option['field'].'"></label>
					<span class="help-block">'.mys_lang('对文本类型字段有效,会实时变动颜色').'</span>
				</div>
			</div>
			<div class="form-group">
	...
	public function input($field, $value = '') {
		...
		$str.= '
		<input type="text" class="form-control color '.$field['setting']['option']['css'].'" data-control="brightness" name="data['.$name.']" id="mys_'.$name.'" '.$style.' value="'.$value.'" >';
		$str.= '
		<script type="text/javascript">
		$(function(){
			$("#mys_'.$name.'").minicolors({
                control: $("#mys_'.$name.'").attr("data-control") || "hue",
                defaultValue: $("#mys_'.$name.'").attr("data-defaultValue") || "",
                inline: "true" === $("#mys_'.$name.'").attr("data-inline"),
                letterCase: $("#mys_'.$name.'").attr("data-letterCase") || "lowercase",
                opacity: $("#mys_'.$name.'").attr("data-opacity"),
                position: $("#mys_'.$name.'").attr("data-position") || "bottom left",
                change: function(t, o) {
                    t && (o && (t += ", " + o), "object" == typeof console && console.log(t));
                    '.($field['setting']['option']['field'] ? '$("#mys_'.$field['setting']['option']['field'].'").css("color", $("#mys_'.$name.'").val());' : '').'
                },
                theme: "bootstrap"
            });
		});
