...
class Files extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		return ['
            <div class="form-group">
                <label class="col-md-2 control-label">'.mys_lang('手动输入').'</label>
                <div class="col-md-9">
                    <input type="checkbox" name="data[setting][option][input]" '.($option['input'] ? 'checked' : '').' value="1"  data-on-text="'.mys_lang('开启').'" data-off-text="'.mys_lang('关闭').'" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
                    <span class="help-block">'.mys_lang('开启将会出现手动输入文件地址按钮').'</span>
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-2 control-label">'.mys_lang('描述文本').'</label>
                <div class="col-md-9">
                    <input type="checkbox" name="data[setting][option][desc]" '.($option['desc'] ? 'checked' : '').' value="1"  data-on-text="'.mys_lang('开启').'" data-off-text="'.mys_lang('关闭').'" data-on-color="success" data-off-color="danger" class="make-switch" data-size="small">
                    <span class="help-block">'.mys_lang('开启将会出现多行文本输入框').'</span>
                </div>
            </div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-md-9">
					...
					<span class="help-block">'.mys_lang('格式：jpg,gif,png,exe,html,php,rar,zip').'</span>
				</div>
	...
	public function insert_value($field) {
		...
		if (isset($_POST['data']['thumb']) && !$_POST['data']['thumb'] && isset($data['file'][0]) && $data['file'][0]) {
            $info = \Soulcms\Service::C()->get_attachment($data['file'][0]);
			in_array($info['fileext'], array('jpg', 'jpeg', 'png', 'gif')) && \Soulcms\Service::L('Field')->data[1]['thumb'] = $data['file'][0];
            unset($info);
		}*/
	...
	public function input($field, $value = '')
    {
        ...
        $area = \Soulcms\Service::C()->_is_mobile() ? '["95%", "90%"]' : '["50%", "45%"]';
        $count = intval($field['setting']['option']['count']);
        $ts = mys_lang('上传格式要求：%s（%s），最多上传%s个文件', str_replace(',', '、', $field['setting']['option']['ext']), intval($field['setting']['option']['size']) . 'MB', $count);
		...
		$tpl.= '<td style="text-align:center;">';
		$tpl.= '<label><button onclick="mys_file_remove(this)" type="button" class="btn red file_delete btn-sm"><i class="fa fa-trash"></i></button></label>';
		$tpl.= '<label><button onclick="mys_file_edit_'.$name.'(this)" type="button" class="fileinput-button btn green file_edit btn-sm"><i class="fa fa-edit"></i>{upload}</button></label>';
		$tpl.= '</td>';
		$tpl.= '</tr>';
		...
		if ($value) {
            ...
            if (isset($value['title']) && $value['title']) {
                foreach ($value['title'] as $i => $title) {
                    ...
                    if ($file) {
                        $disabled = 'readonly';
                        $preview = mys_file_preview_html($file['url']);
                        $filepath = mys_strcut($file['attachment'], 30);
                        $upload = '<input type="file" name="file_data">';
                    } else {
                        $disabled = '';
                        $filepath = $id;
                        $preview = mys_file_preview_html($id);
                        $upload = '';
                        $id = '';
                    }
                    $val.= str_replace(
...
$(function() {
	...
	$(\'#fileupload_'.$name.' .fileinput-unused\' ).click(function(){
		...
		layer.open({
			type: 2,
			title: \'<i class="fa fa-folder-open"></i> '.mys_lang('浏览').'\',
            fix:true,
            scrollbar: false,
            shadeClose: true,
			shade: 0,
					...
					success: function(json) {
						...
						if (json.code == 1) {
							...
							if(numItems + json.data.count > numCount){
								mys_tips(0, \''.mys_lang('上传文件不能超过%s个', $count).'\');
								return false;
							};
							var temp = \''.$tpl.'\';
							var html = "";
	...
	$(\'#fileupload_'.$name.' .fileinput-url\' ).click(function(){
		...
		layer.open({
			type: 2,
			title: \'<i class="fa fa-edit"></i> '.mys_lang('输入文件地址').'\',
            fix:true,
            scrollbar: false,
            shadeClose: true,
			shade: 0,
	...
	$(\'#fileupload_'.$name.'\').fileupload({
		...
		add: function (e, data) {
            ...
            mys_tips(data.result.code, data.result.msg);
...
// 修改URL
function mys_file_edit_'.$name.'(e) {
	...
	var url = "/index.php?s=api&c=file&m=input_file_url&siteid='.SITE_ID.'&fid='.$field['id'].'&p='.$p.'&file="+file+"&name="+name;
		layer.open({
			type: 2,
			title: \'<i class="fa fa-edit"></i> '.mys_lang('修改文件地址').'\',
            fix:true,
            scrollbar: false,
            shadeClose: true,
			shade: 0,
