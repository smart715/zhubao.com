...
class Linkage extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		return ['<div class="form-group">
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('默认选择值').'</label>
				<div class="col-md-9">
	...
	public function insert_value($field) {
		\Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = (int)\Soulcms\Service::L('Field')->post[$field['fieldname']];
	}
	...
	public function input($field, $value = null) {
		...
		// 开始输出
		$str = '<input type="hidden" name="data['.$name.']" id="mys_'.$name.'" value="'.(int)$value.'">';
		...
		// 输出默认菜单
		$str.= '<span id="mys_linkage_'.$name.'_select" style="'.($value ? 'display:none' : '').'">';
		...
		$str.= '</span>';
		// 重新选择
		$value && $str.= '<div class="form-control-static" id="mys_linkage_'.$name.'_cxselect">'.mys_linkagepos($field['setting']['option']['linkage'], $value, ' » ').'&nbsp;&nbsp;<a href="javascript:;" onclick="mys_linkage_select_'.$name.'()" style="color:blue">'.mys_lang('[重新选择]').'</a></div>';
		// 输出js支持
		$str.= '
		<script type="text/javascript">
			function mys_linkage_select_'.$name.'() {
				$("#mys_linkage_'.$name.'_select").show();
				$("#mys_linkage_'.$name.'_cxselect").hide();
			}
			$(function(){
				...
				function onchange(e){
					...
					//$("#'.$name.'-'.$i.'").remove();
					$("#mys_'.$name.'").val($ld5.eq(index).show().val());
					index ++;
					$ld5.eq(index).show();
				}
			})
