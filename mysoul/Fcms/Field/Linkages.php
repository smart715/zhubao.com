...
class Linkages extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		$data = \Soulcms\Service::M()->table('linkage')->getAll();
		if ($data) {
		...
		return ['<div class="form-group">
                  	<label class="col-md-2 control-label">'.mys_lang('选择菜单').'</label>
	...
	public function input($field, $value = null) {
		...
		// 联动菜单缓存
		$linkage = \Soulcms\Service::L('cache')->get('linkage-'.SITE_ID.'-'.$field['setting']['option']['linkage']);
		$linkageid = \Soulcms\Service::L('cache')->get('linkage-'.SITE_ID.'-'.$field['setting']['option']['linkage'].'-id');
		$linkagelevel = (int)\Soulcms\Service::L('cache')->get('linkage-'.SITE_ID.'-'.$field['setting']['option']['linkage'].'-level');
		//
        ...
        // 输出默认菜单
        $tpl = '<div class="linkages_'.$name.'_row" id="mys_linkages_'.$name.'_row_{id}">';
        $tpl.= '<label style="margin-right: 10px;"><a class="btn btn-sm " href="javascript:;" onclick="$(\'#mys_linkages_'.$name.'_row_{id}\').remove()"> <i class="fa fa-close"></i> </a></label>';
        $tpl.= '<input type="hidden" name="data['.$name.'][{id}]" id="mys_'.$name.'_{id}" value="{value}" />';
        $tpl.= '<input type="hidden" id="mys_'.$name.'_{id}_default" value="" />';
        $tpl.= '<span id="mys_linkages_'.$name.'_select_{id}" style="display:{display}">';
        ...
        if ($values) {
            foreach ($values as $id => $value) {
                if ($value) {
                    ...
                    $str.= '<div class="linkages_'.$name.'_row" id="mys_linkages_'.$name.'_row_'.$id.'">';
                    $str.= '<label style="margin-right: 10px;"><a class="btn btn-sm " href="javascript:;" onclick="$(\'#mys_linkages_'.$name.'_row_'.$id.'\').remove()"> <i class="fa fa-close"></i> </a></label>';
                    $str.= '<input type="hidden" name="data['.$name.']['.$id.']" id="mys_'.$name.'_'.$id.'" value="'.$value.'" />';
                    $str.= '<input type="hidden" id="mys_'.$name.'_'.$id.'_default" value="'.$default.'" />';
                    $str.= '<span id="mys_linkages_'.$name.'_select_'.$id.'" style="display:none">';
                    for ($i = 1; $i <= $linklevel; $i++) {
                        $style = $i > $level ? 'style="display:none"' : '';
                    ...
                    $str.= '</span>';
                    $str.= '<div class="form-control-static" id="mys_linkages_'.$name.'_cxselect_'.$id.'">'.mys_linkagepos($field['setting']['option']['linkage'], $value, ' » ').'&nbsp;&nbsp;<a href="javascript:;" onclick="mys_linkages_select_'.$name.'('.$id.')" style="color:blue">'.mys_lang('[重新选择]').'</a></div>';
                    $str.= '</div>';
                }
            }
        }
		...
		// 整体
		$str.= '</div>';
        $str.= '<div class="margin-top-10">	<a href="javascript:;" class="btn blue btn-sm" onClick="mys_add_linkages_'.$name.'()"> <i class="fa fa-plus"></i> '.mys_lang('添加').' </a>';
        $str.= '</div>';
        $str.= '<script type="text/javascript">
		function mys_add_linkages_'.$name.'() {
			...
			mys_linkages_init(id);
		}
		function mys_linkages_select_'.$name.'(id) {
            $("#mys_linkages_'.$name.'_select_"+id).show();
            $("#mys_linkages_'.$name.'_cxselect_"+id).hide();
			mys_linkages_init(id);
        }
        function mys_linkages_init(id) {
          var $ld5 = $(".finecms-selects-'.$name.'-"+id);					  
            ...
            ld5_api.selected($("#mys_'.$name.'_"+id+"_default").val());
            $ld5.bind("change", function(e){
                var $target = $(e.target);
                var index = $ld5.index($target);
                $("#mys_'.$name.'_"+id).val($ld5.eq(index).show().val());
                index ++;
                $ld5.eq(index).show();
            });
    ...
    public function show($field, $value = null) {
        ...
        if ($values) {
            foreach ($values as $value) {
                $str.= '<div class="form-control-static">'.mys_linkagepos($field['setting']['option']['linkage'], $value, ' - ').'</div>';
            }
        }
