 ...
 *
 * */
class Textbtn extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('图标样式').'</label>
			<div class="col-md-9">
				<label><input type="text" class="form-control" size="10" name="data[setting][option][icon]" value="'.$option['icon'].'"></label>
				<span class="help-block">'.mys_lang('例如: fa fa-user').'</span>
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('图标名称').'</label>
			<div class="col-md-9">
		...
		<div class="form-group">
			<label class="col-md-2 control-label">'.mys_lang('点击JS函数').'</label>
			<div class="col-md-9">
				<input type="text" class="form-control Large" size="10" name="data[setting][option][func]" value="'.$option['func'].'">
				<span class="help-block">'.mys_lang('单击按钮时执行的js函数').'</span>
			</div>
		</div>
		';
    ...
    public function insert_value($field) {
		...
		if ($field['setting']['option']['extend_field'] && $field['setting']['option']['extend_function']) {
			...
			$obj = \Soulcms\Service::M($a);
			if (method_exists($obj, $method)) {
                if (IS_ADMIN && isset($_POST['no_author']) && $_POST['no_author']
                    && $field['setting']['option']['extend_function'] == 'member:uid') {
                    // 不验证会员就不变更uid
                } else {
                    \Soulcms\Service::L('Field')->data[$field['ismain']][$field['setting']['option']['extend_field']] = call_user_func([$obj, $method], $value);
                }
	...
	public function input($field, $value = null) {
		...
		// 函数
		$func = $field['setting']['option']['func'] ? $field['setting']['option']['func'] : 'mys_diy_func';

		// 字段默认值
		$value = strlen($value) ? $value : $this->get_default_value($field['setting']['option']['value']);

		$ipt = '<input class="form-control '.$field['setting']['option']['css'].'" type="text" name="data['.$field['fieldname'].']" id="mys_'.$field['fieldname'].'" value="'.$value.'" '.$required.' '.$attr.' />';
		$str = '
		 <div class="input-group" '.$style.'>
				'.$ipt.'
				<span class="input-group-btn">
					<a class="btn btn-success '.$color.'" href="javascript:'.$func.'(\''.$name.'\');" ><i class="'.mys_icon($field['setting']['option']['icon']).'" /></i> '.mys_lang($field['setting']['option']['name']).'</a>
				</span>
			</div>
		';
