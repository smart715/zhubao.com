...
class Date extends \Soulcms\Library\A_Field {
    ...
    public function option($option) {
        ...
        return ['
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('字段格式').'</label>
				<div class="col-md-9">
					<div class="mt-radio-inline">
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][format2]" value="0" '.(!$option['option']['format2'] ? 'checked' : '').'> '.mys_lang('日期时间格式').'
							<span></span>
						</label>
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][format2]" value="1" '.($option['option']['format2'] ? 'checked' : '').'> '.mys_lang('日期格式').'
							<span></span>
						</label>
					</div>
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('图标显示').'</label>
				<div class="col-md-9">
					<div class="mt-radio-inline">
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][is_left]" value="0" '.(!$option['option']['is_left'] ? 'checked' : '').'> '.mys_lang('左侧图标').'
							<span></span>
						</label>
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][is_left]" value="1" '.($option['option']['is_left'] ? 'checked' : '').'> '.mys_lang('右侧图标').'
							<span></span>
						</label>
					</div>
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('按钮颜色').'</label>
				<div class="col-md-9">
    ...
    public function insert_value($field) {
        \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = strtotime(\Soulcms\Service::L('Field')->post[$field['fieldname']]);
    }
    ...
    public function input($field, $value = 0) {
        ...
        $value = $format ? mys_date($value, 'Y-m-d') : mys_date($value, 'Y-m-d H:i:s');
        ...
        APP_DIR && $name == 'updatetime' && $str.= '<label><input name="no_time" type="checkbox" value="1" /> '.mys_lang('不更新').'</label>';
    ...
    public function show($field, $value = null) {
        ...
        $value = (int)$field['setting']['option']['format2'] ? mys_date($value, 'Y-m-d') : mys_date($value, 'Y-m-d H:i:s');
