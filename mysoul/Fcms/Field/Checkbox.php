...
class Checkbox extends \Soulcms\Library\A_Field  {
	...
	public function option($option) {
		...
		return [
			...
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('选项列表').'</label>
				<div class="col-md-9">
					<textarea class="form-control" name="data[setting][option][options]" style="height:150px;width:400px;">'.$option['options'].'</textarea>
					<span class="help-block">'.mys_lang('格式：选项名称|选项值[回车换行]选项名称2|值2....').'</span>
				</div>
			</div>
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('默认选中项').'</label>
				<div class="col-md-9">
					...
					<span class="help-block">'.mys_lang('默认选中项，多个选中项用|分隔').'</span>
				</div>
			</div>'
			,
			'
			<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('显示格式').'</label>
				<div class="col-md-9">
					<div class="mt-radio-inline">
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][show_type]" value="0" '.(!$option['option']['show_type'] ? 'checked' : '').'> '.mys_lang('横排显示').'
							<span></span>
						</label>
						<label class="mt-radio  mt-radio-outline">
							<input type="radio" name="data[setting][option][show_type]" value="1" '.($option['option']['show_type'] ? 'checked' : '').'> '.mys_lang('竖排显示').'
							<span></span>
						</label>
					</div>
	...
	public function insert_value($field) {
		\Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = mys_array2string(\Soulcms\Service::L('Field')->post[$field['fieldname']]);
	}
	...
	public function input($field, $value = '') {
		...
		// 表单选项
		$options = mys_format_option_array($field['setting']['option']['options']);
		if ($options) {
