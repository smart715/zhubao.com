...
class Score extends \Soulcms\Library\A_Field  {
    ...
    public function insert_value($field) {
        ...
        if ((int)$_POST['is_field_'.$field['fieldname']]) {
            ...
            $price = min($sku);
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = (int)$price;
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname'].'_sku'] = mys_array2string($sku);
        } else {
            // 单一
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = (int)$_POST['data'][$field['fieldname']];
    ...
    public function input($field, $value = []) {
        ...
        // 模式
        $sku = mys_string2array(\Soulcms\Service::L('Field')->value[$field['fieldname'].'_sku']);
        $is_field = $sku? 1 : 0;
        $html = '';
        foreach (\Soulcms\Service::C()->member_cache['group'] as $g) {
...
<input type="text" class="form-control" name="data['.$field['fieldname'].'_sku]['.$g['id'].']" value="'.(string)$sku[$g['id']].'" /> 
        ...
        $str = '
            <div class="mt-radio-inline">
                <label class="mt-radio">
                    <input type="radio" onclick="$(\'#mys_field_'.$field['fieldname'].'\').show();$(\'#mys_field_'.$field['fieldname'].'2\').hide();" name="is_field_'.$field['fieldname'].'" value="0" '.(!$is_field ? 'checked' : '').'> 全局
                    <span></span>
                </label>
                <label class="mt-radio">
                    <input type="radio" onclick="$(\'#mys_field_'.$field['fieldname'].'2\').show();$(\'#mys_field_'.$field['fieldname'].'\').hide();" name="is_field_'.$field['fieldname'].'" value="1" '.($is_field ? 'checked' : '').'> 用户组
                    <span></span>
                </label>
            </div>
            <div id="mys_field_'.$field['fieldname'].'" style="display:'.(!$is_field ? 'block' : 'none').';">
                <label><input class="form-control " type="text" name="data['.$field['fieldname'].']" id="mys_'.$field['fieldname'].'" value="'.$value.'" /></label>
            </div>
            <div id="mys_field_'.$field['fieldname'].'2" style="display:'.($is_field ? 'block' : 'none').';">
                <div style="width: 50%">
                '.$html.'
                </div>
            </div>
    ...
    public function show($field, $value = null) {
        ...
        // 模式
        $sku = mys_string2array(\Soulcms\Service::L('Field')->value['score_sku']);
        $is_field = $sku? 1 : 0;
        $html = '';
        foreach (\Soulcms\Service::C()->member_cache['group'] as $g) {
            $html.= '<label><div class="input-group">
...
<input type="text" class="form-control"  readonly name="data['.$field['fieldname'].'_sku]['.$g['id'].']" value="'.(string)$sku[$g['id']].'" /> 
        ...
        $str = '
            <div id="mys_field_'.$field['fieldname'].'" style="display:'.(!$is_field ? 'block' : 'none').';">
                <label><input class="form-control " readonly type="text" name="data['.$field['fieldname'].']" id="mys_'.$field['fieldname'].'" value="'.$value.'" /></label>
            </div>
            <div id="mys_field_'.$field['fieldname'].'2" style="display:'.($is_field ? 'block' : 'none').';">
                <div style="width: 50%">
                '.$html.'
