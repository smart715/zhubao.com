...
class Baidumap extends \Soulcms\Library\A_Field {
    ...
    public function test_sql($tables, $field) {
        ...
        foreach ($tables as $table) {
            if (!\Soulcms\Service::M()->db->fieldExists($field.'_lat', $table)) {
    ...
    public function insert_value($field) {
        ...
        if (\Soulcms\Service::L('Field')->post[$field['fieldname']]) {
            $map = explode(',', \Soulcms\Service::L('Field')->post[$field['fieldname']]);
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname'].'_lng'] = (double)$map[0];
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname'].'_lat'] = (double)$map[1];
        } else {
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname'].'_lng'] = 0;
            \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname'].'_lat'] = 0;
        }
    ...
    public function show($field, $value = null) {
        return $this->input_format($field['fieldname'], $field['name'], mys_baidu_map(
            $value,
            (int)$field['setting']['option']['level'],
            \Soulcms\Service::_is_mobile() ? '100%' : ($field['setting']['option']['width'] ? $field['setting']['option']['width'] : 400),
    ...
    public function input($field, $value = null) {
        ...
        // 表单宽度设置
        $width = \Soulcms\Service::_is_mobile() ? '100%' : ($field['setting']['option']['width'] ? $field['setting']['option']['width'] : 400);
        ...
        $city = \Soulcms\Service::L('ip')->address(\Soulcms\Service::L('input')->ip_address());
        ...
        $str.= '
		<input name="data['.$name.']" id="mys_'.$name.'" type="hidden" '.$attr.' '.$required.' value="'.$value.'">
		
		<div style="width:'.$width.(is_numeric($width) ? 'px' : '').';height:50px">
			<div class="">
				<div class="pull-left" style="width:85%;padding-right:10px">
					<div class="input-group">
					<input type="text" class="form-control" id="baidu_address_'.$name.'" placeholder="'.mys_lang('输入地址，需要精确到街道号').'...">
					<span class="input-group-btn">
						<a title="'.mys_lang('输入地址，需要精确到街道号').'" class="btn blue" href="javascript:baiduSearchAddress(mapObj_'.$name.', \''.$name.'\');">
				...
				<div class="pull-left">
				<label>
					<a title="'.mys_lang('添加标注').'" href="javascript:addMarker(mapObj_'.$name.', \''.$name.'\');" class="btn btn-icon-only red">
		...
		$(function(){
			mys_baidumap(mapObj_'.$name.', \''.$name.'\', \''.$city.'\', \''.$level.'\')
		});
		</script>
		';
