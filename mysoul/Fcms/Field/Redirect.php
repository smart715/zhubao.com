...
class Redirect extends \Soulcms\Library\A_Field {
	...
	public function option($option) {
		...
		return ['<div class="form-group">
				<label class="col-md-2 control-label">'.mys_lang('适应范围').'</label>
				<div class="col-md-9">
					<p class="form-control-static">'.mys_lang('此字段只能用于模块内容自定义字段').'</p>
				</div>
			</div>', '
    ...
    public function input($field, $value = null) {
		...
		$str = '<input class="form-control" type="text" name="data['.$name.']" id="mys_'.$name.'" value="'.$value.'" '.$style.' '.$attr.' />'.$tips;
