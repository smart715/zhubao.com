...
class Pay extends \Soulcms\Library\A_Field  {
    ...
    public function insert_value($field) {
        \Soulcms\Service::L('Field')->data[$field['ismain']][$field['fieldname']] = (float)\Soulcms\Service::L('Field')->post[$field['fieldname']];
    }
    ...
    public function input($field, $value = '', $html = []) {
        ...
        if (!defined('FC_PAY') && (IS_MEMBER || IS_ADMIN)) {
            ...
            // 表单宽度设置
            $width = \Soulcms\Service::_is_mobile() ? '100%' : ((int)$field['setting']['option']['width'] ? $field['setting']['option']['width'] : 250);
    ...
    public function show($field, $value = null) {
        ...
        $url = 'mys_iframe_show(\'cart\', \'/index.php?s=cart&c=show&m=index&fid='.$field['id'].'&id='.$this->id.'\')';
