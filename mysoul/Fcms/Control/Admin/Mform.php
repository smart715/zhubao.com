...
class Mform extends \Soulcms\Table
{
    ...
    public function __construct(...$params) {
        ...
        // 判断表单是否操作
        $this->form = $this->module['form'][str_replace('_verify', '',\Soulcms\Service::L('Router')->class)];
        !$this->form && $this->_admin_msg(0, mys_lang('模块表单【%s】不存在', str_replace('_verify', '',\Soulcms\Service::L('Router')->class)));
        // 支持附表存储
        $this->is_data = 1;
        // 模板前缀(避免混淆)
        ...
        ]) : \Soulcms\Service::M('auth')->_module_menu(
            $this->module,
            ' <i class="'.mys_icon($this->form['setting']['icon']).'"></i>  '.mys_lang('%s管理', $this->form['name']),
           \Soulcms\Service::L('Router')->url(APP_DIR.'/'.$this->form['table'].'/index', ['cid' =>  $this->cid]),
             $this->cid && $this->is_add_menu ? \Soulcms\Service::L('Router')->url(APP_DIR.'/'.$this->form['table'].'/add', ['cid' =>  $this->cid]) : ''
        );
        ...
        \Soulcms\Service::V()->assign([
            ...
            'form_url' =>\Soulcms\Service::L('Router')->url(APP_DIR.'/'.$this->form['table'].'/index', ['cid' =>  $this->cid]),
    ...
    protected function _Admin_Add() {
        ...
        ! $this->cid && $this->_admin_msg(0, mys_lang('缺少cid参数'));
    ...
    protected function _Admin_Edit() {
        ...
        !$data && $this->_admin_msg(0, mys_lang('数据不存在: '.$id));
         $this->cid != $data['cid'] && $this->_admin_msg(0, mys_lang('cid不匹配'));
    ...
    protected function _Save($id = 0, $data = [], $old = [], $func = null, $func2 = null) {
        ...
        return parent::_Save($id, $data, $old, null,
            function ($id, $data, $old) {
                ...
                \Soulcms\Service::M('member')->todo_admin_notice(MOD_DIR.'/'.$this->form['table'].'_verify/edit:cid/'.$old['cid'].'/id/'.$old['id'], SITE_ID);
                // clear
                \Soulcms\Service::L('cache')->clear('module_'.MOD_DIR.'_from_'.$this->form['table'].'_show_id_'.$id);
            }
        );
    }
    ...
    protected function _verify($row) {
        ...
        // 增减金币
        $score = $this->_member_value(\Soulcms\Service::M('member')->authid($row['uid']), $this->member_cache['auth_module'][SITE_ID][MOD_DIR]['form'][$this->form['table']]['score']);
        $score && \Soulcms\Service::M('member')->add_score($row['uid'], $score, mys_lang('%s: %s发布', MODULE_NAME, $this->form['name']), $row['curl']);

        // 增减经验
        $exp = $this->_member_value(\Soulcms\Service::M('member')->authid($row['uid']), $this->member_cache['auth_module'][SITE_ID][MOD_DIR]['form'][$this->form['table']]['exp']);
        $exp && \Soulcms\Service::M('member')->add_experience($row['uid'], $exp, mys_lang('%s: %s发布', MODULE_NAME, $this->form['name']), $row['curl']);

        \Soulcms\Service::M()->db->table($this->init['table'])->where('id', $row['id'])->update(['status' => 1]);

        \Soulcms\Service::L('Notice')->send_notice('module_form_verify_1', $row);
