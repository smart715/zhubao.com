...
// 评论操作类 基于 Ftable
class Comment extends \Soulcms\Table
{
    public $cid; // 内容id
    public $module; // 模块信息
    ...
    public function __construct(...$params) {
        ...
        !$this->module['comment'] && $this->_admin_msg(0, mys_lang('模块【%s】没有启用评论', APP_DIR));
        // 支持附表存储
        $this->is_data = 0;
        // 模板前缀(避免混淆)
        ...
        // 模块显示名称
        $this->name = mys_lang('内容模块[%s]评论', APP_DIR);
        // 获取父级内容
        ...
        $this->module['comment']['field'] = mys_array22array(
            [
                'content' => [
                    'name' => mys_lang('内容'),
                    'ismain' => 1,
        ...
        // 初始化数据表
        $this->_init([
            'table' => mys_module_table_prefix(APP_DIR).'_comment',
        ...
        $menu = $this->is_verify ? \Soulcms\Service::M('auth')->_admin_menu([
                ...
                $this->module,
                ' <i class="fa fa-comments"></i>  '.mys_lang('评论管理'),
               \Soulcms\Service::L('Router')->url(APP_DIR.'/comment/index', ['cid' =>  $this->cid]),
                 $this->cid ?\Soulcms\Service::L('Router')->url(APP_DIR.'/comment/add', ['cid' =>  $this->cid]) : ''
            ).($this->module['comment']['review'] &&  $this->cid ? '<li> <a href="javascript:mys_iframe_show(\'show\', \''.\Soulcms\Service::L('Router')->url(APP_DIR.'/comment/review_index', ['cid' =>  $this->cid]).'\', \'30%\', \'50%\');"> <i class="fa fa-thumbs-o-up"></i> '.mys_lang('查看评分').'</a> <i class="fa fa-circle"></i> </li>' : '');
        ...
        \Soulcms\Service::V()->assign([
            ...
            'comment_url' =>\Soulcms\Service::L('Router')->url(APP_DIR.'/comment/index', ['cid' =>  $this->cid]),
        ]);
    }
    ...
    protected function _Admin_Review() {
        ...
        \Soulcms\Service::V()->display('share_comment_review.html');exit;
    }
    ...
    protected function _Admin_Status() {
        ...
        $rows = \Soulcms\Service::M()->init($this->init)->where_in('id', $in)->getAll();
    ...
    protected function _Admin_Del() {
        $this->_Del(
            ...
            function ($rows) {
                // 对应删除提醒
                foreach ($rows as $t) {
                    \Soulcms\Service::M('member')->delete_admin_notice(MOD_DIR.'/comment_verify/edit:cid/'.$t['cid'].'/id/'.$t['id'], SITE_ID);
                    // 重新统计评论数
    ...
    protected function _Format_Data($id, $data, $old) {
        ...
        $review = \Soulcms\Service::L('input')->post('review');
        if ($review) {
