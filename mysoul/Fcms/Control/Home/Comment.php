...
// 用于前端模块评论显示
class Comment extends \Soulcms\Common
{
    protected $cid;
    protected $index;
    ...
    public function __construct(...$params) {
        ...
        if (!$this->module['comment']) {
            !\Soulcms\Service::L('input')->get('callback') && $this->_msg(0, mys_lang('模块【%s】没有启用评论', MOD_DIR));
            exit('未开启评论'); // jsonp请求时不输出
        }
        // 关联内容数据
        $this->cid = intval(\Soulcms\Service::L('input')->get('id'));
        $this->index = \Soulcms\Service::L('cache')->init()->get('module_'.MOD_DIR.'_show_id_'.$this->cid);
        if (!$this->index) {
            $this->index = $this->content_model->get_data($this->cid);
            !$this->index && $this->_msg(0, mys_lang('内容【id#%s】不存在',  $this->cid));
            // 格式化输出自定义字段
            ...
            $this->index = \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($fields, $this->index);
        }
        $this->index['url'] = mys_url_prefix($this->index['url'], MOD_DIR);
    ...
    protected function _Index() {
        ...
        // 排序模式
        $type = (int)str_replace('#', '', \Soulcms\Service::L('input')->get('type'));
        ...
        switch ($type) {
            ...
            case 4:
                ...
                break;
            default:
                $_GET['order'] && $order = strtolower(mys_get_order_string($_GET['order'], $order));
                break;
        }

        // 判断排序字段是否可用
        !in_array(trim(str_replace([' asc', ' desc'], '', $order)), \Soulcms\Service::L('cache')->get('table-'.SITE_ID, \Soulcms\Service::M()->dbprefix($this->content_model->mytable.'_comment'))) && $order = 'inputtime desc';

        // 获取评论数据
        $comment = $this->content_model->get_comment_index( $this->cid, $this->index['catid']);
        !$comment && exit($this->_msg(0, mys_lang('内容【id#%s】评论索引数据读取失败',  $this->cid)));
        ...
        // ajax动态无刷新调用
        $js = 'mys_ajax_module_comment_'. $this->cid;
        $myfield = \Soulcms\Service::L('Field')->toform(0, $this->module['comment']['field']);

        \Soulcms\Service::V()->assign($this->index);
        \Soulcms\Service::V()->assign(\Soulcms\Service::L('Seo')->comment($this->module, $this->index));
        ...
        if (empty($_GET['callback'])) {
            \Soulcms\Service::V()->display('comment.html');
        } else {
            ob_start();
            \Soulcms\Service::V()->display('comment_ajax.html');
    ...
    protected function _Post() {
        ...
        $rid = (int)\Soulcms\Service::L('input')->get('rid');
        !IS_POST && $this->_json(0, mys_lang('非法请求'));

        // 挂钩点 评论完成之后
        \Soulcms\Hooks::trigger('comment_before', $this->index);

        if ($this->module['comment']['my'] && $this->index['uid'] == $this->uid) {
            // 判断不能对自己评论
            $this->_json(0, mys_lang('系统禁止对自己评论'));
        } elseif (!mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][MOD_DIR]['comment']['add'])) {
            // 判断用户评论权限
            $this->_json(0, mys_lang('您的用户组无权限评论'));
        } elseif ($this->index['is_comment'] == 1) {
            // 判断内容设置的评论权限
            $this->_json(0, mys_lang('该主题禁止评论'));
        } elseif ($this->module['comment']['buy']) {
            // 购买之后才能评论
            $this->_json(0, mys_lang('请到我的订单中评论该商品'));
        } elseif ($this->module['comment']['num'] && \Soulcms\Service::M()->db->table($this->content_model->mytable.'_comment')->where('cid',  $this->cid)->where('uid', $this->uid)->countAllResults()) {
            // 只允许评论一次
            $this->_json(0, mys_lang('您已经评论过了，请勿再次评论'));
        }

        if ($rid) {
            // 判断是否回复权限
            // 禁止回复
            !$this->module['comment']['reply'] && $this->_json(0, mys_lang('系统禁止回复功能'));
            // 查询主题
            $row = $this->content_model->table($this->content_model->mytable.'_comment')->get($rid);
            (!$row ||  $this->cid != $row['cid']) && $this->_json(0, mys_lang('您回复的评论主体不存在'));
            if ($row['reply']) {
                $rid = $row['reply']; // 如果他本是就是回帖内容
            }
            // 判断仅自己
            $this->module['comment']['reply'] == 2 && !(($this->member['uid'] == $row['uid'] && $row['uid'] == $this->index['uid']) || $this->member['is_admin']) && $this->_json(0, mys_lang('您无权限回复'));
        }
        ...
        // 验证操作间隔
        $name = 'module-comment-post-'.md5(mys_now_url().$this->uid);
        $this->session()->get($name) && $this->_json(0, mys_lang('您动作太快了'));

        // 获取评论数据
        $comment = $this->content_model->get_comment_index( $this->cid, $this->index['catid']);
        !$comment && $this->_json(0, mys_lang('内容【id#%s】评论索引数据读取失败',  $this->cid));

        // 判断评论内容
        $content = $this->_safe_replace(\Soulcms\Service::L('input')->post('content', true));
        !$content && $this->_json(0, mys_lang('评论内容不能为空'));
        ...
        if (!$rid && $this->module['comment']['review'] && $this->module['comment']['review']['option']) {
            foreach ($this->module['comment']['review']['option'] as $i => $name) {
                ...
                !$review[$i] && $this->_json(0, mys_lang('选项[%s]未评分', $name));
            }
        }

        // 自定义字段
        if (!$rid && $this->module['comment']['field']) {
            \Soulcms\Service::L('Field')->app(MOD_DIR);
        ...
        // 评论状态
        $status = mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][MOD_DIR]['comment']['verify']) ? 0 : 1;
        ...
        // 附件归档
        !$rid && SYS_ATTACHMENT_DB && $attach && \Soulcms\Service::M('Attachment')->handle(
            $this->member['id'],
            \Soulcms\Service::M()->dbprefix($this->content_model->mytable.'_comment').'-'.$rt['code'],
            $attach
        );
        ...
        $status ? $this->_json(1, mys_lang('评论成功')) : $this->_json(1, mys_lang('评论成功，等待管理员审核'));
    }
    
    // 操作动作
    protected function _Op() {

        $op = \Soulcms\Service::L('input')->get('t');
        $id = (int)\Soulcms\Service::L('input')->get('rid');

        // 查询评论是否存在
        $data = $this->content_model->table($this->content_model->mytable.'_comment')->get($id);
        !$data && $this->_json(0, mys_lang('评论主题不存在'));

        // 获取评论索引数据
        $comment = $this->content_model->get_comment_index( $this->cid, $this->index['catid']);
        !$comment && $this->_msg(0, mys_lang('内容【id#%s】评论索引数据读取失败',  $this->cid));

        // 验证操作间隔
        $name = 'module-comment-op-'.md5(mys_now_url().$op.$id.$this->uid);
        $this->session()->get($name) && $this->_json(0, mys_lang('您动作太快了'));
        ...
        switch ($op) {
            ...
            case 'delete':
                ...
                // 删除
                \Soulcms\Service::M()->table($this->content_model->mytable.'_comment')->delete($data['id']);
                \Soulcms\Service::M('member')->delete_admin_notice(MOD_DIR.'/comment_verify/edit:cid/'.$data['cid'].'/id/'.$data['id'], SITE_ID);
                // 重新统计评论数
    ...
    protected function _get_pages($url, $total, $pagesize) {
        ...
        $file = 'config/page/'.(\Soulcms\Service::IS_PC() ? 'pc' : 'mobile').'/ajax.php';
