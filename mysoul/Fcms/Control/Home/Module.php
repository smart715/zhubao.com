...
class Module extends \Soulcms\Common
{
    ...
    protected function _Index($html = 0) {
        ...
        // 共享模块时禁止访问首页
        IS_SHARE && exit($this->goto_404_page(mys_lang('共享模块没有首页功能')));

        // 判断URL重复问题
        !$html && \Soulcms\Service::L('Router')->is_redirect_url(mys_url_prefix(MODULE_URL, $this->module['dirname']));
        ...
        \Soulcms\Service::V()->assign($this->content_model->_format_home_seo($this->module));
        ...
        if (!defined('SC_HTML_FILE') && $this->module['setting']['module_index_html']) {
            ...
            if (\Soulcms\Service::IS_PC()) {
                // 电脑端访问
                file_put_contents(\Soulcms\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], $file), $html);
                // 生成移动端
                if (SITE_IS_MOBILE_HTML) {
                    ob_start();
                    \Soulcms\Service::V()->init('mobile');
                    \Soulcms\Service::V()->assign([
                        'fix_html_now_url' => defined('SC_HTML_FILE') ? mys_url_prefix(MODULE_URL, $this->module['dirname'], SITE_ID, 1) : '', // 修复静态下的当前url变量
                    ]);
                    \Soulcms\Service::V()->display('index.html');
                    file_put_contents(\Soulcms\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], 'mobile/'.$file), ob_get_clean());
                }
            } else {
                // 移动端访问
                if (SITE_IS_MOBILE_HTML) {
                    file_put_contents(\Soulcms\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], 'mobile/'.$file), $html);
                }
                // 生成电脑端
                ob_start();
                \Soulcms\Service::V()->init('pc');
                \Soulcms\Service::V()->assign([
                    'fix_html_now_url' => defined('SC_HTML_FILE') ? mys_url_prefix(MODULE_URL, $this->module['dirname'], SITE_ID, 0) : '', // 修复静态下的当前url变量
                ]);
                \Soulcms\Service::V()->display('index.html');
                file_put_contents(\Soulcms\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], $file), ob_get_clean());
            }
    ...
    protected function _Donation($id = 0, $rt = 0) {
        ...
        if (!mys_is_app_dir('shang')) {
            $this->goto_404_page('当前模块没有安装打赏应用');
        }

        !$id && $id = intval(\Soulcms\Service::L('input')->get('id'));
        ...
        // 验证
        if (!in_array('donation', \Soulcms\Service::M('table')->get_cache_field(SITE_ID.'_'.MOD_DIR)) ) {
            $this->_msg(0, '当前模块没有安装打赏应用');exit;
        }

        \Soulcms\Service::V()->assign('meta_title', mys_lang('打赏作者').SITE_SEOJOIN.\Soulcms\Service::V()->get_value('meta_title'));
        \Soulcms\Service::V()->display('donation.html');
    }
    ...
    protected function _Category($catid = 0, $catdir = null, $page = 1) {
        ...
        if ($catid) {
            ...
            if (!$category) {
                $this->goto_404_page(mys_lang('模块【%s】栏目（%s）不存在', $this->module['dirname'], $catid));
                return;
            }
        } elseif ($catdir) {
            ...
            if (!$category) {
                ...
                foreach ($this->module['category'] as $t) {
                    if ($t['setting']['urlrule']) {
                        $rule = \Soulcms\Service::L('cache')->get('urlrule', $t['setting']['urlrule']);
                        $rule['value']['catjoin'] = '/';
                ...
                // 返回无法找到栏目
                if (!$category) {
                    $this->goto_404_page(mys_lang('模块【%s】栏目（%s）不存在', $this->module['dirname'], $catdir));
                    return;
                }
            }
        } else {
            $this->goto_404_page(mys_lang('模块【%s】栏目不存在', $this->module['dirname']));
            return;
        }

        // 判断是否外链
        if ($category['tid'] == 2) {
            mys_redirect(mys_url_prefix($category['url'], $this->module['dirname']), 'refresh');exit;
        }
        ...
        if ((IS_SHARE) && $category['tid'] == 0) {
            // 识别栏目单网页
            if (!mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID]['share']['category'][$catid]['show'])) {
                $this->_msg(0, mys_lang('您的用户组无权限访问栏目'));
                return;
            }
        } else {
            if (!mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][$this->module['dirname']]['category'][$catid]['show'])) {
                $this->_msg(0, mys_lang('您的用户组无权限访问栏目'));
                return;
            }
        }
        ...
        // 判断内容唯一性
        \Soulcms\Service::L('Router')->is_redirect_url(mys_url_prefix($category['url'], $this->module['dirname']));

        // 获取同级栏目及父级栏目
        list($parent, $related) = mys_related_cat(
            !IS_SHARE ? $this->module['category'] : \Soulcms\Service::L('cache')->get('module-'.SITE_ID.'-share', 'category'),
            $catid
        );
        ...
        $category = \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($this->module['category_field'], $category, $page);

        \Soulcms\Service::V()->assign($this->content_model->_format_category_seo($this->module, $catid, $page));
        \Soulcms\Service::V()->assign(array(
            ...
            'parent' => $parent,
            'related' => $related,
            'urlrule' => \Soulcms\Service::L('Router')->category_url($this->module, $category, '[page]'),
            'fix_html_now_url' => defined('SC_HTML_FILE') ? mys_url_prefix(\Soulcms\Service::L('Router')->category_url($this->module, $category, $page), $this->module['dirname'], SITE_ID, \Soulcms\Service::V()->_is_mobile == 'mobile') : '', // 修复静态下的当前url变量
        ));

        // 识别栏目单网页模板
        if ((IS_SHARE || (isset($this->module['config']['scategory']) && $this->module['config']['scategory'])) && $category['tid'] == 0) {
            \Soulcms\Service::V()->assign($category);
            \Soulcms\Service::V()->assign(array(
                'pageid' => $catid,
            ));
            $tpl = !$category['setting']['template']['page'] ? 'page.html' : $category['setting']['template']['page'];
        } else {
            \Soulcms\Service::V()->module($this->module['dirname']);
    ...
    protected function _Search($_catid = 0) {
        ...
        // 模型类
        $search = \Soulcms\Service::M('Search', $this->module['dirname'])->init($this->module['dirname']);
        ...
        if (!IS_API_HTTP) {
            if (!isset($this->module['setting']['search']['use']) || !$this->module['setting']['search']['use']) {
                exit($this->_msg(0, mys_lang('此模块已经关闭了搜索功能')));
            } elseif (!mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][$this->module['dirname']]['home'])) {
                exit($this->_msg(0, mys_lang('您的用户组无权限搜索')));
            } elseif ($get['keyword'] && strlen($get['keyword']) < (int)$this->module['setting']['search']['length']) {
                exit($this->_msg(0, mys_lang('关键字不得少于系统规定的长度')));
            } elseif (strlen($get['keyword']) > 100) {
                exit($this->_msg(0, mys_lang('关键字太长了')));
            }
        }
        ...
        if (IS_API_HTTP && $data['id']) {
            // 移动端请求时
            $pagesize = intval(\Soulcms\Service::L('input')->request('pagesize'));
            ...
            $rt = \Soulcms\Service::V()->list_tag($tag);
            $list = $rt['return'];
        }

        \Soulcms\Service::V()->assign($this->content_model->_format_search_seo($this->module, $catid, $data['params'], $get['page']));
        \Soulcms\Service::V()->assign(array(
            'cat' => $catid && $this->module['category'][$catid] ? \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($this->module['category_field'], $this->module['category'][$catid], 1) : [],
            ...
            'related' => $related,
            'urlrule' => \Soulcms\Service::L('Router')->search_url($data['params'], 'page', '{page}'),
            'sototal' => $sototal,
            'searchid' => $data['id'],
            'search_id' => $data['id'],
            'search_sql' => $data['sql'],
        ...
        // 挂钩点 搜索完成之后
        \Soulcms\Hooks::trigger('module_search_data', $data);

        if (isset($_GET['ajax_page']) && $_GET['ajax_page']) {
            $tpl = mys_safe_filename($_GET['ajax_page']);
        } else {
    ...
    protected function _Show($id = 0, $param = [], $page = 1, $rt = 0) {
        ...
        if (!$data) {
            ...
            if (!$data) {
                $this->goto_404_page(mys_lang('%s内容(#%s)不存在', $this->module['name'], $id));
                return;
            }
            ...
            foreach ($this->module['field'] as $t) {
                if ($t['fieldtype'] == 'Redirect' && $data[$t['fieldname']]) {
                    // 存在转向字段时的情况
                    \Soulcms\Service::M()->db->table(SITE_ID.'_'.$this->module['dirname'])->where('id', $id)->set('hits', 'hits+1', FALSE)->update();
                    \Soulcms\Service::V()->assign('goto_url', $data[$t['fieldname']]);
                    \Soulcms\Service::V()->display('goto_url');
                    return;
                }
            }
            ...
            // 上一篇文章
            $builder = \Soulcms\Service::M()->db->table($this->content_model->mytable);
            ...
            // 格式化字段
            $data = \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($fields, $data, $page);
            ...
            if ($data['uid'] != $this->uid && SYS_CACHE) {
                if ($this->member && $this->member['is_admin']) {
                    ...
                    if (!$is_id) {
                        // 表示自定义查询，再缓存一次ID
                        \Soulcms\Service::L('cache')->init()->save(str_replace($id, $data['id'], $name), $data, SYS_CACHE_SHOW * 3600);
                    }
                }
            }
        }
        ...
        // 挂钩点 内容读取之后
        \Soulcms\Hooks::trigger('module_show_read_data', $data);

        // 状态判断
        if ($data['status'] == 10 && !($this->uid == $data['uid'] || $this->member['is_admin'])) {
            $this->goto_404_page(mys_lang('内容被删除，暂时无法访问'));
            return;
        }
        ...
        if ($this->is_hcategory) {
            ...
            if ($data['link_id'] && $data['link_id'] > 0) {
                \Soulcms\Service::V()->assign('gotu_url', mys_url_prefix($data['url'], $this->module['dirname']));
                \Soulcms\Service::V()->display('go.html', 'admin');
                return;
            }
            // 获取同级栏目及父级栏目
            list($parent, $related) = mys_related_cat(
        ...
        if ($page && $data['content_page'] && !$data['content_page'][$page]) {
            $this->goto_404_page(mys_lang('该分页不存在'));
            return;
        }

        // 判断内容唯一性
        !$rt && \Soulcms\Service::L('Router')->is_redirect_url(mys_url_prefix($data['url'], $this->module['dirname']));

        \Soulcms\Service::V()->assign($data);
        \Soulcms\Service::V()->assign($this->content_model->_format_show_seo($this->module, $data, $page));
        \Soulcms\Service::V()->assign([
            'cat' => \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($this->module['category_field'], $this->module['category'][$catid], 1),
            'page' => $page,
            'params' => ['catid' => $catid],
            'parent' => $parent,
            'markid' => 'module-'.$this->module['dirname'].'-'.$catid,
            'related' => $related,
            'urlrule' => \Soulcms\Service::L('Router')->show_url($this->module, $data, '[page]'),
            'fix_html_now_url' => defined('SC_HTML_FILE') ? mys_url_prefix(\Soulcms\Service::L('Router')->show_url($this->module, $data, $page), $this->module['dirname'], SITE_ID, \Soulcms\Service::V()->_is_mobile == 'mobile') : '', // 修复静态下的当前url变量
        ]);
        \Soulcms\Service::V()->module($this->module['dirname']);
        !$rt && (\Soulcms\Service::V()->display(isset($data['template']) && strpos($data['template'], '.html') !== FALSE && is_file(\Soulcms\Service::V()->get_dir().$data['template']) ? $data['template'] : ($this->module['category'][$data['catid']]['setting']['template']['show'] ? $this->module['category'][$data['catid']]['setting']['template']['show'] : 'show.html')));
        return $data;
    }
    ...
    protected function _MyShow($type, $id = 0, $page = 1) {
        ...
        switch($type) {
            ...
            case 'time':
                $row = \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_time')->get($id);
                $data = mys_string2array($row['content']);
                if (!$data) {
                    $this->_msg(0, mys_lang('定时内容#%s不存在', $id));
                } elseif (($this->uid != $data['uid'] && !$this->member['is_admin'])) {
                    $this->_msg(0, mys_lang('定时内容只能自己访问'));
                }
                break;

            case 'recycle':
                $row = \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_recycle')->get($id);
                $row = mys_string2array($row['content']);
                if (!$row) {
                    $this->_msg(0, mys_lang('回收站内容#%s不存在', $id));
                } elseif (!$row[SITE_ID.'_'.$this->module['dirname']]) {
                    $this->_msg(0, mys_lang('回收站内容#%s格式不规范', $id));
                } elseif (!$this->member['is_admin']) {
                    $this->_msg(0, mys_lang('无权限访问回收站的内容'));
                }
            ...
            case 'verify':
                ...
                $data = mys_string2array($row['content']);
                if (!$data) {
                    $this->_msg(0, mys_lang('审核内容#%s不存在', $id));
                } elseif (($this->uid != $data['uid'] && !$this->member['is_admin'])) {
                    $this->_msg(0, mys_lang('无权限访问审核中的内容'));
                }
                break;

            case 'draft':
                $row = \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_draft')->get($id);
                $data = mys_string2array($row['content']);
                if (!$data) {
                    $this->_msg(0, mys_lang('草稿内容#%s不存在', $id));
                } elseif (($this->uid != $data['uid'] && !$this->member['is_admin'])) {
                    $this->_msg(0, mys_lang('无权限访问草稿箱内容'));
                }
                break;

            default:
                $this->_msg(0, mys_lang('未定义的操作'));exit;
        }
        ...
        \Soulcms\Service::V()->assign($data);
        \Soulcms\Service::V()->assign(\Soulcms\Service::L('Seo')->show($this->module, $data, $page));
        \Soulcms\Service::V()->assign([
            'cat' => \Soulcms\Service::L('Field')->app($this->module['dirname'])->format_value($this->module['category_field'], $this->module['category'][$data['catid']], 1),
            'page' => $page,
        ...
        \Soulcms\Service::V()->module($this->module['dirname']);
        \Soulcms\Service::V()->display(is_file(mys_tpl_path().'show_'.$type.'.html') ? 'show_'.$type.'.html' : 'show.html');
        return $data;
    }
    ...
    protected function _Create_Category_Html($catid, $page = 0) {
        ...
        if (!$catid) {
            return mys_return_data(0, '栏目id不存在');
        } elseif (!defined('MODULE_NAME')) {
            return mys_return_data(0, 'MODULE_NAME未定义');
        }

        $cat = $this->module['category'][$catid];
        if (!$cat) {
            return mys_return_data(0, '栏目#'.$catid.'不存在');
        } elseif ($this->module['setting']['search']['catsync'] && $cat['tid'] == 1) {
            return mys_return_data(0, '此模块开启了搜索集成栏目页，因此栏目无法生成静态');
        }

        $url = $page > 0 ?\Soulcms\Service::L('Router')->category_url($this->module, $cat, $page) : $cat['url'];
        $file =\Soulcms\Service::L('Router')->remove_domain($url); // 从地址中获取要生成的文件名

        $root = \Soulcms\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname']);

        $hfile = mys_to_html_file($file, $root);  // 格式化生成文件
        if (!$hfile) {
            return mys_return_data(0, '地址【'.$cat['url'].'】不规范');
        }
        ...
        if (SITE_IS_MOBILE && SITE_IS_MOBILE_HTML) {
            ...
            if (!$size) {
                @unlink($hfile);
                return mys_return_data(0, '无权限写入文件【' . $hfile . '】');
            }
        }
    ...
    protected function _Create_Show_Html($id, $page = 0) {
        ...
        if (!$id) {
            return mys_return_data(0, '内容id不存在');
        }
        ...
        ob_start();
        \Soulcms\Service::V()->init('pc');
        \Soulcms\Service::V()->module(IS_SHARE ? 'share' : $this->module['dirname']);
        ...
        // 同步数据不执行生成
        if ($data['link_id'] > 0) {
            return mys_return_data(0, '同步数据不执行生成');
        }

        $url = $page > 0 ?\Soulcms\Service::L('Router')->show_url($this->module, $data, $page) : $data['url'];
        $file =\Soulcms\Service::L('Router')->remove_domain($url); // 从地址中获取要生成的文件名
        ...
        if (!$hfile) {
            return mys_return_data(0, '地址【'.$data['url'].'】不规范');
        }
    ...
    protected function _Category_Html_File() {
        ...
        $this->_Create_Category_Html(intval(\Soulcms\Service::L('input')->get('id')));
        exit;
    ...
    protected function _Show_Html_File() {
        ...
        // 判断权限
        !mys_html_auth() && $this->_json(0, '权限验证超时，请重新执行生成');
        ...
        $this->_Create_Show_Html(intval(\Soulcms\Service::L('input')->get('id')));
        exit;
    }
    ...
    protected function _Index_Html() {
        ...
        if ($this->module['domain']) {
            ...
            $file = ltrim(\Soulcms\Service::L('Router')->remove_domain(MODULE_URL), '/'); // 从地址中获取要生成的文件名;
            !$file && $this->_json(0, mys_lang('生成文件名不合法: %s', MODULE_URL));
        }
        ...
        $this->_Index(1);
        $html = ob_get_clean();
        $file = mys_format_html_file($file, $root);
        ...
        if (SITE_IS_MOBILE) {
            ...
            $this->_Index(1);
            $html = ob_get_clean();
            $file = mys_format_html_file('mobile/' . $file, $root);
            $mobile = file_put_contents($file, $html, LOCK_EX);
        }

        $this->_json(1, mys_lang('电脑端 （%s），移动端 （%s）', mys_format_file_size($pc), mys_format_file_size($mobile)));
    }
    ...
    protected function _Show_Html() {
        ...
        // 完成
        \Soulcms\Service::L('cache')->clear($name);
        $this->_json(100, '');
    }
