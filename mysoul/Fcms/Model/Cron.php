...
class Cron extends \Soulcms\Model
{
    // 入队任务
    public function add_cron($site, $type, $data) {
        ...
        // 运行任务
        $rt['code'] && \Soulcms\Service::L('thread')->cron(['action' => 'cron', 'id' => $rt['code'] ]);
    ...
    public function save_cron($cron, $data) {
        ...
        if ((!$data['value'] && !$data['error']) || $cron['status'] >= 9) {
            ...
            $this->table('cron')->update($cron['id'], [
                ...
                'error' => mys_array2string($data['error']),
                'status' => $cron['status'] + 1,
                'updatetime' => SYS_TIME,
            ]);
            return mys_return_data(0, $cron['id'].'#'.(is_array($data['error']) ? implode('、', $data['error']) : $data['error']));
        }
    }
    ...
    public function do_cron($cron) {
        ...
        switch ($cron['type']) {
            ...
            case 'notice':
                // 通知消息
                $value = mys_string2array($cron['value']);
                list($error, $value) = \Soulcms\Service::L('notice')->cron_notice($cron['site'], $value);
            ...
            case 'weibo':
                ...
                $config = \Soulcms\Service::L('cache')->init_file('weibo')->get_file($siteid);
                ($siteid > 1 && $config['share'] == 1) && $config = \Soulcms\Service::L('cache')->init_file('weibo')->get_file(1);

                // 初始化类
                $cache = \Soulcms\Service::C()->get_cache('site', $siteid, 'weibo');
                $auth = new \SaeTClientV2($cache['key'], $cache['secret'], $config['access_token']);

                $call = $auth->upload(mys_strcut($value['content'], 250).' '.$value['url'], $value['image']);
            ...
            default:
                ...
                return mys_return_data(0, '任务查询（'.$cron['id'].'）类型【'.$cron['type'].'】不存在');
                break;
        }
    ...
    public function run_cron() {
        ...
        foreach ($crons as $cron) {
            \Soulcms\Service::L('thread')->cron(['action' => 'cron', 'id' => $cron['id'] ]);
        }

        return mys_count($crons);
