...
class Member extends \Soulcms\Model
{
    ...
    function uid($name) {
        ...
        $data = $this->db->table('member')->select('id')->where('username', mys_safe_replace($name))->get()->getRowArray();
    ...
    private function _login_log($data, $type = '') {
        ...
        $ip = \Soulcms\Service::L('input')->ip_address();
        if (!$ip || !$data['id']) {
            return;
        }

        $agent = \Soulcms\Service::L('input')->get_user_agent();
        if (strlen($agent) <= 5) {
            return;
        }
        ...
        // 验证登录情况
        if (mys_is_app('login')) {
            \Soulcms\Service::M('login', 'login')->check($data, $row, $log);
        }
        ...
        $time = \Soulcms\Service::L('input')->get_cookie('member_login');
        if (date('Ymd') != date('Ymd', $time)) {
            // 登录后的通知
            \Soulcms\Service::L('Notice')->send_notice('member_login', $data);
            // 登录后的钩子
            \Soulcms\Hooks::trigger('member_login_after', $data);
            \Soulcms\Service::L('input')->set_cookie('member_login', SYS_TIME, 3600*12);
        }
    ...
    public function member_uid() {
        ...
        // 获取本地cookie
        $uid = (int)\Soulcms\Service::L('input')->get_cookie('member_uid');
        if (!$uid) {
            return 0;
        }
    ...
    public function init_member($member) {
        ...
        // 每日登录积分处理
        $value = \Soulcms\Service::C()->_member_auth_value($member['authid'], 'login_exp');
        if ($value && !\Soulcms\Service::L('input')->get_cookie('login_experience_'.$member['id'])) {
            $this->add_experience($member['id'], $value, mys_lang('每日登陆'), '', 'login_score_'.date('Ymd', SYS_TIME), 1);
            \Soulcms\Service::L('input')->set_cookie('login_experience_'.$member['id'], 1, $time - SYS_TIME);
        }

        // 每日登录金币处理
        $value = \Soulcms\Service::C()->_member_auth_value($member['authid'], 'login_score');
        if ($value && !\Soulcms\Service::L('input')->get_cookie('login_score_'.$member['id'])) {
            $this->add_score($member['id'], $value, mys_lang('每日登陆'), '', 'login_score_'.date('Ymd', SYS_TIME), 1);
            \Soulcms\Service::L('input')->set_cookie('login_score_'.$member['id'], 1, $time - SYS_TIME);
        }
    }
    ...
    public function check_member_cookie($member) {
        ...
        // 获取本地认证cookie
        $cookie = \Soulcms\Service::L('input')->get_cookie('member_cookie');

        // 授权登陆时不验证
        if ($member['id'] && \Soulcms\Service::C()->session()->get('member_auth_uid') == $member['id']) {
            return 1;
        } elseif (!$cookie) {
            return 0;
    ...
    public function get_member($uid = 0, $name = '') {
        ...
        $data['uid'] = $data['id'];
        $data['authid'] = [];
        $data['avatar'] = mys_avatar($data['id']);
        ...
        if ($data2) {
            foreach ($data2 as $t) {
                $t['group_name'] = \Soulcms\Service::C()->member_cache['group'][$t['gid']]['name'];
                $t['group_level'] = \Soulcms\Service::C()->member_cache['group'][$t['gid']]['level'][$t['lid']]['name'];
                $data['group'][$t['gid']] = $t;
    ...
    public function authid($uid) {
        ...
        if (!$uid) {
            ...
            return \Soulcms\Service::C()->member['authid'];
        }
    ...
    public function update_group($member, $groups) {
        ...
        foreach ($groups as $group) {
            // 判断是否可用
            if (!\Soulcms\Service::C()->member_cache['group'][$group['gid']]) {
                continue;
            }
            $group['gid'] = (int)$group['gid'];
            // 判断等级是否有效
            $levels = isset(\Soulcms\Service::C()->member_cache['group'][$group['gid']]['level']) ? \Soulcms\Service::C()->member_cache['group'][$group['gid']]['level'] : [];
            if ($levels) {
                // bu存在
            ...
            $group_info = \Soulcms\Service::C()->member_cache['group'][$group['gid']];
            // 判断过期
            ...
            if ($group['etime'] && $group['etime'] - SYS_TIME < 0) {
                ...
                if ($group_info['setting']['timeout']) {
                    ...
                    if ($name == 'money') {
                        ...
                        if ($price > 0) {
                            ...
                            if ($member[$name] - $price < 0) {
                                ...
                                // 提醒通知
                                $this->notice($uid, 2, mys_lang('您的用户组（%s）已过期，自动续费失败，账户%s不足', $group_info['name'], mys_lang('余额')));
                                continue;
                            }
                            $group['etime'] = mys_member_group_etime($group_info['days']);
                            // 自动续费
                            ...
                            $rt = \Soulcms\Service::M('Pay')->add_paylog([
                                ...
                                'tousername' => '',
                                'mid' => 'system',
                                'title' => mys_lang('用户组（%s）续费', $group_info['name']),
                                'value' => -$price,
                                'type' => 'finecms',
                                'status' => 1,
                                'result' => mys_lang('有效期至%s', $group['etime'] ? mys_date($group['etime']) : mys_lang('永久')),
                                'paytime' => SYS_TIME,
                                'inputtime' => SYS_TIME,
                            ]);
                            // 提醒通知
                            $this->notice(
                                $uid,
                                2,
                                mys_lang('您的用户组（%s）已过期，自动续费成功', $group_info['name']),
                                \Soulcms\Service::L('router')->member_url('paylog/show', ['id'=>$rt['code']])
                            );
                        } else {
                            // 免费组自己续费
                            // 提醒通知
                            $this->notice(
                                $uid,
                                2,
                                mys_lang('您的用户组（%s）已过期，自动免费续期成功', $group_info['name'])
                            );
                        }
                        // 更新时间
                        ...
                        if ($price > 0) {
                            ...
                            if ($member[$name] - $price < 0) {
                                ...
                                $this->db->table('member_group_index')->where('uid', $uid)->where('gid', $group['gid'])->delete();
                                // 提醒通知
                                $this->notice($uid, 2, mys_lang('您的用户组（%s）已过期，自动续费失败，账户%s不足', $group_info['name'], SITE_SCORE));
                                continue;
                            }
                            // 自动续费
                            $group['etime'] = mys_member_group_etime($group_info['days']);
                            // 自动续费
                            $rt = $this->add_score($uid, -$price, mys_lang('您的用户组（%s）自动续费', $group_info['name']));
                            if (!$rt['code']) {
                                continue;
                            }
                            // 提醒通知
                    ...
                    $this->db->table('member_group_index')->where('uid', $uid)->where('gid', $group['gid'])->delete();
                    // 提醒通知
                    $this->notice($uid, 2, mys_lang('您的用户组（%s）已过期，系统权限已关闭', $group_info['name']));
                }

            }
            // 开启自动升级时需要判断等级
            if ($levels
                && \Soulcms\Service::C()->member_cache['group'][$group['gid']]['setting']['level']['auto']) {
                $value = \Soulcms\Service::C()->member_cache['group'][$group['gid']]['setting']['level']['unit'] ? $member['spend'] : $member['experience'];
                ...
                foreach ($level as $t) {
                    if ($value >= $t['value']) {
                        if ($group['lid'] != $t['id']) {
                            ...
                            /* 等级升级 */
                            $this->notice($uid, 2, mys_lang('您的用户组（%s）等级自动升级为（%s）', \Soulcms\Service::C()->member_cache['group'][$group['gid']]['name'], $t['name']));
                            $group['lid'] = $t['id'];
                        }
                        break;
                    }
    ...
    public function delete_group($uid, $gid) {
        ...
        // 提醒
        $this->notice($uid, 2, mys_lang('您的用户组（%s）被取消', \Soulcms\Service::C()->member_cache['group'][$gid]['name']));

        // 判断微信标记组
        if (mys_is_app('weixin')) {
            \Soulcms\Service::C()->init_file('weixin');
            \Soulcms\Service::M('user', 'weixin')->delete_member_group($uid, $gid);
        }
    }
    ...
    public function insert_group($uid, $gid, $is_notice = 1) {
        $data = [
            ...
            'lid' => 0,
            'stime' => SYS_TIME,
            'etime' => mys_member_group_etime(\Soulcms\Service::C()->member_cache['group'][$gid]['days']),
        ];
        $this->table('member_group_index')->insert($data);
        // 挂钩点 用户组变更之后
        $call = $this->member_info($uid);
        $call['group_gid'] = $call['gid'] = $gid;
        $call['group_name'] = \Soulcms\Service::C()->member_cache['group'][$gid]['name'];
        $is_notice && \Soulcms\Service::L('Notice')->send_notice('member_edit_group', $call);
        \Soulcms\Hooks::trigger('member_edit_group_after', $call);
        // 判断微信标记组
        if (mys_is_app('weixin')) {
            \Soulcms\Service::C()->init_file('weixin');
            \Soulcms\Service::M('user', 'weixin')->add_member_group($uid, $gid);
        }
    }
    ...
    public function update_level($uid, $gid, $lid) {
        ...
        $call = $this->member_info($uid);
        $call['group_name'] = \Soulcms\Service::C()->member_cache['group'][$gid]['name'];
        $call['group_level'] = \Soulcms\Service::C()->member_cache['group'][$gid]['level'][$lid]['name'];
        \Soulcms\Service::L('Notice')->send_notice('member_edit_level', $call);
        \Soulcms\Hooks::trigger('member_edit_level_after', $data, $old);
    }
    ...
    public function admin_notice($site, $type, $member, $msg, $uri, $to = []) {
        ...
        $this->db->table('admin_notice')->insert([
            'site' => (int)$site,
            'type' => $type,
            'msg' => mys_strcut(mys_clearhtml($msg), 100),
            'uri' => $uri,
    ...
    public function save_cookie($data, $remember = 0) {
        ...
        \Soulcms\Service::L('input')->set_cookie('member_uid', $data['id'], $expire);
        \Soulcms\Service::L('input')->set_cookie('member_cookie', substr(md5(SYS_KEY.$data['password']), 5, 20), $expire);
    ...
    public function get_sso_url() {
        ...
        if (is_file(WRITEPATH.'config/domain_sso.php')) {
            ...
            foreach ($sso as $u) {
                $this->sso_url[] = mys_http_prefix($u).'/';
            }
        }
    ...
    public function sso($data, $remember = 0) {
        ...
        foreach ($url as $u) {
            $code = mys_authcode($data['id'].'-'.$data['salt'], 'ENCODE');
    ...
    public function logout() {
        ...
        \Soulcms\Service::L('input')->set_cookie('member_uid', 0, -100000000);
        \Soulcms\Service::L('input')->set_cookie('member_cookie', '', -100000000);
        \Soulcms\Service::L('input')->set_cookie('admin_login_member', '', -100000000);
    ...
    private function _find_member_info($username) {
        ...
        if (!$data && \Soulcms\Service::C()->member_cache['login']['field']) {
            if (in_array('email', \Soulcms\Service::C()->member_cache['login']['field'])
                && \Soulcms\Service::L('Form')->check_email($username)) {
                $data = $this->db->table('member')->where('email', $username)->get()->getRowArray();
            } elseif (in_array('phone', \Soulcms\Service::C()->member_cache['login']['field'])
                && \Soulcms\Service::L('Form')->check_phone($username)) {
    ...
    private function _is_admin_login_member($uid) {
        ...
        if (!\Soulcms\Service::C()->member_cache['login']['admin']
            && $this->db->table('admin')->where('uid', $uid)->countAllResults()) {
            return mys_return_data(0, mys_lang('管理员账号不允许前台登录'));
        }
    ...
    public function login($username, $password, $remember = 0) {
        ...
        if (defined('UCSSO_API')) {
                    ...
                    if ($data) {
                        ...
                        */
                        if ($rt['code'] > 0) {
                            ...
                            if (!$rt2['code']) {
                                return mys_return_data(0, mys_lang('通信失败：%s', $rt2['msg']));
                            }
                        } else {
            ...
            if (!$data) {
                return mys_return_data(0, mys_lang('用户不存在'));
            }
        } else {
            $data = $this->_find_member_info($username);
            if (!$data) {
            ...
            if (md5(md5($password).$data['salt'].md5($password)) != $data['password']) {
                return mys_return_data(0, mys_lang('密码不正确'));
            }
        }
    ...
    public function login_sms($phone, $remember) {
        ...
        if (!$data) {
            return mys_return_data(0, mys_lang('手机号码未注册'));
        }
    ...
    public function register_oauth_bang($oauth, $groupid, $data) {
        ...
        if (!$oauth) {
            return mys_return_data(0, mys_lang('OAuth数据不存在，请重试'));
        }
        ...
        mys_is_app('weixin') && $oauth['oauth'] == 'wechat' && $this->db->table('weixin_user')->where('openid', $oauth['oid'])->update([
            'uid' => $data['id'],
        ...
        // 下载头像
        \Soulcms\Service::L('thread')->cron(['action' => 'oauth_down_avatar', 'id' => $oauth['id'] ]);
    ...
    public function register_oauth($groupid, $oauth) {
        ...
        $name = mys_clear_emoji($oauth['nickname']);
        $member = [
            'username' => '',
            'name' => $name,
            'email' => '',
        ...
        // 下载头像和同步登录
        $sso = $this->sso($data);
        $sso[] = \Soulcms\Service::L('router')->member_url('api/avatar', ['id'=>$oauth['id']]);
    ...
    public function register($groupid, $member, $data = [], $sync = 1) {
        ...
        // 默认注册组
        !$groupid && $groupid = (int)\Soulcms\Service::C()->member_cache['register']['groupid'];

        // 验证唯一性
        if ($member['username'] && $this->db->table('member')->where('username', $member['username'])->countAllResults()) {
            return mys_return_data(0, mys_lang('账号%s已经注册', $member['username']), ['field' => 'username']);
        ...
        if ($member['username'] == 'guest') {
            return mys_return_data(0, mys_lang('此名称guest系统不允许注册'), ['field' => 'username']);
        } elseif (!IS_ADMIN && \Soulcms\Service::C()->member_cache['register']['notallow']) {
            foreach (\Soulcms\Service::C()->member_cache['register']['notallow'] as $mt) {
                if ($mt && stripos($member['username'], $mt) !== false) {
                    return mys_return_data(0, mys_lang('账号%s禁止包含关键字[%s]', $member['username'], $mt), ['field' => 'username']);
                }
            }
        }
        ...
        $member['score'] = 0;
        $member['experience'] = 0;
        $member['regip'] = (string)\Soulcms\Service::L('input')->ip_address();
        ...
        // 没有账号，随机一个默认登录账号
        if (!$member['username']) {
            $member['username'] = strtolower(trim(\Soulcms\Service::C()->member_cache['register']['unprefix'].$rt['code']));
        ...
        $data['is_avatar'] = 0;
        // 审核状态值
        $data['is_verify'] = IS_ADMIN ? 1 : (\Soulcms\Service::C()->member_cache['register']['verify'] ? 0 : 1);
        $data['is_mobile'] = 0;
        $data['is_complete'] = 0;
        ...
        // 审核判断
        if (!$data['is_verify']) {
            switch (\Soulcms\Service::C()->member_cache['register']['verify']) {
                ...
                case 'email':
                    $this->sendmail($member['email'], mys_lang('注册邮件验证'), 'member_verify.html', $data);
                    break;
            }
            // 发送审核提醒
            $this->admin_notice(0, 'member', $member, mys_lang('新会员【%s】注册审核', $member['username']), 'member_verify/index:field/id/keyword/'.$uid);
        }

        // 注册后的通知
        \Soulcms\Service::L('Notice')->send_notice('member_register', $data);

        // 注册后的钩子
        \Soulcms\Hooks::trigger('member_register_after', $data);

        // 邀请注册绑定
        if (mys_is_app('yaoqing')) {
            $puid = \Soulcms\Service::C()->session()->get('app_yaoqing_uid');
            if ($puid) {
                \Soulcms\Service::M('yq', 'yaoqing')->register($puid, $data);
            }
        }
    ...
    public function insert_oauth($uid, $type, $data, $state = '') {
        ...
        if ($uid && $data['avatar']) {
            ...
            if (!is_file($cache_path.$uid.'.jpg')) {
                // 没有头像下载头像
                $img = mys_catcher_data($data['avatar']);
                if (strlen($img) > 20) {
        ...
        // 存储session
        \Soulcms\Service::C()->session()->set('member_auth_'.$type.'_'.$data['oauth'], $id);

        return mys_return_data($id, $type == 'login' ? \Soulcms\Service::L('router')->member_url('login/oauth', ['name' => $data['oauth'], 'state' => $state]) : \Soulcms\Service::L('router')->member_url('account/oauth', ['name' => $data['oauth']]));
    }
    ...
    public function edit_password($member, $password) {
        ...
        // 通知
        \Soulcms\Service::L('Notice')->send_notice('member_edit_password', $member);
        // 钩子
        \Soulcms\Hooks::trigger('member_edit_password_after', $member);
    ...
    public function sendmail($tomail, $subject, $msg, $data = []) {
        ...
        if (!$tomail) {
            return mys_return_data(0, mys_lang('第一个参数不能为空'));
        } elseif (!$subject) {
            return mys_return_data(0, mys_lang('第二个参数不能为空'));
        } elseif (!$msg) {
            return mys_return_data(0, mys_lang('第三个参数不能为空'));
        }

        $cache = \Soulcms\Service::L('cache')->get('email');
        if (!$cache) {
            return mys_return_data(0, mys_lang('无邮件smtp配置'));
        }
        ...
        if (trim(strtolower(strrchr($msg, '.')), '.') == 'html') {
            ...
            if (!$content) {
                ...
                return mys_return_data(0, mys_lang('邮件模板#%s不存在', $msg));
            }
            ob_start();
        ...
        return mys_return_data(0, 'Error：'.$dmail->error);
    }
    ...
    public function sendsms_text($mobile, $content, $type = 'text') {
        ...
        if (!$mobile || !$content) {
            return mys_return_data(0, mys_lang('手机号码或内容不能为空'));
        }

        $file = WRITEPATH.'config/sms.php';
        if (!is_file($file)) {
            log_message('error', '短信接口配置文件不存在');
            return mys_return_data(0, mys_lang('接口配置文件不存在'));
        }
        ...
        if ($config['third']) {
            ...
            if (function_exists($method)) {
                ...
                $error = mys_lang('你没有定义第三方短信接口: '. $method);
            ...
            $content = $type == 'code' ? mys_lang('您的本次验证码是: %s', $content) : $content;
            $url = '';
            $result = mys_catcher_data($url);
            if (!$result) {
                log_message('error', '访问官方云短信服务器失败');
                return mys_return_data(0, mys_lang('访问官方云短信服务器失败'));
            }
    ...
    public function _weixin_template_access_token() {
        ...
        $cache_data = \Soulcms\Service::L('cache')->init('file')->get('access_token');
        if (isset($cache_data['time']) && $cache_data['access_token']) {
            $access_token = $cache_data['access_token'];
        } else {
            $weixin = \Soulcms\Service::C()->get_cache('weixin');
            ...
            if (!$result['code']) {
                return mys_return_data(0, 'AccessToken：'.$result['msg']);
            }
            $rt = \Soulcms\Service::L('cache')->init('file')->save('access_token', $result['data'], 3600);
            if (!$rt) {
                return mys_return_data(0, 'AccessToken：/cache/temp/目录不可写，文件写入失败');
            }
            $access_token = $result['data']['access_token'];
        }

        return mys_return_data(1, 'ok', ['access_token' => $access_token]);
    }
    ...
    public function wexin_template($uid, $id, $data, $url = '', $color = '') {
        ...
        if (!$oauth) {
            return mys_return_data(0, 'uid（'.$uid.'）未绑定账号');
        }
    ...
    public function add_experience($uid, $val, $note = '', $url = '', $mark = '', $count = 0) {
        ...
        if (!$uid || !$val) {
            return mys_return_data(0, mys_lang('参数失败'));
        }
        ...
        if ($mark && $count && $this->db->table('member_explog')->where('uid', $uid)->where('mark', $mark)->countAllResults() >= $count) {
            return mys_return_data(0, mys_lang('本次不能累计增减金币'));
        }
    ...
    public function add_score($uid, $val, $note = '', $url = '', $mark = '', $count = 0) {
        ...
        if (!$uid || !$val) {
            return mys_return_data(0, mys_lang('参数错误'));
        }
    ...
    public function add_money($uid, $value) {
        ...
        if (!$value) {
            return mys_return_data(0, mys_lang('金额不正确'));
        }
    ...
    public function add_freeze($uid, $value) {
        ...
        if (!$uid || !$value) {
            return mys_return_data(0, mys_lang('参数不正确'));
        }
    ...
    public function member_delete($id) {
        ...
        SYS_ATTACHMENT_DB && \Soulcms\Service::M('Attachment')->uid_delete($id);
        ...
        foreach ($this->site as $siteid) {
            ...
            if ($module) {
                foreach ($module as $m) {
                    ...
                    $mdb = \Soulcms\Service::M('Content', $mdir);
        ...
        // 同步删除动作
        \Soulcms\Service::M('Sync')->delete_member($id);
    ...
    public function cache($site = SITE_ID) {
        ...
        if ($result) {
            foreach ($result as $t) {
                $cache[$t['name']] = mys_string2array($t['value']);
            }
        }
        ...
        if ($field) {
            foreach ($field as $f) {
                $f['setting'] = mys_string2array($f['setting']);
        ...
        \Soulcms\Service::L('cache')->set_file('member', $cache);
    }
}