...
// 数据表
class Table extends \Soulcms\Model
{
    ...
    public function cache($siteid = SITE_ID, $module = null) {
        ...
        // 缓存表结构
        \Soulcms\Service::L('cache')->set_file('table-'.$siteid, $cache);
        ...
        \Soulcms\Service::L('cache')->set_file('table-field', $cache);

        // 缓存付款表
        \Soulcms\Service::L('cache')->set_file('table-pay-'.$siteid, $paytable);
        /*
    ...
    public function get_cache_field($table) {
        ...
        $tableinfo = \Soulcms\Service::L('cache')->get('table-'.SITE_ID);
        if (!$tableinfo) {
            // 没有表结构缓存时返回空
            return [];
        }
        ...
        if ($sql_data) {
            foreach($sql_data as $query){
                ...
                if ($this->db->simpleQuery($ret)) {
                    ...
                    $rt = $this->db->error();
                    return mys_return_data(0, $rt['message'].'<br> '.$ret);
                }
            }
        }
        
        return mys_return_data(1, '', [$count, $todo]);
    }
    ...
    public function create_form($data) {
        ...
        $this->db->table('field')->insert(array(
            ...
            'issearch' => 1,
            'disabled' => 0,
            'setting' => mys_array2string(array(
                'option' => array(
                    'width' => 300, // 表单宽度
    ...
    public function create_module_form($data) {
        ...
        // 为全部站点模块创建表单
        foreach (\Soulcms\Service::C()->site_info as $sid => $v) {
            $par = $this->dbprefix(mys_module_table_prefix($data['module'], $sid)); // 父级表
    ...
    public function get_export_field_name($table, $update = 0) {
        ...
        $value = $row['value'] ? mys_string2array($row['value']) : [];
