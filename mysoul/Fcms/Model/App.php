...
// 程序插件管理
class App extends \Soulcms\Model
{
    public $cfg_cache;
    ...
    public function install($dir) {
        ...
        if (!is_file($path.'Config/App.php')) {
            return mys_return_data(0, mys_lang('应用配置文件不存在'));
        }
        ...
        if (!$rt) {
            return mys_return_data(0, 'App/'.ucfirst($dir).'/程序目录无法写入');
        }
        ...
        if (is_file($path.'install.lock')) {
            return mys_return_data(0, mys_lang('此程序已经安装'));
        }

        if (isset($config['ftype']) && $config['ftype'] == 'module') {
            // 如果是内容模块，就进入内容模块安装模式
            \Soulcms\Service::M('module')->install($dir, $config, 1);
        } else {
            // 执行sql语句
        ...
        return mys_return_data(1, mys_lang('安装成功'));
    }
    ...
    public function uninstall($dir) {
        ...
        if (isset($config['ftype']) && $config['ftype'] == 'module') {
            // 如果是内容模块，就进入内容模块安装模式
            \Soulcms\Service::M('module')->uninstall($dir, $config, 1);
        } else {
            // 执行sql语句
    ...
    public function get_config($dir) {
        ...
        if (!isset($this->cfg_cache[$dir])) {
            $this->cfg_cache[$dir] = \Soulcms\Service::L('Cache')->get_file($dir, 'app');
        }
    ...
    public function save_config($dir, $data) {
        ...
        \Soulcms\Service::L('Cache')->set_file($dir, $data, 'app');
    }
