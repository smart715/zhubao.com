...
class Menu extends \Soulcms\Model {
        ...
        if ($config['system'] == 1) {
            ...
            if ($left) {
                ...
                $save = [
                    'uri' => $mdir.'/home/index',
                    'mark' => 'module-'.$mdir,
                    'name' => $menu && $menu['name'] ? $menu['name'] : mys_lang('%s管理', $config['name']),
                    'icon' => $menu && $menu['icon'] ? $menu['icon'] : mys_icon($config['icon']),
            ...
            if ($left) {
                ...
                if ($config['system'] == 1) {
                    ...
                    $save = [
                        ...
                        'name' => $menu && $menu['name'] ? $menu['name'] : mys_lang('%s审核', $config['name']),
                        'icon' => $menu && $menu['icon'] ? $menu['icon'] : mys_icon($config['icon']),
                        'displayorder' => $menu ? intval($menu['displayorder']) : '-1',
                    ];
                    $menu ? $this->_edit('admin', $menu['id'], $save) : $this->_add('admin', $left['id'], $save);
                ...
                $save = [
                    ...
                    'name' => $menu && $menu['name'] ? $menu['name'] : mys_lang('%s评论', $config['name']),
                    'icon' => 'fa fa-comments',
                    'displayorder' => $menu ? intval($menu['displayorder']) : '-1',
                ];
                $menu ? $this->_edit('admin', $menu['id'], $save) : $this->_add('admin', $left['id'], $save);
                ...
                if ($form) {
                    foreach ($form as $t) {
                        ...
                        $save = [
                            ...
                            'mark' => $mark,
                            'name' => $menu && $menu['name'] ? $menu['name'] : mys_lang('%s%s', $config['name'], $t['name']),
                            'icon' => $menu && $menu['icon'] ? $menu['icon'] : mys_icon($t['setting']['icon']),
                            'displayorder' => $menu ? intval($menu['displayorder']) : '-1',
                        ];
                        $menu ? $this->_edit('admin', $menu['id'], $save) : $this->_add('admin', $left['id'], $save);
                    }
    ...
    public function init($table = '') {
        ...
        #\Soulcms\Service::M('Form')->cache(); // 更新表单菜单
    ...
    public function gets($table) {
        ...
        if ($data) {
            ...
            $menu = \Soulcms\Service::L('tree')->get($data);
        }
    ...
    public function parent_select($table, $level, $id = NULL, $name = NULL) {
        ...
        switch ($level) {
            case 1: // 顶级菜单
                $select.= '<option value="0">'.mys_lang('顶级菜单').'</option>';
                break;
            case 2: // 分组菜单
    ...
    public function cache($site = SITE_ID) {
        ...
        if ($data) {
            ...
            foreach ($data as $t) {
                if ($t['pid'] == 0) {
                    ...
                    $t['group'] = mys_string2array($t['group']);
                    $list['url'][$t['id']] = $t;
                    foreach ($data as $n) {
                        $n['site'] = mys_string2array($n['site']);
                        $n['group'] = mys_string2array($n['group']);
                        if ($n['pid'] == $t['id']) {
        ...
        \Soulcms\Service::L('cache')->set_file('menu-admin', $menu['admin']);
        \Soulcms\Service::L('cache')->set_file('menu-admin-uri', $menu['admin-uri']);
        \Soulcms\Service::L('cache')->set_file('menu-member', $menu['member']);
