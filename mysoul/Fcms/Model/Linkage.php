...
class Linkage extends \Soulcms\Model
{
    ...
    public function create($data) {
		...
		) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='联动菜单".mys_safe_replace($data['name'])."数据表'"));
        if (!$rt['code']) {
            // 创建失败删除数据
    ...
    public function add_list($key, $data) {
        ...
        if ($data['all']) {
            // 批量
            $c = 0;
            $py = \Soulcms\Service::L('pinyin'); // 拼音转换类
            ...
            return mys_return_data(1, mys_lang('批量添加%s个', $c));
        } else {
            // 单个
            $data['name'] = trim($data['name']);
            if (!$data['name']) {
                return mys_return_data(0, mys_lang('名称不能为空'));
            } elseif (!$data['cname']) {
                return mys_return_data(0, mys_lang('别名不能为空'));
            ...
            // 更新pid
            $pid && $this->table('linkage_data_'.$key)->update($pid, ['child' => 1]);
            return mys_return_data(1, mys_lang('操作成功'));
        }
    ...
    public function getList($link, $pid = 'NULL') {
        ...
        if ($pid === 'NULL') {
            ...
            $data = \Soulcms\Service::L('cache')->get_data($name);
            if ($data) {
                return $data;
            }
            ...
            \Soulcms\Service::L('cache')->set_data($name, $data);
        } else {
            $db = $this->db->table('linkage_data_'.$key);
            // 站点查询
            $link['type'] == 1 && $db->where('site', SITE_ID);
    ...
    public function cache($siteid = SITE_ID) {
        ...
        if ($linkage) {
            foreach ($linkage as $link) {
                ...
                \Soulcms\Service::L('cache')->set_file('linkage-'.$siteid.'-'.$link['code'], $data);
                \Soulcms\Service::L('cache')->set_file('linkage-'.$siteid.'-'.$link['code'].'-id', $cid);
                \Soulcms\Service::L('cache')->set_file('linkage-'.$siteid.'-'.$link['code'].'-key', $link['id']);
                \Soulcms\Service::L('cache')->set_file('linkage-'.$siteid.'-'.$link['code'].'-level', $lv ? max($lv) : 0);
            }
        }
