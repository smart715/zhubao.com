...
// 字段操作表
class Field extends \Soulcms\Model
{
    ...
    public function get_mytable_field($table, $siteid = 0) {
        ...
        if (!$value) {
            ...
            \Soulcms\Service::L('cache')->init()->save($name, $value);
        }
    ...
    public function delete_field($ids) {
        ...
        foreach ($ids as $id) {
            ...
            if (!$data) {
                return mys_return_data(0, mys_lang('字段不存在(id:%s)', $id));
            } elseif ($data['issystem']) {
                return mys_return_data(0, mys_lang('系统字段不允许删除(id:%s)', $id));
            }
    ...
    public function add($data, $field) {
        ...
        $data['setting'] = mys_array2string($data['setting']);
        $data['issystem'] = 0;
        ...
        if ($rt['code'] && $sql) {
            ...
            if ($this->_table_field && $yz = $field->test_sql($this->_table_field, $data['fieldname'])) {
                ...
                return mys_return_data(0, mys_lang('字段创建失败: %s', $yz));
            }
        }
    ...
    public function edit($_data, $data, $sql) {
        ...
        if (!$_data || !$data) {
            return mys_return_data(0, mys_lang('参数不完整'));
        }
    ...
    private function _field_mform($name) {
        // 主表
        $table = $this->dbprefix(mys_module_table_prefix($this->data['module']).'_form_'.$this->data['table']);
