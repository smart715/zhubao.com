...
class Module extends \Soulcms\Model
{
    ...
    public function All($is_file_name = 0) {
        ...
        if ($list && $is_file_name) {
            foreach ($list as $i => $t) {
                if (is_file(mys_get_app_dir($t['dirname']).'Config/App.php')) {
                    $cfg = require mys_get_app_dir($t['dirname']).'Config/App.php';
    ...
    public function config($data, $post, $value = []) {
        ...
        if ($value['site']) {
            ...
            $update['site'] = mys_array2string($site);
        }

        // 排列table字段顺序
        $update['setting']['list_field'] = mys_list_field_order($update['setting']['list_field']);

        $update['setting'] = mys_array2string($update['setting']);
    ...
    public function create_form($dir, $data) {
        ...
        // 创建表
        \Soulcms\Service::M('Table')->create_module_form($data);
    ...
    public function delete_form($ids) {
        ...
        foreach ($ids as $id) {
            ...
            if (!$row) {
                return mys_return_data(0, mys_lang('模块表单不存在(id:%s)', $id));
            }
            ...
            // 删除表数据
            \Soulcms\Service::M('Table')->delete_module_form($row);
        }
    ...
    public function uninstall($dir, $config = [], $is_app = 0) {
        ...
        if (!$module) {
            return mys_return_data(0, mys_lang('模块尚未安装'));
        }

        $mpath = mys_get_app_dir($dir);
        $table = $this->dbprefix(mys_module_table_prefix($dir)); // 当前表前缀
        ...
        if (count($site) == 1 && isset($config['ftype']) && $config['ftype'] == 'module' && $is_app == 0) {
            // 只有一个站点时，卸载需要 验证应用模块
            return mys_return_data(0, mys_lang('此模块属于应用类型，请到[本地应用]中去卸载'));
        }
        ...
        if (isset($site[SITE_ID]) && $site[SITE_ID]) {
            ...
            if (is_file($mpath.'Config/Uninstall.sql')) {
                ...
                $sql && \Soulcms\Service::M('table')->_query(
                    $sql,
                    [
                        [
                            '{moduleid}',
        ...
        if (empty($site)) {
            ...
            // 删除自定义菜单
            \Soulcms\Service::M('Menu')->delete_app($dir);
            // 删除自定义表单
            $this->db->table('module_form')->where('module', $dir)->delete();
        } else {
            // 删除当前站点配置
            $this->table('module')->update($module['id'], ['site' => mys_array2string($site)]);
        }

        return mys_return_data(1, mys_lang('卸载成功'));
    }
    ...
    public function install($dir, $config = [], $is_app = 0) {
        ...
        if (!$config) {
            if (!is_file($mpath.'Config/App.php')) {
                return mys_return_data(0, mys_lang('模块配置文件不存在'));
            }
        ...
        $system_table = require CMSPATH.'Config/SysTable.php';
        if (!$system_table ) {
            return mys_return_data(0, mys_lang('系统表配置文件不存在'));
        }
        ...
        if (!$module) {
            if (isset($config['ftype']) && $config['ftype'] == 'module' && $is_app == 0) {
                // 首次安装模块时，验证应用模块
                return mys_return_data(0, mys_lang('此模块属于应用类型，请到[本地应用]中去安装'));
            }
            $module = [
                'site' => mys_array2string([
                    SITE_ID => [
                        'html' => 0,
                        'theme' => 'default',
                        'domain' => '',
            ...
            $module['site'] = mys_string2array($module['site']);
            $module['site'][SITE_ID] = [
                'html' => 0,
                'theme' => 'default',
                'domain' => '',
            ...
            $this->table('module')->update($module['id'], [
                'site' => mys_array2string($module['site'])
            ]);
        }
        ...
        foreach ($system_table as $name => $sql) {
            if (mys_count($site) == 1) {
                ...
                $sql = $this->db->query("SHOW CREATE TABLE `".$this->dbprefix(mys_module_table_prefix($dir, $siteid).$name)."`")->getRowArray();
                $sql = str_replace(
        ...
        if (isset($config['scategory']) && $config['scategory']) {
            if (!\Soulcms\Service::M()->db->fieldExists('tid', $table.'_category')) {
                \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'_category` ADD `tid` tinyint(1) NOT NULL COMMENT \'栏目类型，0单页，1模块，2外链\'');
            }
            if (!\Soulcms\Service::M()->db->fieldExists('content', $table.'_category')) {
                \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'_category` ADD `content` mediumtext NOT NULL COMMENT \'单页内容\'');
            }
        }
        // 第一次安装模块
        // system = 2 2菜单不出现在内容下，由开发者自定义
        if ($config['system'] == 2 && mys_count($site) == 1 && $is_app == 0 && is_file($mpath.'Config/Menu.php')) {
            \Soulcms\Service::M('Menu')->add_app($dir);
        }
        ...
        if (isset($_GET['is_install_db']) && $_GET['is_install_db'] == 0) {
            return mys_return_data(1, mys_lang('安装成功'), $module);
        }
        ...
        return mys_return_data(1, mys_lang('操作成功，请刷新后台页面'), $module);
    }
    ...
    private function _add_field($field, $ismain, $rid, $rname) {
        ...
        $this->db->table('field')->insert(array(
            ...
            'ismain' => $ismain,
            'setting' => mys_array2string($field['setting']),
    ...
    public function get_module_info($module = null) {
        ...
        if ($module) {
            foreach ($module as $data) {
                $mdir = $data['dirname'];
                // 如果没有配置文件就不更新缓存
                if (!is_file(mys_get_app_dir($mdir).'Config/App.php') || $data['disabled']) {
                    continue;
                }
                $config = require mys_get_app_dir($mdir).'Config/App.php';
                $setting = mys_string2array($data['setting']);#print_r($setting);
                $setting['list_field'] = mys_list_field_order($setting['list_field']);
                $setting['comment_list_field'] = mys_list_field_order($setting['comment_list_field']);
                $rt[$mdir] = [
                    'id' => $data['id'],
                    'name' => $config['name'],
                    'icon' => $config['icon'],
                    'site' => mys_string2array($data['site']),
                    'config' => $config,
                    'share' => $data['share'],
                    'comment' => mys_string2array($data['comment']),
                    'setting' => $setting,
                    'dirname' => $mdir,
                ];
            }
    ...
    private function _get_category_cache($siteid, $cache) {
        ...
        // 修复优化栏目
        $category = \Soulcms\Service::M('category')->init(['table' => $siteid.'_'.$cdir.'_category'])->repair($category, $cdir);
        ...
        if ($category) {
            foreach ($category as $c) {
                ...
                $c['setting'] = mys_string2array($c['setting']);
                if ($cache['share']) {
                    // 共享栏目时
                    //以本栏目为准
                ...
                // 权限
                $c['permission'] = $c['child'] && !$cache['setting']['pcatpost'] ? '' : mys_string2array($c['permission']);
                // 获取栏目url
                $c['url'] = $c['tid'] == 2 && $c['setting']['linkurl'] ? $c['setting']['linkurl'] : \Soulcms\Service::L('router')->category_url($cache, $c);
                //$c['furl'] = $c['setting']['linkurl'] ? $c['setting']['linkurl'] : \Soulcms\Service::L('router')->category_url($cache, $c, 0, '{fid}');
                // 按分站生成url
                // 统计栏目文章数量
        ...
        if ($cache['share'] && $this->cat_share_lock[$siteid]) {
            // 删除缓存
            \Soulcms\Service::L('cache')->clear('module-'.$siteid.'-share');
            // 写入缓存
            \Soulcms\Service::L('cache')->set_file('module-'.$siteid.'-share', [
                'id' => 0,
                'name' => '共享',
                'share' => 1,
                'dirname' => 'share',
    ...
    public function cache($siteid = SITE_ID, $module = null) {
        ...
        \Soulcms\Service::L('cache')->set_file('module-'.$siteid, []);
        \Soulcms\Service::L('cache')->set_file('module-'.$siteid.'-share', []);
        \Soulcms\Service::L('cache')->set_file('module-'.$siteid.'-content', []);

        $all = $content = [];
        $module = $this->get_module_info($module);
        $menu_model = \Soulcms\Service::M('Menu');
        if ($module) {
            foreach ($module as $mdir => $data) {
                ...
                if (isset($data['site'][$siteid]) && $data['site'][$siteid]) {
                    ...
                    // 绑定的域名
                    $cache['domain'] = $data['site'][$siteid]['domain'] ? mys_http_prefix($data['site'][$siteid]['domain'].'/') : '';
                    $cache['mobile_domain'] = $data['site'][$siteid]['mobile_domain'] ? mys_http_prefix($data['site'][$siteid]['mobile_domain'].'/') : '';
                    // 补全url
                    $cache['url'] = \Soulcms\Service::L('router')->module_url($cache, $siteid); // 模块的URL地址
                ...
                if ($form) {
                    foreach ($form as $t) {
                        ...
                        $t['permission'] = mys_string2array($t['permission']);
                ...
                // 执行模块自己的缓存程序
                if (is_file(mys_get_app_dir($mdir).'Config/Cache.php')) {
                    require mys_get_app_dir($mdir).'Config/Cache.php';
                }
                ...
                // 删除缓存
                \Soulcms\Service::L('cache')->clear('module-'.$siteid.'-'.$mdir);

                // 写入缓存
                \Soulcms\Service::L('cache')->set_file('module-'.$siteid.'-'.$mdir, $cache);
            }
        }

        // 写入缓存
        \Soulcms\Service::L('cache')->set_file('module-'.$siteid, $all);
        \Soulcms\Service::L('cache')->set_file('module-'.$siteid.'-content', $content);
