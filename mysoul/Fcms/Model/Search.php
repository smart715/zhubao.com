...
// 模块搜索类
class Search extends \Soulcms\Model {
    ...
    public function get($module, $get, $catid) {
        ...
        // 查询缓存
        $id = md5(MOD_DIR.mys_array2string($get));
        if (SYS_CACHE_SEARCH) {
        ...
        if (!$data) {
            ...
            // 主表的字段
            $field = \Soulcms\Service::L('cache')->get('table-'.SITE_ID, $this->dbprefix($this->mytable));
            if (!$field) {
                return mys_return_data(0, mys_lang('主表【%s】字段不存在', $this->mytable));
            }
            ...
            if ($param['keyword'] != '') {
                ...
                $temp = [];
                $search_keyword = trim(str_replace([' ', '_'], '%', mys_safe_replace($param['keyword'])), '%');
                foreach ($sfield as $t) {
            ...
            if ($result) {
                ...
                $data = [
                    'id' => $id,
                    'catid' => intval($catid),
                    'params' => mys_array2string(['param' => $param, 'sql' => $sql]),
        ...
        // 格式化值
        $p = mys_string2array($data['params']);
        $data['sql'] = $p['sql'];
        $data['params'] = $p['param'];
    ...
    public function get_param($module) {
        ...
        $get =  $_GET;
        $get = isset($get['rewrite']) ? mys_search_rewrite_decode($get['rewrite'], $module['setting']['search']) : $get;
        $get && $get = \Soulcms\Service::L('input')->xss_clean($get);
        ...
        $_GET['page'] = $get['page'];
        $get['keyword'] = mys_get_keyword($get['keyword']);
    ...
    private function _where($table, $name, $value, $field) {
        ...
        if (strpos($value, '%') === 0 && strrchr($value, '%') === '%') {
            ...
            if (SITE_MAP_LAT && SITE_MAP_LNG) {
                ...
                $lat = '`'.$table.'`.`'.$name.'_lat`';
                $lng = '`'.$table.'`.`'.$name.'_lng`';
                $squares = mys_square_point(SITE_MAP_LNG, SITE_MAP_LAT, $value);
                ...
                \Soulcms\Service::C()->goto_404_page(mys_lang('没有定位到您的坐标'));
            }
            ...
            foreach ($arr as $value) {
                $data = mys_linkage($field['setting']['option']['linkage'], $value);
                if ($data) {
                    if ($data['child']) {
            ...
            foreach ($arr as $value) {
                if (is_numeric($value)) {
                    ...
                    $where[] = '`'.$table.'`.`'.$name.'`="'.mys_safe_replace($value, ['\\', '/']).'"';
                }
            }
            return $where ? '('.implode(' OR ', $where).')' : '';
            ...
            return '`'.$table.'`.`'.$name.'`="'.mys_safe_replace($value, ['\\', '/']).'"';
        }
    }
