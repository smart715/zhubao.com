...
class Category extends \Soulcms\Model
{
    ...
    public function get_parent_mid($category, $id, $update = 0) {
        ...
        $mid = '';
        $ids = mys_array2array(explode(',',  $category[$id]['childids']), explode(',',  $category[$id]['pids']));
        foreach ($ids as $id) {
    ...
    public function update_parent_mid($category, $catid) {
        ...
        $mid && mys_count(array_unique($mid)) > 1 && $this->table($this->tablename)->update((int)$catid, array(
            'mid' => '',
            'tid' => 0
        ));
    }
    ...
    public function copy_value($at, $setting, $id) {
        ...
        $this->table($this->tablename)->update($id, [
            'setting' => mys_array2string($row['setting']),
        ]);
    }
    ...
    public function delete_content($cats, $module) {
        ...
        if ($module['share']) {
            // 共享模块单独删除
            foreach ($cats as $t) {
                $mod = \Soulcms\Service::L('cache')->get('module-'.SITE_ID.'-content', $t['mid']);
               if ($mod && $t['mid']) {
                   // 删除栏目模型字段
                   ...
                   if ($mod['form']) {
                       foreach ($mod['form'] as $form) {
                           $ftable = mys_module_table_prefix($t['mid']).'_form_'.$form['table'];
            ...
            if ($module['form']) {
                foreach ($module['form'] as $form) {
                    $ftable = mys_module_table_prefix(APP_DIR).'_form_'.$form['table'];
        ...
        foreach ($this->_tree as $t) {
            if ($t['pid'] == $pid) {
                if ($t['child']) {
                    ...
                    $url = '';
                    if ($t['tid'] == 1) {
                        $url = mys_url($t['mid'].'/home/index', ['catid' => $t['id']]);
                    } elseif ($t['tid'] == 2) {
                        $url = mys_url('category/edit', ['id' => $t['id']]);
                    } else {
                        $url = mys_url('category/edit', ['id' => $t['id']]);
                    }
