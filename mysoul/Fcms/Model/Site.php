...
class Site extends \Soulcms\Model
{
    ...
    public function config($siteid, $name = '', $data = []) {
        ...
        $site['setting'] = mys_string2array($site['setting']);
        ...
        if ($name && $data) {
            ...
            $this->table('site')->update($siteid, [
                'name' => $site['name'],
                'domain' => $site['domain'],
                'setting' => mys_array2string($site['setting']),
            ]);
        }
    ...
    public function create($data) {
        ...
        \Soulcms\Service::M('Table')->create_site($siteid);
        ...
        \Soulcms\Service::M('cache')->update_webpath('Web', $data['webpath'], [
            'SITE_ID' => $siteid
        ]);
    }
    ...
    public function domain($value = []) {
        ...
        // 用户中心域名
        /*
        if (mys_is_app('member')) {
            $member = $this->db->table('member_setting')->where('name', 'domain')->get()->getRowArray();
            $member && $member['value'] = mys_string2array($member['value']);
            if ($value) {
                ...
                $this->db->table('member_setting')->replace([
                    'name' => 'domain',
                    'value' => mys_array2string($member['value'])
                ]);
            }
        ...
        foreach ($module as $t) {
            ...
            $my[$t['dirname']] = [
                'share' => $t['share'],
                'name' => mys_lang($cfg['name']),
                'error' => '',
            ];
            if ($t['share']) {
                $my[$t['dirname']]['error'] = mys_lang('共享模块不支持绑定');
                continue;
            }
            if ($t['site'][SITE_ID]) {
                if ($value) {
                ...
                $my[$t['dirname']]['error'] = mys_lang('当前站点未安装');
            }
        }
    ...
    public function cache($siteid = null, $data = null, $module = null) {
        ...
        if ($data) {
            foreach ($data as $t) {
                ...
                $logo = mys_get_file($t['setting']['config']['logo']);
                $config[$t['id']] = [
                    'SITE_NAME' => $t['name'],
                ...
                if ($t['id'] > 1 && $t['setting']['webpath']) {
                    $webpath[$t['id']]['site'] = mys_get_dir_path($t['setting']['webpath']);
            ...
            // 用户中心域名
            $member = $this->db->table('member_setting')->where('name', 'domain')->get()->getRowArray();
            $value = $member ? mys_string2array($member['value']) : [];
            if ($value) {
            ...
            if ($module) {
                foreach ($module as $t) {
                    ...
                    foreach ($cache as $_sid => $v) {
                        \Soulcms\Service::L('cache')->set_file('module-'.$_sid.'-'.$t['dirname'], []);
                        \Soulcms\Service::L('cache')->del_file('module-'.$_sid.'-'.$t['dirname']);
                    }
                    ...
                    foreach ($t['site'] as $sid => $v) {
                        ...
                        if ($v['domain']) {
                            ...
                            // 网站路径
                            if ($v['webpath']) {
                                $webpath[$sid][$t['dirname']] = mys_get_dir_path($v['webpath']);
                            }
                        }
                        if ($v['mobile_domain']) {
        ...
        \Soulcms\Service::L('Cache')->set_file('site', $cache);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/site.php', '站点配置文件', 32)->to_require($config);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/domain_sso.php', '同步域名配置文件', 32)->to_require_one($sso_domain);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/domain_app.php', '项目域名配置文件', 32)->to_require_one($app_domain);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/domain_site.php', '站点域名配置文件', 32)->to_require_one($site_domain);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/domain_client.php', '客户端域名配置文件', 32)->to_require_one($client_domain);
        \Soulcms\Service::L('Config')->file(WRITEPATH.'config/webpath.php', '入口文件目录配置文件', 32)->to_require($webpath);
    }
}