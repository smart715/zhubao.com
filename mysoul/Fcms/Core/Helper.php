...
// ftable字段输出
function mys_get_ftable($id, $value, $class = '') {

    $field = \Soulcms\Service::C()->get_cache('table-field', $id);
    if (!$field) {
...
function mys_is_double_search($param, $value) {
...
// 获取多重选择是的参数值
function mys_get_double_search($param, $value) {
...
// 获取内容中的缩略图
function mys_get_content_img($value, $num = 0) {
 ...
 * 插件是否被安装
 */
function mys_is_app($dir) {
    return is_file(mys_get_app_dir($dir).'/install.lock');
}

/**
 * 模块是否被安装
 */
function mys_is_module($dir, $siteid = SITE_ID) {
    return \Soulcms\Service::L('cache')->get('module-'.$siteid, $dir) ? 1 : 0;
}

/**
 * 字符串替换函数
 */
function mys_rp($str, $o, $t) {
 ...
 * 二维码调用
 */
function mys_qrcode($text, $thumb = '', $level = 'H', $size = 5) {
 ...
 * 内容文章显示内链
 */
function mys_content_link($tags, $content, $num = 0) {
...
// 内容加内链
function mys_neilian($content, $blank = 1, $num = 1) {
    if (!$content) return '';

    $tags = \Soulcms\Service::L('cache')->get('tag-'.SITE_ID);
    if ($tags) {
        foreach ($tags as $t) {
            $data = explode(',', $t['tags']);
            if ($data) {
...
// 星级显示
function mys_star_level($num, $shifen = 0) {
 ...
 * 301域名定位
 */
function mys_domain_301($domain, $uri = '') {
    ...
    if (CI_DEBUG) {
        \Soulcms\Service::C()->_admin_msg(1, '正在做自动识别终端（关闭开发者模式时即可自动跳转）', $url);exit;
    }

    mys_redirect($url, 'auto', 301);exit;
}

/**
 * 获取cms域名部分
 */
function mys_cms_domain_name($url) {
...
function mys_lang(...$param) {
    ...
    // 调用语言包内容
    $string = \Soulcms\Service::L('lang')->text($string);

    return $param ? vsprintf($string, $param) : $string;
}

// 判断用户中心菜单权限
function mys_member_menu_show($t) {
    ...
    if ($t['mark']) {
        ...
        switch ($a) {
            case 'module':
                // 判断模块当前站点是否可用
                if (!\Soulcms\Service::C()->get_cache('module-'.SITE_ID.'-'.$b)) {
                    return 0;
                }
                break;
        }
    ...
    // 判断用户组显示权限
    if ($is_site && (!$t['group'] || array_intersect(\Soulcms\Service::C()->member['groupid'], $t['group']))) {
        return 1;
    }
...
// 获取url在导航的id
function mys_navigator_id($type, $markid) {
    return (int)\Soulcms\Service::L('cache')->get('navigator-'.SITE_ID.'-url', $type, $markid);
}

// 获取栏目数据及自定义字段
function mys_cat_value(...$get) {
    ...
    return call_user_func_array(array(\Soulcms\Service::C(), 'get_cache'), $param);
}


// 获取栏目数据及自定义字段
function mys_page_value($id, $field, $site = SITE_ID) {

    if (empty($id)) {
        return '';
    }

    return \Soulcms\Service::C()->get_cache('page-'.$site, 'data', $id, $field);
}

// 获取共享栏目数据及自定义字段
function mys_share_cat_value($id, $field='') {
...
// 获取域名部分
function mys_get_domain_name($url) {
...
// 分割数组
function mys_save_bfb_data($data) {

    $cache = [];
    $count = mys_count($data);
    if ($count > 100) {
 ...
 * 提取关键字
 */
function mys_get_keywords($kw, $siteid = SITE_ID) {
    ...
    //tag数据
    $tags = \Soulcms\Service::L('cache')->get('tag-'.$siteid);
    if ($tags) {
        foreach ($tags as $t) {
...
// 会员头像路径和url
function mys_avatar_path() {

    $config = \Soulcms\Service::C()->get_cache('site', SITE_ID, 'image');
 ...
 * @return	string
 */
function mys_member_info($uid, $name = '', $cache = -1) {

    $data = \Soulcms\Service::L('cache')->get_data('member-info-'.$uid);
    if (!$data) {
        $data = \Soulcms\Service::M('member')->get_member($uid);
        SYS_CACHE && \Soulcms\Service::L('cache')->set_data('member-info-'.$uid, $data, $cache > 0 ? $cache : SYS_CACHE_SHOW * 3600);
    }

    return $name ? $data[$name] : $data;
}

function mys_member_username_info($username, $name = '', $cache = -1) {

    $data = \Soulcms\Service::L('cache')->get_data('member-info-name-'.$username);
    if (!$data) {
        $data = \Soulcms\Service::M('member')->get_member(0, $username);
        SYS_CACHE && \Soulcms\Service::L('cache')->set_data('member-info-name-'.$username, $data, $cache > 0 ? $cache : SYS_CACHE_SHOW * 3600);
    }
 ...
 * @return
 */
function mys_member_invite($uid, $name = 'uid') {

    $data = \Soulcms\Service::M()->db->where('rid', $uid)->get('member_invite')->row_array();
 ...
 * 执行函数
 */
function mys_list_function($func, $value, $param = [], $data = []) {

    if (!$func) {
        return $value;
    }

    $obj = \Soulcms\Service::L('Function_list');
 ...
 * @return	string
 */
function mys_catpos($catid, $symbol = ' > ', $url = true, $html= '', $dirname = MOD_DIR, $url_call_func = '') {

    if (!$catid) {
        return '';
    }

    $cat = \Soulcms\Service::L('cache')->get('module-'.SITE_ID.'-'.(!$dirname || $dirname == 'MOD_DIR' ? 'share' : $dirname), 'category');
    if (!isset($cat[$catid])) {
        return '';
    }
 ...
 * @return	string
 */
function mys_linkagepos($code, $id, $symbol = ' > ', $url = '', $html = '') {
    ...
    $link = \Soulcms\Service::L('cache')->get('linkage-'.SITE_ID.'-'.$code);
    $cids = \Soulcms\Service::L('cache')->get('linkage-'.SITE_ID.'-'.$code.'-id');
    if (is_numeric($id)) {
        // id 查询
        $id = $cids[$id];
        $data = $link[$id];
 ...
 * @return	array
 */
function mys_linkage($code, $id, $level = 0, $name = '') {
 ...
 * @return	string
 */
function mys_page_catpos($id, $symbol = ' > ', $html = '') {

    if (!$id) {
        return '';
    }

    $page = \Soulcms\Service::C()->get_cache('page-'.SITE_ID, 'data');
    if (!isset($page[$id])) {
        return '';
    }
 ...
 * title    支付说明
 * */
function mys_payform($mark, $value = 0, $title = '', $url = '',  $remove_div  = 1) {
    return \Soulcms\Service::M('Pay')->payform($mark, $value, $title, $url, $remove_div);
}
 ...
 * value    默认值
 * */
function mys_fieldform($field, $value = '', $remove_div  = 1) {
...
// 打赏支付
function mys_donation($id, $title = '', $dir = null, $remove_div  = 1) {
    !$dir && $dir = MOD_DIR;
    return \Soulcms\Service::M('Pay')->payform('donation-'.$dir.'-'.$id.'-'.SITE_ID, 0, $title, '/index.php?s='.$dir.'&c=show&id='.$id, $remove_div);
}

// 是否存在收藏夹中 1收藏了 2没有收藏
function mys_is_favorite($dir, $id, $uid = 0) {

    !$uid && $uid = \Soulcms\Service::C()->uid;
    if (!$uid) {
        return 0;
    } elseif (!$dir) {
        return 0;
    ...
    return \Soulcms\Service::M()->db->table(SITE_ID.'_'.$dir.'_favorite')->where('uid', $uid)->where('cid', $id)->countAllResults();
}

// 获取字段表单框
function mys_field_form($field, $value = '', $app = '', $remove_div  = 1) {
 ...
 * @return	array
 */
function mys_block($id, $type = 0, $site = 0) {
    return \Soulcms\Service::C()->get_cache('block-'.($site ? $site : SITE_ID), $id, $type);
}
 ...
 * @return
 */
function mys_var_value($name) {
    return  \Soulcms\Service::C()->get_cache('var', $name);
}

// 获取自定义目录
function mys_get_dir_path($path) {
...
// 缩略图
function mys_thumb($img, $width = 0, $height = 0, $water = 0) {


    if (is_numeric($img)) {
        list($cache_path, $cache_url) = mys_thumb_path();
        ...
        return \Soulcms\Service::L('image')->thumb($img, $width, $height, $water);
    }
...
function mys_get_file($id) {
    ...
    if (is_numeric($id)) {
        // 表示附件id
        $info = \Soulcms\Service::C()->get_attachment($id);
        if ($info['url']) {
            return $info['url'];
        }
    }
 ...
 * @return  string
 */
function mys_get_file_url($data) {

    if (!$data) {
        return '文件信息不存在';
    } elseif ($data['remote'] && ($info = \Soulcms\Service::C()->get_cache('attachment', $data['remote']))) {
 ...
 * @return	array
 */
function mys_field_options($id) {

    if (!$id) {
        return NULL;
    }

    $data = \Soulcms\Service::L('cache')->get_data('field-info-'.$id);
    if (!$data) {
        $field = \Soulcms\Service::C()->get_cache('table-field', $id);
        if (!$field) {
            return NULL;
        }
        $data = mys_format_option_array($field['setting']['option']['options']);
        if (!$data) {
            return NULL;
        }
        // 存储缓存
        \Soulcms\Service::L('cache')->set_data('field-info-'.$id, $data, 10000);
    }
 ...
 * return 1有权限 0无权限
 */
function mys_member_auth($my, $auth) {
 ...
 * 用于用户权限取取反值
 */
function mys_member_auth_id($authid, $postid) {
    ...
    if (!$authid) {
        ...
        return $authid;
    } elseif (mys_count($authid) == mys_count($postid)) {
...
// 微信端的错误码转中文解释
function mys_weixin_error_msg($code) {
    $msg = array (
        '-1' => '系统繁忙，此时请开发者稍候再试',
        '0' => '请求成功',
...
function wx_get_https_json_data($url) {
    ...
    if (!$url) {
        return mys_return_data(0, 'url为空');
    }
    ...
    if (!$response) {
        ...
        if ($error=curl_error($ch)){
            return mys_return_data(0, $error);
        }
        curl_close($ch);
    }
    ...
    if (!$data) {
        return mys_return_data(0, $response.' 不是一个有效的json数据');
    } elseif (isset($data['errcode']) && $data['errcode']) {
        return mys_return_data(0, '错误代码（'.$data['errcode'].'）：'.$data['errmsg']);
    }
...
function wx_post_https_json_data($url, $param = []) {
    ...
    $data = json_decode($response, true);
    if (!$data) {
        return mys_return_data(0, $response);
    } elseif (isset($data['errcode']) && $data['errcode']) {
        return mys_return_data(0, '错误代码（'.mys_weixin_error_msg($data['errcode']).'）：'.$data['errmsg']);
    }
 ...
 * 获取折扣价格值
 */
function mys_zhe_price($value, $zhe) {
 ...
 * 获取价格值
 */
function mys_price_value($value, $num = 2) {
 ...
 * sku 价格信息
 */
function mys_sku_price($value, $number = 2, $join = ' - ', $zhe = 0) {
    ...
    $max = max($price);
    if ($zhe) {
        $min = mys_zhe_price($min, $zhe);
        $max = mys_zhe_price($max, $zhe);
    }
 ...
 * sku获取名称
 */
function mys_sku_name($key, $data, $type = 0) {
 ...
 * 下一个升级值
 */
function mys_level_next_value($array, $id) {
...
// 提醒图标
function mys_notice_icon($type, $c = '') {
 ...
 * 付款方式显示
 */
function mys_pay_type_html($name) {
    return mys_pay_name($name);
}

/**
 * 付款名称
 */
function mys_pay_name($name) {
    return \Soulcms\Service::M('Pay')->payname($name);
}

/**
 * 付款方式的名称
 */
function mys_pay_type($name) {
    return mys_clearhtml(\Soulcms\Service::M('Pay')->paytype($name));
}

/**
 * 付款状态的名称
 */
function mys_pay_status($data) {
    return mys_clearhtml(\Soulcms\Service::M('Pay')->paystatus($data));
}

/**
 * 付款金额显示
 */
function mys_pay_money_html($data, $v = 2) {
 ...
 * 清除空白字符
 */
function mys_clear_empty($value) {
 ...
 * 列表字段进行排序筛选
 */
function mys_list_field_order($field) {
 ...
 * 格式化搜索关键词参数
 */
function mys_get_keyword($s) {
    return mys_safe_replace(str_replace(array('+', ' '), '%', urldecode($s)));
}

/**
 * 两数组追加合并
 */
function mys_array2array($a1, $a2) {
 ...
 * 两数组覆盖合并，1是老数据，2是新数据
 */
function mys_array22array($a1, $a2) {
 ...
 * 模块表前缀
 */
function mys_module_table_prefix($dir, $siteid = SITE_ID) {
    return $siteid.'_'.$dir;;
}
 ...
 * 根据文件扩展名获取文件预览信息
 */
function mys_file_preview_html($value, $target = 0) {

    $ext = trim(strtolower(strrchr($value, '.')), '.');
    if (in_array($ext, ['jpg', 'gif', 'png', 'jpeg'])) {
        $url = $target ? $value.'" target="_blank' : 'javascript:mys_preview_image(\''.$value.'\');';
        ...
        $url = $target ? $value.'" target="_blank' : 'javascript:mys_preview_url(\''.mys_file($value).'\');';
        return '<a href="'.$url.'"><img src="'.$file.'"></a>';
    } elseif (strpos($value, 'http://') === 0) {
        $file = ROOT_THEME_PATH.'assets/images/ext/url.png';
        $url = $target ? $value.'" target="_blank' : 'javascript:mys_preview_url(\''.$value.'\');';
        return '<a href="'.$url.'"><img src="'.$file.'"></a>';
    } else {
        return $value;
    }
 ...
 * @return	string
 */
function mys_get_page_pname($id, $symbol = '_', $page) {
 ...
 * 生成静态文件的错误信息
 */
function mys_to_html_file_error($url, $file, $msg) {
    $error = '**************************************************************************'.PHP_EOL;
    $error.= $msg.''.PHP_EOL;
 ...
 * 生成静态文件名
 */
function mys_to_html_file($url, $root = WEBPATH) {
    ...
    return mys_format_html_file($url, $root);
}
 ...
 */
function mys_format_option_array($value) {
 ...
 * @return  intval
 */
function mys_field_input($name, $type, $option, $value = NULL, $id = 0) {
    return '';
}
 ...
 * @return	array
 */
function mys_dir_map($source_dir, $directory_depth = 0, $hidden = FALSE) {
    ...
    if ($fp = @opendir($source_dir)) {
        ...
        while (FALSE !== ($file = readdir($fp))) {
            ...
            if (($directory_depth < 1 OR $new_depth > 0)
                ...
                $filedata[$file] = mys_dir_map($source_dir.DIRECTORY_SEPARATOR.$file, $new_depth, $hidden);
            } else {
                $filedata[] = $file;
            }
        }
 ...
 * @return	array
 */
function mys_file_map($source_dir) {
 ...
 * 数据返回统一格式
 */
function mys_return_data($code, $msg = '', $data = []) {
    return array(
        'code' => $code,
        'msg' => $msg,
        'data' => $data,
 ...
 * 搜索表单隐藏域
 */
function mys_form_search_hidden($p = []) {
 ...
 * @return	string
 */
function mys_base64_encode($string) {
 ...
 * @return	string
 */
function mys_base64_decode($string) {
 ...
 * @return	string|NULL
 */
function mys_get_theme() {

    if (!function_exists('mys_dir_map')) {
        return ['default'];
    }

    return array_diff(mys_dir_map(ROOTPATH.'static/', 1), ['assets', 'space']);
}
...
function mys_dir_delete($path, $del_dir = FALSE, $htdocs = FALSE, $_level = 0)
{
    ...
    while (FALSE !== ($filename = @readdir($current_dir)))
    {
        if ($filename !== '.' && $filename !== '..')
        {
            ...
            {
                mys_dir_delete($filepath, $del_dir, $htdocs, $_level + 1);
            } else {
                @unlink($filepath);
            }
        }
 ...
 * discuz加密/解密
 */
function mys_authcode($string, $operation = 'DECODE') {

    if (!$string) {
        return '';
    }

    is_array($string) && $string = mys_array2string($string);
 ...
 * 验证码图片获取
 */
function mys_code($width, $height, $url = '') {
 ...
 * 百度地图定位浏览器坐标
 */
function mys_baidu_position_js($ak = SYS_BDMAP_API) {
	$code = \Soulcms\Service::V()->load_js((strpos(FC_NOW_URL, 'https') === 0 ? 'https' : 'http').'://api.map.baidu.com/api?v=2.0&ak='.$ak);
 ...
 */
function mys_baidu_map_form_hidden($field, $ak = SYS_BDMAP_API) {
	$code = \Soulcms\Service::V()->load_js((strpos(FC_NOW_URL, 'https') === 0 ? 'https' : 'http').'://api.map.baidu.com/api?v=2.0&ak='.$ak);
	$code.= '<input type="hidden" id="mys_'.$field.'" name="data['.$field.']" value=""><script type="text/javascript">';
	$code.= '<script type="text/javascript">';
	$code.= '// 百度地图定位坐标
   var geolocation = new BMap.Geolocation();
   geolocation.getCurrentPosition(function(r){
      if(this.getStatus() == BMAP_STATUS_SUCCESS){
		 $("#mys_'.$field.'").val(r.point.lng+\',\'+r.point.lat);
         $.ajax({type: "GET", url: "/index.php?s=api&c=api&m=baidu_position&value="+r.point.lng+\',\'+r.point.lat, dataType:"jsonp"});
      } else {
         alert(\'定位失败：\'+this.getStatus());
      }
 ...
 * 百度地图JS
 */
function mys_baidu_map_js($ak = SYS_BDMAP_API) {
	return \Soulcms\Service::V()->load_js((strpos(FC_NOW_URL, 'https') === 0 ? 'https' : 'http').'://api.map.baidu.com/api?v=2.0&ak='.$ak);
}

/**
 * 百度地图调用
 */
function mys_baidu_map($value, $zoom = 15, $width = 600, $height = 400, $ak = SYS_BDMAP_API, $class= '') {

    if (!$value) {
        return '没有坐标值';
    }

    $id = 'mys_map_'.rand(0, 99);
    $width = $width ? $width : '100%';
    list($lngX, $latY) = explode(',', $value);

    $js = \Soulcms\Service::V()->load_js((strpos(FC_NOW_URL, 'https') === 0 ? 'https' : 'http').'://api.map.baidu.com/api?v=2.0&ak='.$ak);
 ...
 * 腾讯地图调用
 */
function mys_qq_map($value, $zoom = 10, $width = 600, $height = 400, $ui = 0, $class = '') {
    ...
    $ui = !$ui ? 'false' : 'true';
    $id = 'mys_qq_map_'.rand(0, 99);
    $width = $width ? $width : '100%';
    list($lngX, $latY) = explode(',', $value);
    $js = \Soulcms\Service::V()->load_js('http://map.qq.com/api/js?v=2.exp');
    return $js.'<div class="'.$class.'" id="' . $id . '" style="width:' . $width . 'px; height:' . $height . 'px; overflow:hidden"></div>
	<script type="text/javascript">
 ...
 * @return	string
 */
function mys_show_stars($num, $starthreshold = 4) {
 ...
 * @return	string
 */
function mys_module_comment($dir, $id) {
    $url = "/index.php?s=".$dir."&c=comment&m=index&id={$id}";
    return "<div id=\"mys_module_comment_{$id}\"></div><script type=\"text/javascript\">
	function mys_ajax_module_comment_{$id}(type, page) {
		var index = layer.load(2, { time: 10000 });
	    $.ajax({type: \"GET\", url: \"{$url}&type=\"+type+\"&page=\"+page+\"&\"+Math.random(), dataType:\"jsonp\",
            success: function (data) {
            	layer.close(index);
            	if (data.code) {
					$(\"#mys_module_comment_{$id}\").html(data.msg);
				} else {
					mys_tips(0, data.msg);
				}
            },
	...
	mys_ajax_module_comment_{$id}(0, 1);
	</script>";
}
 ...
 * 动态调用模板
 */
function mys_ajax_template($id, $filename) {
 ...
 * @return	string
 */
function mys_show_hits($id, $dom = "") {
    $is = $dom;
    !$dom && $dom = "mys_show_hits_{$id}";
 ...
 * @return	string
 */
function mys_show_module_total($name, $id, $dom) {
    return "<script type=\"text/javascript\">
		$.ajax({
			type: \"GET\",
 ...
 * @return	string
 */
function mys_search_rewrite_encode($params, $search) {
    ...
    if ($search['param_rule']) {
        ...
        // 自由组合
        return mys_rewrite_encode($params, $join, $search['param_field']);
    }
}
 ...
 * @return	array
 */
function mys_search_rewrite_decode($params, $search) {
    ...
    if ($search['param_rule']) {
        ...
        // 自由组合
        return mys_rewrite_decode($params, $join, $search['param_field']);
    }
}
 ...
 * @return	string
 */
function mys_rewrite_encode($params, $join = '-', $field = []) {
    ...
    !$join && $join = '-';
    $field = array_flip(mys_format_option_array($field));
    $url = '';
 ...
 * @return	array
 */
function mys_rewrite_decode($params, $join = '-', $field = []) {

    if (!$params) {
        return [];
    }

    $field = mys_format_option_array($field);
    !$join && $join = '-';
 ...
 * 安全过滤函数
 */
function mys_safe_replace($string, $diy = null) {

    $replace = ['%20', '%27', '%2527', '*', "'", '"', ';', '<', '>', "{", '}'];
    $diy && is_array($diy) && $replace = mys_array2array($replace, $diy);
 ...
 * 安全过滤文件及目录名称函数
 */
function mys_safe_filename($string) {
    return str_replace(
        ['..', "/", '\\', ' ', '<', '>', "{", '}', ';', '[', ']', '\'', '"'],
        '',
        $string
 ...
 * 安全过滤用户名函数
 */
function mys_safe_username($string) {
    return str_replace(
        ['..', "/", '\\', ' ', "#",'\'', '"'],
        '',
        $string
 ...
 * 安全过滤密码函数
 */
function mys_safe_password($string) {
 ...
 * 将路径进行安全转换变量模式
 */
function mys_safe_replace_path($path) {
    return str_replace(
        [
            WRITEPATH,
            WEBPATH,
 ...
 * @return  string
 */
function mys_strcut($string, $length = 100, $dot = '...') {
 ...
 * @return	string
 */
function mys_fdate($sTime, $formt = 'Y-m-d') {
 ...
 * @return	string
 */
function mys_date($time = NULL, $format = SITE_TIME_FORMAT, $color = NULL) {
 ...
 * @return	array
 */
function mys_object2array($obj) {
    $_arr = is_object($obj) ? get_object_vars($obj) : $obj;
    if ($_arr && is_array($_arr)) {
        foreach ($_arr as $key => $val) {
            $val = (is_array($val) || is_object($val)) ? mys_object2array($val) : $val;
            $arr[$key] = $val;
        }
    }
    return $arr;
 ...
 * @return	array
 */
function mys_string2array($data) {
 ...
 * @return	string
 */
function mys_array2string($data) {
 ...
 * @return	bool|void
 */
function mys_mkdirs($dir, $null = true) {
 ...
 * @return	string
 */
function mys_format_file_size($fileSize, $round = 2) {
 ...
 * @return	string
 */
function mys_keyword_highlight($string, $keyword) {
...
// 替换互动表情内容
function mys_replace_emotion($content) {
...
// 两数折扣
function mys_discount($price, $nowprice) {
 ...
 *  @param float $lng 经度值
 */
function mys_distance($lat1, $lng1, $lat2, $lng2, $mark = '米,千米') {
 ...
 */
function mys_square_point($lng, $lat, $distance = 0.5){
...
// 格式化生成文件
function mys_format_html_file($file, $root = WEBPATH) {
...
// 删除静态文件
function mys_delete_html_file($url, $root = WEBPATH) {

    $file = mys_format_html_file($url, $root);
    if (is_file($file)) {
        unlink($file);
    }
}
...
// 获取当前模板目录
function mys_tpl_path($is_member = IS_MEMBER) {

    $tpl = mys_get_app_tpl(APP_DIR && APP_DIR != 'member' ? APP_DIR : '');
    $path = $is_member ? $tpl.(\Soulcms\Service::IS_MOBILE() && is_dir($tpl.'mobile/'.SITE_TEMPLATE.'/') ? 'mobile' : 'pc').'/'.SITE_TEMPLATE.'/member/' : $tpl.(\Soulcms\Service::IS_MOBILE() && is_dir($tpl.'mobile/'.SITE_TEMPLATE.'/') ? 'mobile' : 'pc').'/'.SITE_TEMPLATE.'/home/';
...
// 获取网站表单发布页面需要的变量值
function mys_get_form_post_value($table) {

    $rt = [
        'form' => mys_form_hidden(),
    ];
    $form = \Soulcms\Service::L('cache')->get('form-'.SITE_ID, $table);
    if (!$form) {
        return $rt;
    }
    ...
    // 是否有验证码
    $rt['is_post_code'] = mys_member_auth(
        \Soulcms\Service::C()->member_authid,
        \Soulcms\Service::C()->member_cache['auth_site'][SITE_ID]['form'][$form['table']]['code']
    );
    $rt['rt_url'] =  $form['setting']['rt_url'] ? $form['setting']['rt_url'] : mys_now_url();
    ...
    $rt['myfield'] = \Soulcms\Service::L('Field')->toform(0, $my_field, []);
    $rt['sysfield'] = \Soulcms\Service::L('Field')->toform(0, $sys_field, []);
    $rt['diyfield'] = \Soulcms\Service::L('Field')->toform(0, $diy_field, []);

    $rt['post_url'] = mys_url('form/'.$table.'/post');

    return $rt;
}

// 获取模块表单发布页面需要的变量值
function mys_get_mform_post_value($mid, $table, $cid) {
    ...
    $module = \Soulcms\Service::L('cache')->get('module-'.SITE_ID.'-'.$mid);
    if (!$module) {
        return $rt;
    }
    ...
    $rt['is_post_code'] = mys_member_auth(
        \Soulcms\Service::C()->member_authid,
        \Soulcms\Service::C()->member_cache['auth_module'][SITE_ID][$mid]['form'][$form['table']]['code']
    );
    $rt['rt_url'] =  $form['setting']['rt_url'] ? $form['setting']['rt_url'] : mys_now_url();
    ...
    $rt['post_url'] = mys_url($mid.'/'.$table.'/post', ['cid' => $cid]);

    return $rt;
}

// 获取当前模板文件路径
function mys_tpl_file($file) {
    return mys_tpl_path().$file;
}

// 兼容统计
function mys_count($array_or_countable, $mode = COUNT_NORMAL){
    return is_array($array_or_countable) || is_object($array_or_countable) ? count($array_or_countable, $mode) : 0;
}

// http模式
function mys_http_prefix($url) {
    return (defined('SYS_HTTPS') && SYS_HTTPS ? 'https://' : 'http://').$url;
}

// 补全url
function mys_url_prefix($url, $domain = '', $siteid = SITE_ID, $is_mobile = '') {

    if ($url && strpos($url, 'http') === 0) {
        return $url;
    }

    strlen($is_mobile) == 0 && $is_mobile = \Soulcms\Service::IS_MOBILE();
    ...
    if ($domain && strpos($url, 'http') === false) {
        if (is_dir(APPSPATH.ucfirst($domain))) {
            $mod = \Soulcms\Service::L('cache')->get('module-'.$siteid.'-'.$domain);
            $domain = $mod && $mod['domain'] ? (\Soulcms\Service::IS_MOBILE() && $mod['mobile_domain'] ? $mod['mobile_domain'] : $mod['domain']) : '';
        }
        // 域名是不是http开通
    ...
    if (!$domain) {
        if (defined('IS_CLIENT')) {
            // 来自客户端
            $domain = CLIENT_URL;
        } elseif ($siteid > 1 && \Soulcms\Service::C()->site_info[$siteid]['SITE_URL']) {
            // 存在多站点时
            $domain = $is_mobile ? \Soulcms\Service::C()->site_info[$siteid]['SITE_MURL'] : \Soulcms\Service::C()->site_info[$siteid]['SITE_URL'];
        } else {
...
// 计算用户组到期时间
function mys_member_group_etime($days) {
 ...
 * @return type
 */
function mys_clear_emoji($str){
    preg_match_all("#(\|ud[0-9a-f]{3})#ie", $str, $match) && $str = str_replace($match[1], '', $str);
    return mys_clear_empty(mys_html2emoji($str));
}

// 判断是否支持回复
function mys_comment_is_reply($reply, $member, $cuid) {
...
// 将同步代码转为数组
function mys_member_sync_url($string) {
 ...
 * 栏目下级或者同级栏目
 */
function mys_related_cat($data, $catid) {
 ...
 * @return	string
 */
function mys_get_cat_pname($mod, $catid, $symbol = '_') {
...
class php5replace {
    ...
    function php55_replace_var($value) {
        $v = '';
        $val = (defined($value[1]) ? $value[1] : \Soulcms\Service::C()->get_cache('site', SITE_ID, 'config', $value[1]));
        @eval('$v = '.($val ? $val : '""').';');
        return $v;
    }
...
// 模块首页地址
function mys_module_url($dir) {

    if ($dir == MOD_DIR) {
        return MODULE_URL;
    }

    return \Soulcms\Service::L('cache')->get('module-'.SITE_ID.'-'.$dir, 'url');
}

/**
 * 获取对应的手机端地址
 */
function mys_mobile_url($url = SITE_MURL) {
...
if (! function_exists('mys_redirect'))
{
    /**
     * 跳转地址
     */
    function mys_redirect($url = '', $method = 'auto', $code = NULL) {
...
if ( ! function_exists('mys_directory_map'))
{
    /**
     ...
     * @return	array
     */
    function mys_directory_map($source_dir, $directory_depth = 0, $hidden = FALSE)
    {
        if ($fp = @opendir($source_dir))
        {
            $filedata	= [];
...
// 判断是否购买// 0 没有购买 1 购买了
if (!function_exists('mys_is_buy')) {

    function mys_is_buy($fid, $id, $uid = 0) {
        ...
        $data = \Soulcms\Service::C()->get_cache('table-pay-'.SITE_ID, $field['relatedname'].'-'.$field['relatedid']);
        if (!$data) {
            return 0;
        }

        !$uid && $uid = (int)\Soulcms\Service::C()->uid;

        // 表名-主键id-字段id*** 模糊匹配
        $mid = $data['table'].'-'.$id.'-'.$field['id'].'%';

        return \Soulcms\Service::M()->db->table('member_paylog')->where('uid', $uid)->where('status', 1)->like('mid', $mid)->countAllResults();
    }
}
...
function mys_url($url, $query = [], $self = SELF) {
    return \Soulcms\Service::L('router')->url($url, $query, $self);
}

function mys_furl($name) {
    return \Soulcms\Service::L('router')->furl($name);
}

function mys_get_tag_url($name, $mid = '') {
    return \Soulcms\Service::L('router')->get_tag_url($name, $mid);
}

function mys_comment_url($id, $moddir = '')  {
    return \Soulcms\Service::L('router')->comment_url($id, $moddir);
}

function mys_form_show_url($table, $id, $page = 0)  {
    return \Soulcms\Service::L('router')->form_show_url($table, $id, $page);
}

function mys_oauth_url($name, $type) {
    return \Soulcms\Service::L('router')->oauth_url($name, $type);
}

function mys_member_url($url, $query = [],  $self = 'index.php') {
    return \Soulcms\Service::L('router')->member_url($url, $query, $self);
}

function mys_search_url($params = [], $name = '', $value = '', $moddir = '') {
    return \Soulcms\Service::L('router')->search_url($params, $name, $value, $moddir);
}
