...
class Service
{
    ...
    public static function C() {
        return class_exists('\Soulcms\Common') ? \Soulcms\Common::get_instance() : null;
    }
    ...
    public static function model() {
        ...
        if (!is_object(static::$model)) {
            static::$model = new \Soulcms\Model();
        }
    ...
    public static function L( $name,  $namespace = '') {
        ...
        if (!isset(static::$instances[$_cname]) or !is_object(static::$instances[$_cname])) {
            ...
            if ($extendFile && is_file($extendFile)) {
                require $extendFile;
                $className = $namespace ? '\\Soulcms\\Library\\'.ucfirst($namespace).'\\'.$className : '\\My\\Library\\'.$className;
            } else {
                $className = '\\Soulcms\\Library\\'.$className;
                // 多个应用引用同一个类名称时的区别
                if ($namespace) {
                    $newClassName2 = '\\Soulcms\\Library\\'.ucfirst($namespace).'\\'.$className;
    ...
    public static function M( $name = '',  $namespace = '') {
        ...
        if (!isset(static::$instances[$_cname]) or !is_object(static::$instances[$_cname])) {
            ...
            if ($extendFile && is_file($extendFile)) {
                require $extendFile;
                $newClassName = $namespace ? '\\Soulcms\\Model\\'.ucfirst($namespace).'\\'.$className : '\\My\\Model\\'.$className;
            } else {
                $newClassName = '\\Soulcms\\Model\\'.$className;
                // 多个应用引用同一个类名称时的区别
                if ($namespace) {
                    $newClassName2 = '\\Soulcms\\Model\\'.ucfirst($namespace).'\\'.$className;
                    if (class_exists($newClassName2)) {
                        static::$instances[$_cname] = new $newClassName2();
                        return static::$instances[$_cname];
                    }
    ...
    public static function H($name, $namespace) {
        ...
        $file = mys_get_app_dir($namespace).'Helpers/'.ucfirst($name).'.php';
        $_cname = md5($file);
        ...
        if (!is_file($file)) {
            defined('IS_API_HTTP') && IS_API_HTTP ? \Soulcms\Common::json(0, '函数文件：'.str_replace(FCPATH, '', $file).'不存在') : exit('函数文件：'.str_replace(FCPATH, '', $file).'不存在');
        }
    ...
    private static function _get_class_file($name, $namespace, $class) {
        ...
        if (!is_file($extendFile) && $namespace) {
            // 当前是app时优先考虑本级继承目录文件
            $extendFile = mys_get_app_dir($namespace).($class == 'Library' ? 'Librarie' : $class ).'s/'.$className.'.php';
        }
        if (!is_file($classFile)) {
            // 相对于APP目录
            if ($namespace) {
                $classFile = mys_get_app_dir($namespace).($class == 'Library' ? 'Librarie' : $class ).'s/'.$className.'.php';
                $extendFile = '';
            ...
            if (!$classFile || !is_file($classFile)) {
                defined('IS_API_HTTP') && IS_API_HTTP ? \Soulcms\Common::json(0, '类文件：'.str_replace(FCPATH, '', $classFile).'不存在') : exit('类文件：'.str_replace(FCPATH, '', $classFile).'不存在');
            }
        }
