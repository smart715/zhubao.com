...
class View {
    ...
    public function init($name = 'pc') {
        ...
        } elseif (IS_API || (APP_DIR && mys_is_app_dir(APP_DIR))) {
            // 插件模块或者api
    ...
    private function get_client_home_path($name, $mid = '') {
        return mys_get_app_tpl($mid).$name.'/'.SITE_TEMPLATE.'/home/'.($mid ? $mid.'/' : '');
    }

    // 终端路径
    private function get_client_member_path($name, $mid = '') {
        return mys_get_app_tpl($mid).$name.'/'.SITE_TEMPLATE.'/member/'.($mid ? $mid.'/' : '');
    }
    ...
    public function display($_name, $_dir = '') {
        ...
        if (!$this->_options['my_web_url']) {
            $this->_options['my_web_url'] = $this->_options['fix_html_now_url'] ? $this->_options['fix_html_now_url'] : mys_now_url();
        }

        if (IS_API_HTTP) {
            // 如果是来自api就不解析模板，直接输出变量
            $call = \Soulcms\Service::L('input')->request('api_call_function');
            if ($call) {
                $call = mys_safe_replace($call);
                if (method_exists(\Soulcms\Service::L('http'), $call)) {
                    \Soulcms\Service::C()->_json(1, 'view', \Soulcms\Service::L('http')->$call($this->_options));
                }
                \Soulcms\Service::C()->_json(0, '回调方法('.$call.')未定义');
            }
            \Soulcms\Service::C()->_json(1, 'view', $this->_options);
        }
    ...
    public function code2php($code, $time = 0, $include = 1) {
        ...
        !is_file($this->_cache.$file) && @file_put_contents($this->_cache.$file, str_replace('$this->', '\Soulcms\Service::V()->', $this->handle_view_file($code)));
    ...
    public function handle_view_file($view_content) {
        ...
        $replace_array = [
            ...
            "<?php echo \\1; ?>",
            "<?php echo \$\\1; ?>",
            "<?php echo \\Soulcms\\Service::L('\\1')->\\2(\\3); ?>",
        ...
        $view_content = str_replace([
            ...
            '\Soulcms\Service::C()->',
            '\Soulcms\Service::IS_PC()',
            '\Soulcms\Service::IS_MOBILE()',
            '\Soulcms\Service::IS_MOBILE2()',
        ], $view_content);
    ...
    public function library_method($class, $method, $param) {
        ...
        return call_user_func_array(
            [\Soulcms\Service::L($class), $method],
            $param
        );
    }
    ...
    public function list_tag($_params) {
        ...
        foreach ($params as $t) {
            ...
            if (isset($system[$var])) { // 系统参数，只能出现一次，不能添加修饰符
                $system[$var] = mys_safe_replace($val);
            } else {
        ...
        switch ($system['action']) {
            ...
            case 'function': //执行函数
                ...
                $name = 'function-'.md5(mys_array2string($param));
                ...
                if (!$cache) {
                    ...
                    \Soulcms\Service::L('cache')->init()->save($name, $cache, $system['cache']);
                }
            ...
            case 'cache': // 系统缓存数据
                ...
                if (!$cache) {
                    ...
                    foreach ($cache as $i => $t) {
                        $cache[$i] = \Soulcms\Service::L('cache')->get('module-'.$system['site'].'-'.$t['dirname']);
                    }
                }
            ...
            case 'category': // 栏目
                ...
                $module = \Soulcms\Service::L('cache')->get('module-'.$system['site'].'-'.$dirname);
                if (!$module) {
                ...
                foreach ($module['category'] as $t) {
                    ...
                    $t['url'] = mys_url_prefix($t['url'], $dirname);
                    $return[] = $t;
                    $i ++;
                }
            ...
            case 'linkage': // 联动菜单
                ...
                $linkage = \Soulcms\Service::L('cache')->get('linkage-'.$system['site'].'-'.$param['code']);
                if (!$linkage) {
                    return $this->_return($system['return'], "联动菜单{$param['code']}不存在，请在后台更新缓存");
                }

                // 通过别名找id
                $ids = @array_flip(\Soulcms\Service::C()->get_cache('linkage-'.$system['site'].'-'.$param['code'].'-id'));
                if (isset($param['pid'])) {
                    if (is_numeric($param['pid'])) {
                        ...
                        !$pid && is_numeric($param['pid']) && \Soulcms\Service::C()->get_cache('linkage-'.$system['site'].'-'.$param['code'].'-id', $param['pid']) && $pid = intval($param['pid']);
                    }
                }
            ...
            case 'category_search_field': // 栏目搜索字段筛选
                ...
                } elseif (mys_count($module['category'][$catid]['field']) == 0) {
                    return $this->_return($system['return'], '模块未安装或者此栏目无模型字段');
                }

                $return = [];
                foreach ($module['category'][$catid]['field'] as $t) {
                    $data = mys_format_option_array($t['setting']['option']['options']);
            ...
            case 'navigator': // 网站导航
                ...
                $navigator = \Soulcms\Service::C()->get_cache('navigator-'.$system['site']); // 导航缓存
                if (!$navigator) {
                ...
                $field = \Soulcms\Service::L('cache')->get('navigator-'.$system['site'].'-field');
                $return = [];
                foreach ($data as $t) {
                    if ($system['num'] && $i >= $system['num']) {
                        break;
                    ...
                    $return[] = \Soulcms\Service::L('Field')->format_value($field, $t, 1);
                    $i ++;
                }
            ...
            case 'page': // 自定义页调用
                ...
                $data = \Soulcms\Service::C()->get_cache('page-'.$system['site'], 'data'); // 单页缓存
                if (!$data) {
                    return $this->_return($system['return'], '没有查询到内容');
                }
                ...
                $i = 0;
                $show = isset($param['show']) ? 1 : 0; // 有show参数表示显示隐藏栏目
                $field = \Soulcms\Service::L('cache')->get('page-'.$system['site'].'-field');
                $return = array();
            ...
            case 'tag': // 调用tag
                ...
                $system['order'] = mys_safe_replace($system['order'] ? ($system['order'] == 'rand' ? 'RAND()' : $system['order']) : 'displayorder asc');

                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_tag'); // tag表
                $where = '';
                if ($param['tag']) {
                    $in = $tag = $sql = [];
                    $array = explode(',', $param['tag']);
                    foreach ($array as $name) {
                        $name && $sql[] = '`name`="'.mys_safe_replace($name).'"';
                    }
                    $sql && $tag = $this->_query("SELECT code FROM {$table} WHERE ".implode(' OR ', $sql), $system['db'], $system['cache']);
                    if ($tag) {
                        $cache = \Soulcms\Service::C()->get_cache('tag-'.$system['site']); // tag缓存
                        foreach ($tag as $t) {
                ...
                $cache = \Soulcms\Service::L('cache')->get_data($name);
                ...
                if (!$cache) {
                    ...
                    $cache = $system['cache'] ? \Soulcms\Service::L('cache')->set_data($name, $data, $system['cache']) : $data;
                }
            ...
            case 'sql': // 直接sql查询
                ...
                if (preg_match('/sql=\'(.+)\'/sU', $_params, $sql)) {
                    ...
                    $sql = str_replace(
                        array('@#S', '@#'),
                        array(\Soulcms\Service::M()->dbprefix.$system['site'], \Soulcms\Service::M()->dbprefix),
                        trim($sql[1])
                    );
            ...
            case 'table': // 表名查询
                ...
                $tableinfo = \Soulcms\Service::L('cache')->get_data('table-'.$system['table']);
                if (!$tableinfo) {
                    $tableinfo = \Soulcms\Service::M('Table')->get_field($system['table']);
                    \Soulcms\Service::L('cache')->set_data('table-'.$system['table'], $tableinfo, 36000);
                }
                if (!$tableinfo) {
                ...
                $table = \Soulcms\Service::M()->dbprefix($system['table']);
                ...
                if ($system['join'] && $system['on']) {
                    $table2 = \Soulcms\Service::M()->dbprefix($system['join']); // 关联表
                    $tableinfo2 = \Soulcms\Service::L('cache')->get_data('table-'.$system['join']);
                    if (!$tableinfo2) {
                        \Soulcms\Service::M('Table')->get_field($system['join']);
                        \Soulcms\Service::L('cache')->set_data('table-'.$system['join'], $tableinfo, 36000);
                    }
                    if (!$tableinfo2) {
            ...
            case 'form': // 表单调用
                ...
                $mid = $system['form'];
                // 表单参数为数字时按id读取
                $form = \Soulcms\Service::C()->get_cache('form-'.$system['site'], $mid);
                // 判断是否存在
                if (!$form) {
                ...
                // 表结构缓存
                $tableinfo = \Soulcms\Service::L('cache')->get('table-'.$system['site']);
                if (!$tableinfo) {
                    // 没有表结构缓存时返回空
                    return $this->_return($system['return'], '表结构缓存不存在');
                }
                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_form_'.$form['table']); // 主表
                ...
                if (!$cache && is_array($data)) {
                    ...
                    $dfield = \Soulcms\Service::L('Field')->app('form');
                    foreach ($data as $i => $t) {
            ...
            case 'mform': // 模块表单调用
                ...
                $mid = $system['form'];
                $form = \Soulcms\Service::L('cache')->get('module-'.$system['site'].'-'.$dirname, 'form', $mid);
                // 判断是否存在
                if (!$form) {
                ...
                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_'.$dirname.'_form_'.$form['table']); // 模块主表
                if (!isset($tableinfo[$table])) {
                    return $this->_return($system['return'], '表（'.$table.'）结构缓存不存在');
                }
                ...
                if (!$cache && is_array($data)) {
                    // 表的系统字段
                    $fields['inputtime'] = array('fieldtype' => 'Date');
                    $dfield = \Soulcms\Service::L('Field')->app($dirname);
                    foreach ($data as $i => $t) {
                        $data[$i] = $dfield->format_value($fields, $t, 1);
                    }
                    $cache = $system['cache'] ? \Soulcms\Service::L('cache')->set_data($name, $data, $system['cache']) : $data;
            ...
            case 'comment': // 模块评论调用
                ...
                $comment = \Soulcms\Service::L('cache')->get('module-'.$system['site'].'-'.$dirname, 'comment');
                // 判断是否存在
                ...
                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_'.$dirname.'_comment'); // 模块主表
                if (!isset($tableinfo[$table])) {
                    return $this->_return($system['return'], '表（'.$table.'）结构缓存不存在');
                }
                ...
                var_dump(\Soulcms\Service::L('cache')->get('module-'.$system['site'].'-'.$dirname, 'comment'));
            ...
            case 'member': // 会员信息
                ...
                $table = \Soulcms\Service::M()->dbprefix('member'); // 主表
                if (!isset($tableinfo[$table])) {
                    return $this->_return($system['return'], '表（'.$table.'）结构缓存不存在');
                }

                $fields = \Soulcms\Service::C()->get_cache('member', 'field');
                ...
                if (isset($param['groupid']) && $param['groupid']) {
                    ...
                    if (strpos($param['groupid'], ',') !== false) {
                        $gwhere = ' `'.$table.'`.`id` in (select uid from `'.$table.'_group_index` where `gid` = in ('.mys_safe_replace($param['groupid']).'))';
                ...
                if ($system['more']) {
                    // 会员附表
                    $more = \Soulcms\Service::M()->dbprefix('member_data'); // 附表
                ...
                if (!$cache && is_array($data)) {
                    ...
                    // 格式化显示自定义字段内容
                    $dfield = \Soulcms\Service::L('Field')->app('member');
                    foreach ($data as $i => $t) {
                        $data[$i] = $dfield->format_value($fields, $t, 1);
                    }
                    $cache = $system['cache'] ? \Soulcms\Service::L('cache')->set_data($name, $data, $system['cache']) : $data;
            ...
            case 'content': // 模块内容详细页面
                ...
                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_'.$module['dirname']); // 模块主表`
                if (!isset($tableinfo[$table])) {
                    return $this->_return($system['return'], '表（'.$table.'）结构缓存不存在');
                }

                // 初始化数据表
                $db = \Soulcms\Service::M('Content', $dirname);
                ...
                if (!$cache && is_array($data)) {
                    ...
                    // 格式化显示自定义字段内容
                    $dfield = \Soulcms\Service::L('Field')->app($module['dirname']);
                    $data['url'] = mys_url_prefix($data['url'], $dirname);
            ...
            case 'related': // 模块的相关文章
                ...
                $table = \Soulcms\Service::M()->dbprefix($system['site'].'_'.$dirname); // 模块主表

                $sql = [];
                $array = explode(',', $param['tag']);
                foreach ($array as $name) {
                    $name && $sql[] = '(`'.$table.'`.`title` LIKE "%'.mys_safe_replace($name).'%" OR `'.$table.'`.`keywords` LIKE "%'.mys_safe_replace($name).'%")';
                }
                $sql = $where[] = [
                    'adj' => 'SQL',
            ...
            case 'module': // 模块数据
                ...
                // 关联表
                if ($system['join'] && $system['on']) {
                    $table_more = \Soulcms\Service::M()->dbprefix($system['join']); // 关联表
                ...
                if ($system['page']) {
                    ...
                    if ($system['catid'] && is_numeric($system['catid'])) {
                        if (!$system['urlrule']) {
                            $system['urlrule'] = \Soulcms\Service::L('router')->category_url($module, $module['category'][$system['catid']], '{page}');
                        }
                        if (!$system['sbpage']) {
                            if ($this->_is_mobile) {
                        ...
                        $first_url = \Soulcms\Service::L('router')->category_url($module, $module['category'][$system['catid']]);
                    }
                    $pagesize = (int)$system['pagesize'];
    ...
    public function _query($sql, $db, $cache, $all = TRUE) {
        ...
        // 缓存存在时读取缓存文件
        $cname = md5($db.$sql.mys_now_url());
        if (SYS_CACHE && $cache && $data = \Soulcms\Service::L('cache')->get_data($cname)) {
            return $data;
        }

        $mysql = \Soulcms\Service::M()->db;
        if ($db) {
        ...
        // 开启缓存时，重新存储缓存数据
        $cache && \Soulcms\Service::L('cache')->set_data($cname, $data, $cache);
    ...
    public function _get_pagination($url, $pagesize, $total, $name = 'page', $first_url = '') {
        ...
        // 这里要支持移动端分页条件
        !$name && $name = 'page';
        $file = 'config/page/'.($this->_is_mobile ? 'mobile' : 'pc').'/'.(mys_safe_filename($name)).'.php';
        ...
        if ($this->_page_config) {
            $config = mys_array22array($config, $this->_page_config);
        }
        ...
        if ($where) {
            ...
            foreach ($where as $i => $t) {
                ...
                switch ($t['adj']) {
                    ...
                    case 'JSON':
                        if (version_compare(\Soulcms\Service::M()->db->getVersion(), '5.7.0') < 0) {
                            // 兼容写法
                            $string.= $join. " {$t['name']}  LIKE \"%\"".\Soulcms\Service::M()->db->escapeString($t['value'], true)."\"%\"";
                        } else {
                            // 高版本写法
                            $string.= $join." JSON_CONTAINS ({$t['name']}->'$[*]', '\"".mys_safe_replace($t['value'])."\"', '$')";
                        }
                        break;

                    case 'FIND':
						$v = mys_safe_replace($t['value']);
						if (!is_numeric($v)) {
							$v = "'".$v."'";
						}
                    ...
                    case 'LIKE':
                        $string.= $join." {$t['name']} LIKE \"".mys_safe_replace($t['value'])."\"";
                        break;

                    case 'IN':
                        $string.= $join." {$t['name']} IN (".mys_safe_replace($t['value']).")";
                        break;

                    case 'NOTIN':
                        $string.= $join." {$t['name']} NOT IN (".mys_safe_replace($t['value']).")";
                        break;

                    case 'NOT':
                        $string.= $join.(is_numeric($t['value']) ? " {$t['name']} <> ".$t['value'] : " {$t['name']} <> \"".($t['value'] == "''" ? '' : mys_safe_replace($t['value']))."\"");
                        break;
        ...
        foreach ($where as $i => $t) {
            if (in_array($t['name'], $field)) {
                ...
                if ($myfield && $myfield[$t['name']]['fieldtype'] == 'Linkage') {
                    // 联动菜单
                    $data = mys_linkage($myfield[$t['name']]['setting']['option']['linkage'], $t['value']);
                    if ($data) {
                        if ($data['child']) {
                            $where[$i]['adj'] = 'IN';
        ...
        foreach ($array as $i => $t) {
            ...
            foreach ($fields as $prefix => $field) {
                if (in_array($a, $field)) {
                    ...
                    if (\Soulcms\Service::C()->my_position) {
                        $my[$i] = $a.' ASC';
                        $this->pos_order = $a;
                    } else {
                        \Soulcms\Service::C()->msg('没有定位到您的坐标');
                    }
                }
            }
    ...
    private function _get_select_field($field) {
        ...
        $this->pos_order && (\Soulcms\Service::C()->my_position ? $field.= ',ROUND(6378.138*2*ASIN(SQRT(POW(SIN(('.\Soulcms\Service::C()->my_position['lat'].'*PI()/180-'.$this->pos_order.'_lat*PI()/180)/2),2)+COS('.\Soulcms\Service::C()->my_position['lat'].'*PI()/180)*COS('.$this->pos_order.'_lat*PI()/180)*POW(SIN(('.\Soulcms\Service::C()->my_position['lng'].'*PI()/180-'.$this->pos_order.'_lng*PI()/180)/2),2)))*1000) AS '.$this->pos_order : \Soulcms\Service::C()->msg('没有定位到您的坐标'));
    ...
    public function _return($return, $data = [], $sql = '', $total = 0, $pages = '', $pagesize = 0) {
        ...
        $total = isset($total) ? $total : mys_count($data);
        $page = max(1, (int)$_GET['page']);
    ...
    public function _cache_var($name, $siteid = 0) {
        ...
        switch ($name) {
            case 'member':
                $data = \Soulcms\Service::C()->member_cache;
                break;
            case 'member_group':
                $data = \Soulcms\Service::C()->member_cache['group'];
                break;
            case 'urlrule':
                $data = \Soulcms\Service::L('cache')->get('urlrule');
                break;
            case 'module-content':
                $data = \Soulcms\Service::L('cache')->get('module-'.$siteid.'-content');
                break;
            case 'category':
                $data = \Soulcms\Service::L('cache')->get('module-'.$siteid.'-'.MOD_DIR, 'category');
                break;
            case 'page':
                $data = \Soulcms\Service::L('cache')->get('page-'.$siteid, 'data');
                break;
            default:
                $data = \Soulcms\Service::L('cache')->get($name.'-'.$siteid);
                break;
        }
