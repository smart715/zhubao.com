...
if (is_file(MYPATH.'Dev.php')) {
    ...
    // 判断是否是app目录
    function mys_is_app_dir($name) {
        return is_dir(APPSPATH.ucfirst($name));
    }
    function mys_get_app_dir($name) {
        return APPSPATH.ucfirst($name).'/';
    }
    function mys_get_app_tpl($name = '') {
        return TPLPATH;
    }
    function mys_get_app_list() {
        return APPSPATH;
    }
    function mys_get_app_css($name) {
...
if (!IS_API && isset($_GET['s']) && preg_match('/^[a-z]+$/i', $_GET['s'])) {
    ...
    if (!IS_ADMIN && $dir == 'Member') {
        // 会员
        if ($_GET['app'] && mys_is_app_dir($_GET['app'])) {
            // 模块应用
            define('APPPATH', mys_get_app_dir($_GET['app']));
    ...
    } elseif (mys_is_app_dir($dir)) {
        // 模块应用
        define('APPPATH', mys_get_app_dir($dir));
...
$app = new \Soulcms\Extend\CodeIgniter(config(\Config\App::class));
$app->initialize();
$app->run();