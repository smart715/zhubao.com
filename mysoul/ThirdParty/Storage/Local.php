<?php namespace Soulcms\ThirdParty\Storage;
...
class Local {
    ...
    public function upload($type = 0, $data, $watermark) {
        ...
        // 目录不存在先创建它
        !is_dir($this->fullpath) && mys_mkdirs($this->fullpath);
        if (!is_dir($this->fullpath)) {
            log_message('error', '目录创建失败：'.$this->fullpath);
            return mys_return_data(0, mys_lang('创建目录%s失败', IS_ADMIN ? $this->fullpath : ''));
        }

        if ($type) {
            // 移动失败
            if (!(move_uploaded_file($this->data, $this->fullname) || !is_file($this->fullname))) {
                return mys_return_data(0, mys_lang('文件移动失败'));
            }
        } else {
            ...
            if (!$filesize || !is_file($this->fullname)) {
                ...
                return mys_return_data(0, mys_lang('文件创建失败'));
            }
        }

        // 强制水印
        if ($this->watermark && ($config = \Soulcms\Service::C()->get_cache('site', SITE_ID, 'watermark'))) {
            $config['source_image'] = $this->fullname;
            $config['dynamic_output'] = false;
            \Soulcms\Service::L('image')->watermark($config);
        }
