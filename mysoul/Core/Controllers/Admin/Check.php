...
class Check extends \Soulcms\Common
{
	...
	public function index() {
        ...
        \Soulcms\Service::V()->assign([
            'menu' => \Soulcms\Service::M('auth')->_admin_menu(
                [
                    '系统体检' => [\Soulcms\Service::L('Router')->class.'/index', 'fa fa-wrench'],
                    '系统更新' => ['cache/index', 'fa fa-refresh'],
                    'PHP环境' => [\Soulcms\Service::L('Router')->class.'/php_index', 'fa fa-code'],
                ]
            ),
            'list' => $this->_list,
        ]);
		\Soulcms\Service::V()->display('check_index.html');
	}
	...
	public function do_index() {
	    ...
	    switch ($id) {
            ...
            case '03':
                ...
                list($thumb_path) = mys_thumb_path();
                list($avatar_path) = mys_avatar_path();
            ...
            case '05':
                ...
                $field = \Soulcms\Service::M()->db->query('SHOW FULL COLUMNS FROM `'.\Soulcms\Service::M()->dbprefix('admin').'`')->getResultArray();
                if (!$field) {
            ...
            case '07':
                ...
                // 模块
                $module = \Soulcms\Service::M()->table('module')->order_by('displayorder ASC,id ASC')->getAll();

                // 站点
                $prefix = \Soulcms\Service::M()->prefix;
                foreach ($this->site as $siteid) {
                    // 升级资料库
                    $table = $prefix.$siteid.'_block';
                    if (\Soulcms\Service::M()->db->tableExists($table)) {
                        // 创建code字段 代码
                        if (!\Soulcms\Service::M()->db->fieldExists('code', $table)) {
                            \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'` ADD `code` VARCHAR(100) NOT NULL');
                        }
                    }
                    if ($module) {
                        foreach ($module as $m) {
                            ...
                            if (\Soulcms\Service::M()->db->tableExists($table)) {
                                // 创建字段 删除理由
                                if (!\Soulcms\Service::M()->db->fieldExists('result', $table)) {
                                    \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'` ADD `result` Text NOT NULL');
                                }
                            }
                        }
                    }
                ...
                $table = $prefix.'cron';
                if (!\Soulcms\Service::M()->db->fieldExists('site', $table)) {
                    \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'` ADD `site` INT(10) NOT NULL COMMENT \'站点\'');
                }
                ...
                if (!\Soulcms\Service::M()->db->fieldExists('username', $table)) {
                    \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'` ADD `username` VARCHAR(100) NOT NULL');
                }
                ...
                if (!\Soulcms\Service::M()->db->fieldExists('site', $table)) {
                    \Soulcms\Service::M()->query('ALTER TABLE `'.$table.'` ADD `site` TEXT NOT NULL');
                }
/*
                $table = $prefix.'email';
                if (!\Soulcms\Service::M()->db->tableExists($table)) {
                    \Soulcms\Service::M()->query('CREATE TABLE IF NOT EXISTS `'.$table.'` (
  ...
  KEY (`displayorder`)
            ...
            case '08':
                // 程序兼容性
                $local = mys_dir_map(APPSPATH, 1); // 搜索本地模块
                foreach ($local as $dir) {
                    if (is_file(APPSPATH.$dir.'/Config/App.php')) {
                        ...
                        if (is_file($file)) {
                            ...
                            if (strpos($code, '\Soulcms\Home\Search') !== false) {
                                file_put_contents($file, str_replace(
                                    ['\Soulcms\Home\Search', '_Module_Search'],
                                    ['\Soulcms\Home\Module', '_Search'],
                                    $code
                                ));
                            }
                        }
            ...
            case '09':
                //
                $local = mys_file_map(WEBPATH, 1); // 搜索根目录
            ...
            case '10':
                ...
                // 模块数据检测
                $rt = [];
                $module = \Soulcms\Service::M()->table('module')->getAll();
                if ($module) {
                    foreach ($module as $m) {
                        $site = mys_string2array($m['site']);
                        $mform = \Soulcms\Service::M()->table('module_form')->where('module', $m['dirname'])->getAll();
                ...
                if ($data) {
                    ...
                    foreach ($data as $name => $domain) {
                        ...
                        if ($name == 'mobile_domain') {
                            if ($domain) {
                                $url = mys_http_prefix($domain) . '/api.php';
                            } else {
                        ...
                        if ($url) {
                            ...
                            if ($code != 'soulcms ok') {
                                $tips[] = '域名绑定异常，无法访问：' . $url . '，可以尝试手动访问此地址，如果提示soulcms ok就表示成功，<a href="'.mys_url('site_domain/index').'">查看详情</a>';
                            }
                        }
                    }
    ...
    private function _check_table_counts($table, $name) {
	    ...
	    $ptable = \Soulcms\Service::M()->dbprefix($table);
	    if (!\Soulcms\Service::M()->db->tableExists($ptable)) {
            return '数据表【'.$name.'/'.$ptable.'】不存在，请创建';
        }
	    $counts = \Soulcms\Service::M()->table($table)->counts();
	    if ($counts > 100000) {
