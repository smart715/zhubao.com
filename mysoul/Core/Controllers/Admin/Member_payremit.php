...
// 汇款管理
class Member_payremit extends \Soulcms\Table
{
    ...
    public function __construct(...$params)
    {
        ...
        \Soulcms\Service::V()->assign([
            'menu' => \Soulcms\Service::M('auth')->_admin_menu(
                [
                    '汇款管理' => [\Soulcms\Service::L('Router')->class.'/index', 'fa fa-cc-visa'],
    ...
    protected function _Save($id = 0, $data = [], $old = [], $before = null, $after = null) {
        ...
        // 更新提醒
        \Soulcms\Service::M('member')->todo_admin_notice('member_payremit/edit:id/'.$id, 0);

        // 通过打款
        if ($post['verify']) {
            \Soulcms\Service::M('member')->notice($old['uid'], 5, '流水号('.$id.') 转账汇款已成功',\Soulcms\Service::L('Router')->member_url('paylog/show', ['id'=>$id]));
            return \Soulcms\Service::M('Pay')->paysuccess('fc-'.$id, $post['note']);
        } else {
            \Soulcms\Service::M('member')->notice($old['uid'], 5, '流水号('.$id.') 转账汇款失败',\Soulcms\Service::L('Router')->member_url('paylog/show', ['id'=>$id]));
