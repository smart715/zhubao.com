...
// 审核流程
class Verify extends \Soulcms\Table
{
    public $role;
    public $type;
    ...
    public function __construct(...$params)
    {
        ...
        \Soulcms\Service::V()->assign([
            'menu' => \Soulcms\Service::M('auth')->_admin_menu(
                [
                    '审核流程' => [\Soulcms\Service::L('Router')->class.'/index', 'fa fa-sort-numeric-asc'],
    ...
    public function index() {
        ...
        $this->_List([], -1);
        \Soulcms\Service::V()->display('verify_index.html');
    }

    // 后台添加url内容
    public function add() {
        $this->_Post(0);
        \Soulcms\Service::V()->display('verify_add.html');
    }
    ...
    public function copy_edit() {
        ...
        $data = \Soulcms\Service::M()->db->table('admin_verify')->where('id', $id)->get()->getRowArray();
        !$data && $this->_josn(0, mys_lang('数据#%s不存在', $id));

        unset($data['id']);
        $data['name'].= '_copy';

        $rt = \Soulcms\Service::M()->table('admin_verify')->insert($data);

        !$rt['code'] && $this->_json(0, mys_lang($rt['msg']));
        \Soulcms\Service::M('cache')->sync_cache('verify');
    ...
    public function show_index() {
        ...
        \Soulcms\Service::V()->assign([
            'value' => mys_string2array($data['verify']),
        ]);
        \Soulcms\Service::V()->display('verify_show.html');exit;
    }
    ...
    protected function _Save($id = 0, $data = [], $old = [], $func = null, $func2 = null) {
        return parent::_Save($id, $data, $old, function($id, $data){
            ...
            $data[1]['verify'] = mys_array2string($value);
    ...
    protected function _Data($id = 0) {
        ...
        $data['value'] = mys_string2array($data['verify']);
        return $data;
    }
