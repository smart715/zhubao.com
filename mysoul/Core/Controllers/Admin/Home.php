...
class Home extends \Soulcms\Common
{
	...
	public function main() {
		...
		\Soulcms\Service::V()->display($table_data ? 'index_main.html' : 'main.html');exit;
	}
	...
	public function edit() {
	    ...
	    if (IS_POST) {
            $data = \Soulcms\Service::L('input')->post('tables');
            \Soulcms\Service::L('Config')->file($file, '后台自定义面板', 32)->to_require($data);
		...
		\Soulcms\Service::V()->display('index_edit.html');exit;
	}

	public function index() {

        $menu_top = $my_menu = [];
		$menu = \Soulcms\Service::L('cache')->get('menu-admin');
        if (!$menu) {
            $m = \Soulcms\Service::M('menu')->cache();
            $menu = $m['admin'];
        }

        // 自定义后台菜单显示
        if (function_exists('mys_my_admin_menu')) {
            $menu = mys_my_admin_menu($menu);
        }
            ...
            foreach ($my_menu as $tid => $top) {
                ...
                foreach ($top['left'] as $if => $left) {
                    ...
                    foreach ($left['link'] as $i => $link) {
                        ...
                        $url = $link['url'] ? $link['url'] :\Soulcms\Service::L('Router')->url($link['uri']);
                        if (!$_link) {
                            // 第一个链接菜单时 指定class
                        ...
                        $link_string.= '<li id="mys_menu_link_'.$link['id'].'" class="'.$class.'"><a href="javascript:Mlink('.$tid.', '.$left['id'].', '.$link['id'].', \''.$url.'\');"><i class="iconm '.$link['icon'].'"></i> <span class="title">'.mys_lang($link['name']).'</span></a></li>';
                        $mlink_string.= '<li id="mys_menu_m_link_'.$link['id'].'" class="'.$class.'"><a href="javascript:Mlink('.$tid.', '.$left['id'].', '.$link['id'].', \''.$url.'\');"><i class="iconm '.$link['icon'].'"></i> <span class="title">'.mys_lang($link['name']).'</span></a></li>';
                    }
                    if (!$link_string) {
                        continue; // 没有链接菜单就不要
                    }
                    $left_string.= '
				<li id="mys_menu_left_'.$left['id'].'" class="mys_menu_'.$tid.' mys_menu_item nav-item '.($_left ? '' : 'active open').' " style="'.($first==$tid ? '' : 'display:none').'">
                    <a href="javascript:;" class="nav-link nav-toggle">
                        <i class="'.$left['icon'].'"></i>
                        <span class="title">'.mys_strcut(mys_lang($left['name']), 5).'</span>
                ...
                $string.= $left_string;
                /*
                $mstring.= '<div class="btn-group pull-left" id="mys_m_top_'.$tid.'" style=" '.($first == $tid ? '' : 'display:none').'">
                    <button type="button" class="btn green btn-sm btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"> '.mys_lang($top['name']).'
                ...
                $mstring.= '<li class="dropdown dropdown-extended dropdown-tasks fc-mb-sum-menu" id="mys_m_top_'.$tid.'" style=" '.($first == $tid ? '' : 'display:none').'">
		...
		\Soulcms\Service::V()->display('index.html');exit;
	}
}
