...
class Check_bom extends \Soulcms\Common
{
    private $phpfile = [];
	...
	public function __construct(...$params) {
		...
		\Soulcms\Service::V()->assign('menu', \Soulcms\Service::M('auth')->_admin_menu(
			[
				'Bom检测' => [\Soulcms\Service::L('Router')->class.'/index', 'fa fa-code'],
			]
		));
	...
	public function index() {
		\Soulcms\Service::V()->display('check_bom_index.html');
	}
	...
	public function php_count_index() {
        ...
        // 存储文件
        \Soulcms\Service::L('cache')->init()->save('check-index', $cache, 3600);
	...
	public function php_check_index() {
        ...
        $cache = \Soulcms\Service::L('cache')->init()->get('check-index');
        ...
        if ($data) {
            ...
            foreach ($data as $filename) {
                ...
                $html.= '<p class="'.$class.'"><label class="rleft">'.mys_safe_replace_path($filename).'</label><label class="rright">'.$ok.'</label></p>';
        ...
        // 完成
        \Soulcms\Service::L('cache')->clear('check-index');
        $this->_json(100, '');
    }
