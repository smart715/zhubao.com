...
class Paylog extends \Soulcms\Table
{
    ...
    public function index() {
        ...
        foreach ($type as $i => $t) {
            ...
            $my[$i] = [
                'name' => mys_lang($t),
                'url' =>\Soulcms\Service::L('Router')->member_url('member/paylog/index', $data['param'])
            ];
        }
        ...
        \Soulcms\Service::V()->display('paylog_index.html');
    }
    
    public function show() {

        $id = \Soulcms\Service::L('input')->get('id');
        strpos($id, '-') !== false && list($a, $id) = explode('-', $id);

        list($a, $data) = $this->_Show((int)$id);
        if (!$data) {
            $this->_msg(0, mys_lang('交易记录不存在'));exit;
        } elseif ($data['uid'] != $this->uid) {
            $this->_msg(0, mys_lang('无权限查看'));exit;
        }
        
        \Soulcms\Service::V()->display('paylog_show.html');
    }
