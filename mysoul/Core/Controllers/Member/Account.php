...
// 账号信息
class Account extends \Soulcms\Common
{
    ...
    public function index() {
        ...
        if ($this->member_cache['field'] && $this->member['groupid']) {
            ...
            foreach ($this->member['groupid'] as $gid) {
                ...
                && $fieldid = mys_array2array($fieldid, $this->member_cache['group'][$gid]['field']);
            }
            if ($fieldid) {
        ...
        if (IS_POST) {
            ...
            if (!$post['name']) {
                $this->_json(0, mys_lang('姓名没有填写'), ['field' => 'name']);
            } elseif (strlen($post['name']) > 20) {
                $this->_json(0, mys_lang('姓名太长了'), ['field' => 'name']);
            }
            list($data, $return, $attach) = \Soulcms\Service::L('form')->validation($post, null, $field, $this->member);
            // 输出错误
            $return && $this->_json(0, $return['error'], ['field' => $return['name']]);
            \Soulcms\Service::M()->table('member')->update($this->uid, [
                'name' => mys_safe_replace($post['name']),
            ]);
            $data[1]['is_complete'] = 1;
            \Soulcms\Service::M()->table('member_data')->update($this->uid, $data[1]);
            // 附件归档
            ...
            $this->_json(1, mys_lang('保存成功'), IS_API_HTTP ? \Soulcms\Service::M('member')->get_member($this->uid) : []);
            exit;
        }

        \Soulcms\Service::V()->assign([
            'form' => mys_form_hidden(),
            'myfield' => \Soulcms\Service::L('field')->toform($this->uid, $field, $this->member),
        ]);
        \Soulcms\Service::V()->display('account_index.html');
    }
    ...
    public function avatar() {
        ...
        if (IS_POST) {
            ...
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/i', $content, $result)) {
                ...
                $temp = mys_upload_temp_path().'member.'.$this->uid.'.jpg';
                ...
                \Soulcms\Service::M()->db->table('member_data')->where('id', $this->member['id'])->update(['is_avatar' => 1]);
                $this->_json(1, mys_lang('上传成功'), IS_API_HTTP ? \Soulcms\Service::M('member')->get_member($this->uid) : []);
            } else {
        ...
        \Soulcms\Service::V()->display('account_avatar.html');
    }
    ...
    public function password() {
        ...
        if (IS_POST) {
            ...
            if ((empty($post['password2']) || empty($post['password3']))) {
                $this->_json(0, mys_lang('密码不能为空'), ['field' => 'password2']);
            } elseif ($post['password3'] != $post['password2']) {
                $this->_json(0, mys_lang('两次密码不一致'), ['field' => 'password3']);
            } elseif (md5(md5($password).$this->member['salt'].md5($password)) != $this->member['password']) {
                $this->_json(0, mys_lang('原密码不正确'), ['field' => 'password']);
            } elseif (md5(md5($post['password2']).$this->member['salt'].md5($post['password2'])) == $this->member['password']) {
                $this->_json(0, mys_lang('原密码不能与新密码相同'), ['field' => 'password2']);
            }
            // 修改密码
            \Soulcms\Service::M('member')->edit_password($this->member, $post['password2']);
        ...
        \Soulcms\Service::V()->display('account_password.html');
    }
    ...
    public function mobile() {
        ...
        if (IS_POST) {
            $post = \Soulcms\Service::L('input')->post('data', true);
            $value = mys_safe_replace($post['phone']);
            ...
            if (!$this->member['randcode']) {
                ...
                $this->_json(0, mys_lang('验证码储存过期'));
            } elseif ($cache != $value) {
                $this->_json(0, mys_lang('手机号码或验证码不正确'));
            }
            ...
            if ($is_update && $value) {
                ...
                if (!is_numeric($value) || strlen($value) != 11) {
                    $this->_json(0, mys_lang('手机号码格式不正确'));
                } elseif (\Soulcms\Service::M()->db->table('member')->where('id<>'.$this->member['id'])->where('phone', $value)->countAllResults()) {
                    $this->_json(0, mys_lang('手机号码已经注册'));
                } elseif (defined('UCSSO_API') && $rt = ucsso_edit_phone($this->uid, $value)) {
                    if (!$rt['code']) {
                        return mys_return_data(0, mys_lang('通信失败：%s', $rt['msg']));
                    }
                }
                \Soulcms\Service::M()->db->table('member')->where('id', $this->member['id'])->update(['phone' => $value]);
            }

            // 认证号码
            $is_mobile && \Soulcms\Service::M()->db->table('member_data')->where('id', $this->member['id'])->update(['is_mobile' => 1]);
        ...
        \Soulcms\Service::V()->assign([
            'form' => mys_form_hidden(),
            'api_url' =>\Soulcms\Service::L('Router')->member_url('account/mobile_code'),
            'is_update' => $is_update,
            'is_mobile' => $is_mobile,
        ]);
        \Soulcms\Service::V()->display('account_mobile.html');
    }
    ...
    public function mobile_code() {
        ...
        (!is_numeric($value) || strlen($value) != 11) && $this->_json(0, mys_lang('手机号码格式不正确'));
        ...
        \Soulcms\Service::M()->db->table('member')->where('id', $this->member['uid'])->update(['randcode' => $this->member['randcode']]);

        $rt = \Soulcms\Service::M('member')->sendsms_code($value, $this->member['randcode']);
    ...
    public function login() {
        ...
        \Soulcms\Service::V()->display('account_login.html');
    }
    ...
    public function oauth_delete() {
        ...
        \Soulcms\Service::M()->db->table('member_oauth')->where('uid', $this->uid)->where('oauth', $name)->delete();
        $this->_json(1, mys_lang('操作成功'), ['url' =>\Soulcms\Service::L('Router')->member_url('account/oauth')]);
    }
    ...
    public function oauth() {
        ...
        \Soulcms\Service::V()->assign([
            'list' => \Soulcms\Service::M('member')->oauth($this->uid),
            'oauth' => $name,
        ]);
        \Soulcms\Service::V()->display('account_oauth.html');
    }
}
