...
class Pay extends \Soulcms\Common
{
    ...
    public function index() {
        ...
        if (IS_POST) {
            $pay = \Soulcms\Service::L('input')->post('pay');
            $pay['uid'] = $this->member['uid'];
            $pay['username'] = $this->member['username'];
            $money = floatval($pay['money']);
            if (!$money) {
                $this->_msg(0, mys_lang('金额(%s)不正确', $money));
                exit;
            }
            $rt = \Soulcms\Service::M('Pay')->post($pay);
            if (!$rt['code']) {
            ...
            if (IS_API_HTTP) {
                ...
                mys_redirect($url, 'auto');
            }
            exit;
        } else {
            $this->_msg(0, mys_lang('请求错误'));
        }
    }
