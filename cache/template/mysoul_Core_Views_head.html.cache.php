...
<head>
    <meta charset="utf-8" />
    <title><?php if ($meta_title) {  echo $meta_title;  } else {  echo mys_lang('%s - 后台管理平台', SITE_NAME);  } ?></title>
    ...
    <?php if (isset($_GET['isback']) && $_GET['isback']) { ?>
        $(function(){
            mys_tips(<?php echo intval($_GET['iscode']); ?>, "<?php echo $_GET['isback']; ?>")
        });
    <?php } ?>
        function mys_update_cache_all() {
            ...
            $.ajax({type: "GET",dataType:"json", url: admin_file+"?c=api&m=cache_update",
                success: function(json) {
                    layer.close(index);
                    mys_tips(1, "<?php echo mys_lang('全站缓存更新完成'); ?>");
                },
                error: function(HttpRequest, ajaxOptions, thrownError) {
                    layer.closeAll('loading');
                    mys_tips(0, "<?php echo mys_lang('更新失败'); ?>");
                }
            });
        }
        function mys_update_cache_data() {
            var index = layer.load(2, {
                shade: [0.3,'#fff'], //0.1透明度的白色背景
                time: 10000
            });
            $.ajax({type: "GET",dataType:"json", url: admin_file+"?c=api&m=cache_clear",
                success: function(json) {
                    layer.close(index);
                    mys_tips(1, "<?php echo mys_lang('前端数据缓存更新完成'); ?>");
                },
                error: function(HttpRequest, ajaxOptions, thrownError) {
                    layer.closeAll('loading');
                    mys_tips(0, "<?php echo mys_lang('更新失败'); ?>");
