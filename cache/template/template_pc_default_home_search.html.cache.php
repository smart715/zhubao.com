        ...
        <div class="container">
            <div class="row"> 
                <ul class="list clearfix" style="list-style:none;display: flex;justify-content: space-between;">
                        <?php $list_return = $this->list_tag("action=search module=MOD_DIR id=$searchid total=$sototal order=$params[order] catid=$catid urlrule=$urlrule  page=10 pagesize=2"); if ($list_return) extract($list_return, EXTR_OVERWRITE); $count=count($return); if (is_array($return)) { foreach ($return as $key=>$t) { $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
                        <li class="item"> <a class="clearfix" href="<?php echo $t['url']; ?>" title="<?php echo $t['title']; ?>">
                          <div class="txt fr">
                            ...
                            <div class="mark" style="background:none;color:grey;display: flex;
                            justify-content: space-around;"><span>时间：<?php echo mys_date($t['_inputtime'],'Y-m-d'); ?></span>
                                <span>浏览量：<?php echo $t['hits']; ?></span></div>
                            <p class="desc" style="width:800px;margin:10px auto"><?php echo $t['description']; ?></p>
                          </div>
