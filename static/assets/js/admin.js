...
function mys_show_member(name) {
    ...
    var url = admin_file+"?c=api&m=member&name="+$("#mys_"+name).val();
    layer.open({
        type: 2,
...
// 动态执行菜单链接
function mys_admin_menu_ajax(url, not_sx) {
...
// 动态更新缓存
function mys_update_cache(model, namespace) {
    mys_update_cache_all();
}

// 动态执行链接
function mys_load_ajax(msg, url, go) {
    layer.confirm(
        msg,
        {
            icon: 3,
...
// 安装模块提示
function mys_install_module_select(url) {
    layer.confirm(
...
function mys_install_module(url) {
    layer.confirm(
        '你确定要安装到当前站点吗？',
        {
            shade: 0,
...
// 推送模块数据
function mys_module_send(title, url) {
    var width = '50%';
    var height = '60%';
    if (is_mobile_cms == 1) {
        width = height = '90%';
...
// 批量模块数据 ajax
function mys_module_send_ajax(url) {
    url+= '&'+$("#myform").serialize();
    var index = layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 10000
...
// ajax关闭或启用
function mys_ajax_open_close(e, url, fan) {
    var index = layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 5000
    });
...
// 百分百进度控制
function mys_bfb(title, myform, url) {
    layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 百分百提交再进度控制
function mys_bfb_submit(title, myform, url) {
    layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 打开预览文件
function mys_show_file_code(title, url) {
    layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 提交生成静态页面
function mys_submit_htmlfile(myform, url) {
    layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 提交到执行页面
function mys_submit_todo(myform, url) {
    layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 提交到执行页面 post
function mys_submit_post_todo(myform, url) {
    var loading = layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
// 提交到执行sql页面 post
function mys_submit_sql_todo(myform, url) {
    var loading = layer.load(2, {
        shade: [0.3,'#fff'], //0.1透明度的白色背景
        time: 1000
    });
...
function mys_url_module_search() {
    layer.alert('<b>举例</b><hr>'+
...
function mys_url_module_search_page() {
    layer.alert('<b>举例</b><hr>'+
...
function mys_ajax_admin_alert_error(HttpRequest, ajaxOptions, thrownError) {
    layer.closeAll('loading');
    var msg = HttpRequest.responseText;
    if (!msg) {
        mys_tips(0, lang['error']);
    } else {
        layer.open({
            type: 1,
            title: lang['error'],
