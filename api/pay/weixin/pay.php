...
if (IS_API_HTTP) {
    define('APPID', \Soulcms\Service::C()->get_cache('weixin', 'xcx', 'appid'));
    define('APPSECRET', \Soulcms\Service::C()->get_cache('weixin', 'xcx', 'appsecret'));
} else {
...
if (IS_API_HTTP) {
    ...
    if (!$oauth) {
        $return = mys_return_data(0, '服务器没有此用户');
    } else {
        ...
        if (isset($order['code']) && $order['code'] == 0) {
            $return = mys_return_data(0, $order['msg']);
        } elseif ($order["err_code_des"]) {
            $return = mys_return_data(0, $order['err_code_des']);
        } else {
            // 存储支付结果
            $order['sn'] = $sn;
            $tools = new JsApiPay();
            $param = $tools->GetJsApiParameters($order);
            \Soulcms\Service::C()->_json(1, 'ok', json_decode($param, true));
        }
    }
    ...
    if (!$openId) {
        $return = mys_return_data(0, 'openId获取失败');
    } else {
        // 统一下单
        $input = new WxPayUnifiedOrder();
        $input->SetBody($data['title']);
...
<script type="text/javascript">
        ...
        WeixinJSBridge.invoke(
            ...
            function(res){
                if (res.err_msg == "get_brand_wcpay_request:ok") {
                    ...
                    mys_tips(0, "'.mys_lang('付款取消').'");
                } else {
                    mys_tips(0, "'.mys_lang('服务端错误: ').'"+res.err_msg);
                }
            }
        );
    }
...
} elseif (\Soulcms\Service::C()->_is_mobile()) {
    // 手机端H5支付
    $input = new WxPayUnifiedOrder();
    $input->SetBody($data['title']);
    $input->SetOut_trade_no($sn);
    ...
    if (isset($result['code']) && $result['code'] == 0) {
        $return = mys_return_data(0, $result['msg']);
    } elseif ($result["err_code_des"]) {
        $return = mys_return_data(0, $result['err_code_des']);
    } else {
        // 存储支付结果
        $result['sn'] = $sn;
        $code = '
    <img  src="'.mys_qrcode($result["code_url"]).'" style="width:150px;height:150px;margin-top:20px;"/>
    <script>
        function mys_weixin_notify() {
            $.ajax({
                type : "post",
